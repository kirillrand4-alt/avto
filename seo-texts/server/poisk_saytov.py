        ...
        for site, ист in спис:
            ...
            if имя_на_сайте:
                return {'inn': k['inn'], 'name': k['name'][:60], 'site': site,
                        ...
                        'проверено': len(отказы) + 1}
            # Открылся, но ни ИНН, ни имени. Кандидатом держим, а перебор НЕ
            # прекращаем: владелец 19.08 — «там где мы отбросили кандидата тоже
            # стоит проверить следующих». Ниже в той же выдаче нередко стоит
            # сайт с ИНН на странице, и подтверждённый всегда лучше кандидата.
