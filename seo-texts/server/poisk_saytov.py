...
def _слить_копилку(c):
    ...
    if not копилка:
        return {}
    # ПЕРВЫЙ ОТКАЗ ОСТАНАВЛИВАЕТ СЛИВ. Иначе сто накопленных записей при занятой
    # базе — это сто ожиданий таймаута подряд, и пачка встанет ещё до старта.
    # База занята для первой записи — занята и для сотой; ждём следующего круга.
    легло, осталось, занята = 0, [], False
    for з in копилка:
        if занята:
            осталось.append(з)
            continue
...
def прогон(skolko=1000, potokov=8):
    ...
    # СНИМОК ЗАНЯТЫХ ДОМЕНОВ — ДО ПУЛА, отдельным соединением только на чтение.
    ...
    # обходом. Исключение одно: ИНН самой компании напечатан на страницах.
    #
    # Читаем ОТДЕЛЬНЫМ соединением и закрываем его до первой записи. Раньше этот
    # обход 166 тысяч строк шёл тем же соединением, которым потом писали, и оно
    # оставалось в открытой ЧИТАЮЩЕЙ транзакции. Превратить её в пишущую sqlite
    # не даёт, если рядом кто-то писал: снимок устарел, и «database is locked»
    # приходит НЕЗАВИСИМО от busy_timeout. Отсюда картина 28.08, которая сбила
    # меня с толку: оба конвейера стоят одновременно, гашение одного не
    # освобождает другого, а «держателя» замка нет вовсе.
    ...
    # ТЕРПЕНИЕ ЗДЕСЬ ВРЕДНО: для ожидания есть копилка. Стояло 15 000 мс, и
    # каждый запрос к занятой базе висел до пятнадцати секунд — последовательно,
    # в главном потоке, на каждой находке. Замер 28.08: пачка в 60 компаний шла
    # 654 секунды, то есть 261 секунда потока на компанию, при том что запрос к
    # xmlriver и открытие сайта занимают вместе 2,7 секунды. Сто раз разницы
    # съедало ожидание замка, а не работа. Две секунды — на случай короткой
    # чужой транзакции, дальше сразу в копилку.
    ...
    # ПИШЕМ ПО МЕРЕ ГОТОВНОСТИ, А НЕ В КОНЦЕ ПАЧКИ. Владелец 28.08: «а пачки
    # обязательно по 100, почему не делаю по 1?» По одной нельзя — пачка это
    # единица параллельности (внутри неё работают все потоки) и амортизация
    # шестнадцати секунд на подбор целей. Но копить результаты в памяти до конца
    # пачки было незачем: двадцать минут работы висели незафиксированными, и
    # падение процесса стирало их целиком. Теперь каждая компания фиксируется
    # сразу, как досчиталась, — пачка осталась большой ради скорости, а
    # надёжность стала поштучной.
    # ПОСЛЕ ПЕРВОГО ОТКАЗА НЕ ПРОБУЕМ ДО КОНЦА ПАЧКИ. Сверки по расписанию
    # держат базу по четверти часа: если она занята на первой находке, занята и
    # на тридцатой. Пробовать каждую — значит тридцать раз отстоять таймаут
    # впустую. Один отказ, и вся пачка едет в копилку; слив на следующем круге.
    занята = False
    отложить, журнал = [], []
    жф = open(LOG, 'a', encoding='utf-8')

    def _сбросить_журнал():
        if not журнал:
            return
        for стр in журнал:
            жф.write(json.dumps(стр, ensure_ascii=False) + '\n')
        жф.flush()
        os.fsync(жф.fileno())
        журнал.clear()

    поимённо = {str(k['inn']): k for k in задачи}
    with ThreadPoolExecutor(max_workers=potokov) as ex:
        for r in ex.map(одна, задачи):
            k = поимённо.get(str(r.get('inn'))) or {'name': '', 'city': ''}
            if r.get('site'):
            ...
            if r.get('site'):
                итог['нашли'] += 1
                з = {'inn': r['inn'], 'name': k.get('name') or '',
                     'city': k.get('city') or '', 'site': r['site'],
                     'verdikt': r.get('verdikt') or ''}
                if not занята and _записать_находку(c, з):
                    итог['подтвердили' if r.get('verdikt') else 'в_кандидаты'] += 1
                else:
                    занята = True
                    # База занята — не ждём и не теряем: в копилку, следующий
                    # круг перельёт.
                    отложить.append(з)
                    _отложить([з])
                # домен занят теперь и нами: внутри одной пачки два ИНН не должны
                # получить один сайт (раньше снимок был статичным и это пропускал)
                занятые.setdefault(PL.домен(r['site']), set()).add(str(r['inn']))
            else:
                итог['не_нашли'] += 1
                if _ne_iskali(r.get('src')):
                    итог['канал_не_ответил'] = итог.get('канал_не_ответил', 0) + 1
            # ЖУРНАЛ — ПОСЛЕ базы (или копилки): запись здесь означает «компанию
            # больше не берём», и раньше она стояла ДО базы, из-за чего
            # непрошедшая запись сайта стоила компании навсегда.
            журнал.append(r)
            if len(журнал) >= 20:
                _сбросить_журнал()
    _сбросить_журнал()
