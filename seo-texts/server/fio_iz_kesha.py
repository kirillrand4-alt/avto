...
def main():
    ...
    # ПИШЕТ ОДИН ПОТОК. Параллельные писатели в sqlite дерутся за замок, а мы
    # только что потратили полдня, разбирая ровно эту драку.
    всего = len(цели)
    получено = 0
    while получено < всего:
        ...
        if сбой:
            ...
            with замок_журнала:
                журнал.write(json.dumps({'инн': инн, 'сбой': сбой},
                                        ensure_ascii=False) + '\n')
                журнал.flush()
            continue
        ...
        итог['разобрано'] += 1
        with замок_журнала:
            журнал.write(json.dumps({'инн': инн, 'адресов': 0,
                                     'найдено': [[a, f, p] for a, f, p in найдено],
                                     'ts': time.strftime('%H:%M:%S')},
                                    ensure_ascii=False) + '\n')
            журнал.flush()
            os.fsync(журнал.fileno())
        if итог['разобрано'] % 50 == 0:
