...
if (vzyato.Count == 0)
{
    // Пауза перед выходом: при повторе выполнения десять потоков на пустом
    // задании крутились вхолостую по десятку запусков в секунду и забивали лог
    // (видно на снимке владельца: «задание пусто» двенадцать раз за 11 секунд).
    project.SendInfoToLog("задание пусто — жду 30 с", true);
