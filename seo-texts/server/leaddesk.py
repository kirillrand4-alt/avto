...
class LeadDesk:
    ...
    def queue(self, *, status=None, assigned_to=None, unassigned=False,
        ...
        # napravlenie — «кц» или «мейер» (владелец 20.08: «в ленте лидов ещё
        # возможность выбрать ответы по направлениям тоже надо»). Считается по
        # ящику, который вёл переписку, — см. store._sql_napravleniya_lida.
        return self._store.list_leads(
            status=status, assigned_to=assigned_to, unassigned=unassigned,
            reply_kind=reply_kind, napravlenie=napravlenie,
