    ...
    def _send_live_inner(self, row: dict, subject: str, body: str,
        ...
        if row.get("kind") == "reply":
            ...
            self._sender.send_reply(
                ...
                # Вложения оператора. Живут в panel_json черновика (пути на
                # диске, не содержимое) — схему очереди менять не пришлось,
                # а sender читает файлы в момент сборки письма.
                vlozheniya=(panel_j.get("vlozheniya")
                            if isinstance(panel_j, dict) else None))
