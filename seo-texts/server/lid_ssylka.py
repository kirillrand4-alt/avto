...
написал, чем компания занимается, кому звонить. И НЕ нужны две вещи:
  ...
  * адреса почты — ни наш ящик, ни адрес получателя. Отдел продаж работает
    голосом, а рассылочные ящики и персоны рассылки наружу не показываем;
...
def bez_adresov(текст: str) -> str:
    """Любые адреса почты заменяем на пометку, а не на пустоту.

    Пустота выглядит как потеря текста («пишите на  »), а пометка честно
    говорит, что адрес есть, но наружу не отдан.
    """
    return _АДРЕС.sub('[адрес скрыт]', str(текст or ''))
