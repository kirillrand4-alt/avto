...
def _tekst_stranicy(html: str) -> str:
    ...
    Скрипты и стили вырезаем разбиением, а не регуляркой: ленивое «.*?» с
    флагом re.S по мегабайтному html стоило секунд на страницу.
    """
    т = str(html or '')
    for тег in ('script', 'style'):
        куски = re.split(r'(?i)<%s\b' % тег, т)
        if len(куски) > 1:
            собрано = [куски[0]]
            for к in куски[1:]:
                i = к.lower().find('</%s>' % тег)
                собрано.append(к[i + len(тег) + 3:] if i >= 0 else '')
            т = ' '.join(собрано)
    т = re.sub(r'<[^>]+>', '\n', т)
    for a, b in (('&nbsp;', ' '), ('&amp;', '&'), ('&quot;', '"')):
        т = т.replace(a, b)
...
# Пределы разбора. Телефоны живут в подвале и на контактных страницах, а у
# завода в кэше бывает две сотни страниц по мегабайту: сбор 21.08 упёрся в
# них и пошёл со скоростью десяти компаний в минуту вместо полутора сотен.
# Контактные разделы берём первыми, остальное — сколько влезет в предел.
_ПРЕДЕЛ_СТРАНИЦ = 40
_ПРЕДЕЛ_ЗНАКОВ = 150000
_КОНТАКТНЫЕ = ('contact', 'kontakt', 'связ', 'about', 'o-kompanii', 'company',
               'rukovod', 'sotrud', 'staff')
...
def _kontakty_so_stranic(страницы) -> dict:
    ...
    отобранные = sorted(
        страницы or [],
        key=lambda п: 0 if any(к in str(п[0]).lower() for к in _КОНТАКТНЫЕ) else 1
    )[:_ПРЕДЕЛ_СТРАНИЦ]
    for url, html in отобранные:
        # РЕЖЕМ ДО РАЗБОРА, а не после: снятие тегов регуляркой по мегабайтному
        # html и было тем, на чём сбор буксовал, — предел после него не спасал
        текст = _tekst_stranicy(str(html or '')[:_ПРЕДЕЛ_ЗНАКОВ])
