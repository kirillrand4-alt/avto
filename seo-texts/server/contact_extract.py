...
# ПОЧЕМУ ТАК СЛОЖНО, А НЕ `[...]+@[...]+\.[a-z]{2,}` (правка 30.08). Простая
# запись губительна на больших страницах: для КАЖДОЙ позиции внутри длинной
# череды букв и цифр (base64, минифицированный CSS, хеши) движок съедает всю
# череду, не находит собаку и откатывается назад по одному символу. На странице
# в 300 тысяч знаков разбор одной страницы занимал 124 секунды, а у компании
# таких страниц восемь — 16 минут на одну карточку. Так и встал разбор кэша.
# Лечится двумя вещами: взгляд назад не даёт начинать внутри череды (позиция
# пробуется один раз, а не n раз), а границы {1,64}/{1,63} обрывают откат.
EMAIL_RE = re.compile(
    r'(?<![A-Za-z0-9._%+\-])[A-Za-z0-9._%+\-]{1,64}'
    r'@[A-Za-z0-9\-]{1,63}(?:\.[A-Za-z0-9\-]{1,63}){0,4}\.[A-Za-z]{2,24}')
# четыре и больше разделителей подряд — не номер, а мусор разметки
_РАЗДЕЛИТЕЛИ = re.compile(r'[\s\-().\u00a0/\\|_=+*#~]{4,}')
...
def extract(html):
    ...
    phones = set()
    # ПОРОГ ПО РАЗМЕРУ. Проверенный разбор enrich_contacts вычищает разметку
    # выражениями вида `<svg\b.*?</svg>` с точкой-любым-символом. На странице с
    # незакрытым тегом каждое такое совпадение сканирует остаток файла целиком,
    # и на сотнях килобайт это превращается в квадрат. Замер 30.08: разбор кэша
    # встал НАСМЕРТЬ на четвёртой компании очереди — процесс жёг ядро больше
    # трёх минут на одной странице. Крупные страницы разбираем своей простой
    # регуляркой: она линейна, а рубеж против реквизитов у нас всё равно стоит
    # вторым — на гейте слива.
    крупная = len(blob) > 200000
    if _EC is not None and not крупная:
        # ДЛИННЫЕ ЦЕПОЧКИ РАЗДЕЛИТЕЛЕЙ СХЛОПЫВАЕМ (правка 30.08). Регулярка
        # телефона в enrich_contacts допускает разделители между любыми двумя
        # цифрами: `(?:[\s\-().]*\d)` повторяется семь раз. На обычной странице
        # это безобидно, а на строке из сотен пробелов, точек или дефисов подряд
        # — база64, «...........», псевдографика таблиц — перебор вариантов
        # взрывается, и разбор ОДНОЙ страницы встаёт на часы. Разбор кэша так и
        # встал: процесс жёг ядро десять минут и не закрыл ни одной компании.
        # В живом номере четырёх разделителей подряд не бывает, поэтому такие
        # цепочки заменяем одним пробелом — ни один настоящий номер не теряется.
        текст_для_тел = _РАЗДЕЛИТЕЛИ.sub(' ', _EC.без_графики(blob))
        for s in _EC.phone_strings(текст_для_тел):
            d = re.sub(r'\D', '', s)
            if 10 <= len(d) <= 11:
                phones.add(d)
    else:
        for p in _PHONE_RE.findall(_РАЗДЕЛИТЕЛИ.sub(' ', text)):
