...
# ТРЕТЬЯ СТУПЕНЬ: конкурент виден в СОБСТВЕННОМ паспорте. Появилась 26.08 после
# ООО «Энергоремкомплект» (ИНН 6679054575, erk-ekb.ru): письмо ушло, ответили «мы
# тоже производим поршневые компрессоры». Обе прежние ступени промахнулись честно —
# ОКВЭД у них металлообработка (25.62, компрессорного нет вовсе), а провайдер-судья
# по сайту не проходил, потому что обогащения по компании не было. При этом паспорт,
# собранный за три дня до письма, перечислял в «продукции» винтовые и поршневые
# компрессоры. Улика лежала в нашей же базе, и её никто не читал.
#
# Смотреть можно ТОЛЬКО поле «продукция» — это то, что компания делает и продаёт.
# Соседнее «оборудование_линии» — то, чем она пользуется, а «разбор_КЦ» — наш
# собственный вывод о потреблении сжатого воздуха; первый вариант скана залез в
# «разбор_КЦ» и записал в конкуренты асфальтовый завод, у которого в уликах стояло
# «воздух_точно: Компрессорных». Это признак КЛИЕНТА, а не соперника.
PASPORT_KONKURENT = (
    ('компрессоры', _re.compile(
        r'(винтов\w+|поршнев\w+|спиральн\w+|мембранн\w+|центробежн\w+|воздушн\w+|'
        r'газов\w+|дожимн\w+|безмаслян\w*|азотн\w+|передвижн\w+|мобильн\w+)\s+компрессор|'
        r'компрессор\w*\s+(винтов|поршнев|воздушн|высокого давления|низкого давления)|'
        r'производств\w*\s+компрессор|изготовлени\w*\s+компрессор|'
        r'компрессорн\w+\s+(установк|оборудован|техник)', _re.I)),
    ('генераторы газов', _re.compile(
        r'генератор\w*\s+(азот|кислород)|(азотн|кислородн)\w+\s+(станци|генератор|установк)|'
        r'мембранн\w+\s+(азотн|газоразделительн)|адсорбцион\w+\s+(азотн|генератор)|'
        r'производств\w*\s+(азот|кислород)', _re.I)),
    ('фотосепараторы', _re.compile(
        r'фотосепарат|фото-сепарат|оптическ\w+\s+сепарат', _re.I)),
    ('рентген-инспекция', _re.compile(
        r'рентген\w*[- ]?(инспекц|телевизион|интроскоп)|инспекцион\w+\s+рентген|'
        r'рентгеновск\w+\s+(инспекц|контрол\w+ качеств)', _re.I)),
    ('ЗИП к компрессорам', _re.compile(
        r'(запчаст|запасн\w+ част|зип|комплектующ)\w*\s+(к|для)?\s*компрессор', _re.I)),
)
# СТРОИТЬ компрессорную станцию — не то же, что делать компрессоры. Подрядчики
# нефтегаза («Строительство дожимных компрессорных станций», sfera-tula.ru,
# ngtii.ru) попадали в конкуренты по слову «дожимные компрессорные», хотя они нам
# клиенты: компрессоры они покупают. Если КАЖДОЕ упоминание компрессора в продукции
# стоит в стройке или проектировании — признак снимаем.
_STROYKA = _re.compile(
    r'строительств|строим|монтаж|шефмонтаж|проектирован|реконструкц|'
    r'капитальн\w+\s+ремонт|пусконаладк|обвязк', _re.I)
_UPOMINANIE = _re.compile(r'компрессор', _re.I)


def konkurent_po_pasportu(facts_json):
    """Конкурент ли компания по своему же паспорту. Вход — facts_json (строка
    или разобранный словарь). Возврат: (bool, [названия признаков]).

    Ступень информативная и намеренно узкая: слово «компрессор» должно стоять при
    товаре. Подрядчик, который СТРОИТ дожимные компрессорные станции для нефтегаза,
    сюда не попадает — он нам клиент, а не соперник."""
    if not facts_json:
        return (False, [])
    if isinstance(facts_json, str):
        try:
            facts_json = json.loads(facts_json)
        except Exception:  # noqa: BLE001
            return (False, [])
    if not isinstance(facts_json, dict):
        return (False, [])
    сырьё = facts_json.get('продукция')
    # Разбираем ПОЗИЦИЯМИ, а не сплошным текстом. Сплошной текст даёт течь через
    # границу позиции: у «Монтаж компрессорных станций | Производство винтовых
    # компрессоров» слово «Монтаж» попадало в окно перед вторым упоминанием, и
    # настоящий производитель уходил из конкурентов.
    if isinstance(сырьё, (list, tuple)):
        pozicii = [_re.sub(r'\s+', ' ', str(x)) for x in сырьё]
    else:
        pozicii = [_re.sub(r'\s+', ' ', x) for x in
                   _re.split(r"['\"],\s*['\"]|\s*[;|]\s*",
                             str(сырьё or '').strip("[]'\" "))]
    hits = []
    for поз in pozicii:
        if not поз.strip():
            continue
        свои = [имя for имя, рег in PASPORT_KONKURENT if рег.search(поз)]
        if свои == ['компрессоры'] and _STROYKA.search(поз):
            continue          # строит станции, а не делает компрессоры
        hits += свои
