...
# ОКВЭД-КОНКУРЕНТЫ: коды, означающие, что компания САМА производит оборудование,
# которое мы продаём → это конкурент, не покупатель. Проверять по ВСЕМ ОКВЭД
# (основной+доп): у КОНАР основной 28.14 (арматура — потенц. клиент), но в доп
# 28.13 (компрессоры) → конкурент. Владелец 2026-07-24: «убрать этот оквед, чтобы
# компании где он есть не попадали в рассылку». Пока подтверждён только 28.13;
# 28.12/28.25/28.93 — кандидаты (в okved_audit считаются отдельно для решения).
COMPETITOR_OKVEDS = {
    '28.13': 'производство насосов и компрессоров (конкурент КЦ)',
}
# кандидаты на исключение — показываем счётчики, НЕ исключаем без слова владельца
COMPETITOR_OKVEDS_CANDIDATES = {
    '28.12': 'гидравлическое/пневматическое силовое оборудование',
...
def is_competitor_by_okved(*okved_texts, extra=None):
    """True, если среди ВСЕХ ОКВЭД компании есть код-конкурент (префиксный матч,
    28.13.х матчит 28.13). extra — доп. набор кодов (например кандидаты, если
    владелец их подтвердит). Возврат: (bool, [сматченные коды])."""
