...
def в_поток(запись):
    with _ПОТОК_L:
        try:
            os.makedirs(os.path.dirname(ПОТОК), exist_ok=True)
            with open(ПОТОК, 'a', encoding='utf-8') as f:
...
# ---------------------------------------------------------------- прогон
def сайт(метка, url, пагинация, ддток, применять, db):
    html = качать(url)
    if not html:
        return []
    страницы = [(url, html)]
    ...
    # параллельно, потому что шлюз рубит длинные потоки
    итог = []
    лок = threading.Lock()

    def работа(z):
        u, т, вид = z
        for эл in провайдер(т):
    ...
    потоки = []
    for z in задания[:60]:
        while len([p for p in потоки if p.is_alive()]) >= 5:
            time.sleep(0.25)
        p = threading.Thread(target=работа, args=(z,), daemon=True)
        p.start()
        потоки.append(p)
    for p in потоки:
        p.join(timeout=150)
    ...
    for эл in итог:
        ...
        if почта and EC._is_junk_email(почта):
            почта = ''
        зап = {'сайт': метка, 'url': эл.get('_url'), 'вид': эл.get('_вид'),
