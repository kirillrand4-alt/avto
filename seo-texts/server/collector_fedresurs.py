...
PREDMET_PROM = re.compile(
    ...
    r'упаковочн|фасовочн|термопласт', re.I)
...
PREDMET_NE_NASH = re.compile(
    ...
    r'погрузчик|трактор|комбайн|блок-модуль|вагон|баржа|судно|катер', re.I)
...
def _hotness(code, predmet):
    ...
    """
    h = (TIPY_CAPEX.get(code) or {}).get('hotness', 2)
    if predmet:
