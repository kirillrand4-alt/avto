...
# Сколько адресов дописываем тем, кто уже в партии. Ноль — без потолка
# (владелец 30.08: «давай без потолка»). Панель всё равно выбирает одного
# адресата при отправке, а лишние адреса — это запас на случай отказа первого.
ДОБАВОК_НА_КОМПАНИЮ = int(os.environ.get('ZALIV_POTOLOK', 0))
ПАРТИЯ = 'партия-935'
...
def залить(делать=False):
    ...
    строки = []
    for i in новые:
        # Выбираем лучший из СВОБОДНЫХ адресов, а не лучший вообще. Прежняя
        # версия брала лучший адрес и, если он уже стоял в recipients, теряла
        # компанию целиком — хотя другие её адреса были свободны. На прогоне
        # 31.08 так отвалилось около сотни компаний из четырёхсот.
        свободные = [x for x in свои[i] if x['email'] not in уже_адреса]
        for a in _лучшие(свободные, 1):
            строки.append((i, a, 'новая компания'))
    дописано = 0
    for i in добор:
        свободные = [x for x in свои[i] if x['email'] not in уже_адреса]
        if ДОБАВОК_НА_КОМПАНИЮ:
            есть = sum(1 for a in свои[i] if a['email'] in уже_адреса)
            свободные = _лучшие(свободные, max(0, ДОБАВОК_НА_КОМПАНИЮ - есть))
        else:
            свободные = _лучшие(свободные, len(свободные))
        for a in свободные:
