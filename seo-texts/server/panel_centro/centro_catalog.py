...
def facts(inn: str, limit: int = 500) -> list[dict]:
    ...
    for row in rows:
        ...
        result.append(item)
    result.sort(key=_dostovernost)
    return result


# Порядок фактов на карточке. Просьба владельца дословно: «сделай сортировку
# фактов о компании чтобы наверху были только достоверно известные ВОЗДУШНЫЕ
# ЦЕНТРОБЕЖНИКИ». До этого порядок был по дате, и наверху 163-фактовой карточки
# стояли три подряд «тип не установлен (марка не опознана)» — просто потому что
# они свежее. Продавец видел первым самое непонятное.
#
# Достоверность считаем по тому, что СКАЗАНО, а не по тому, что мы вывели:
#   0  центробежная + воздух + марка опознана  — доказано целиком
#   1  центробежная + воздух, марка не опознана
#   2  центробежная, среда не названа
#   3  среда воздух, тип не установлен
#   4  всё остальное непустое
#   9  отсеянное как мусор — всегда в конце
# Внутри разряда — свежее выше: дата остаётся вторичным ключом, а не главным.
_ЦЕНТРОБЕЖНАЯ = ("центробеж", "турбокомпрессор", "турбовоздуход", "нагнетател")
_ВОЗДУХ = ("воздух", "атмосферн")
_НЕ_НАЗВАНА = ("", "не названа", "среда не названа", "не установлен",
               "не установлена", "—", "-")
# ПОДТВЕРЖДЕНИЕ МАРКИ — не «поле не пустое», а сказанное словами в разборе.
# Владелец показал, почему разница решает всё. Наверху стояло:
#     Модель/марка  C-738 · центробежная · воздух
#     цитата: «Центробежная воздуходувка SF 1_8 SP, ПОЗ. C-738, инв. № 1392»
# то есть в поле марки лежит ПОЗИЦИЯ ПО СХЕМЕ, а настоящая марка (SF 1_8 SP)
# осталась в тексте. А утоплен был честный факт:
#     Модель/марка  К-250-61-2 · «центробежность: марка опознана справочником»
# только потому, что среда у него «не названа». Непустое поле оказалось сильнее
# доказанной марки — это и есть перевёрнутый порядок.
_ПОДТВЕРЖДЕНА = ("марка опознана", "опознана справочником", "справочником семейств",
                 "перепрос провайдера", "подтверждена перепросом",
                 "по паспорту", "паспорт изготовителя", "по серии")
# позиция по технологической схеме: «поз. С-738», «поз. М61/2», «поз. 101»
_ПОЗИЦИЯ = re.compile(r"поз(?:иция)?\.?\s*№?\s*([A-ZА-Яа-яa-z]?-?\d+[/\-]?\d*)",
                      re.I)


def _marka_nastoyashchaya(item: dict) -> bool:
    """Марка доказана, а не просто записана в поле."""
    марка = str(item.get("model") or "").strip()
    if not марка:
        return False
    дока = (str(item.get("evidence") or "") + " "
            + str(item.get("equipment_type") or "")).casefold()
    if any(к in дока for к in _ПОДТВЕРЖДЕНА):
        return True
    цитата = str(item.get("quote") or "")
    ключ = марка.replace(" ", "").casefold()
    for м in _ПОЗИЦИЯ.finditer(цитата):
        if м.group(1).replace(" ", "").casefold() == ключ:
            return False   # это позиция по схеме, а не марка машины
    return True


def _dostovernost(item: dict) -> tuple:
    if item.get("rejected"):
        разряд = 9
    else:
        тип = str(item.get("equipment_type") or "").casefold()
        среда = str(item.get("medium") or "").casefold().strip()
        центр = any(к in тип for к in _ЦЕНТРОБЕЖНАЯ)
        воздух = any(к in среда for к in _ВОЗДУХ)
        марка = _marka_nastoyashchaya(item)
        # ДОКАЗАННАЯ МАРКА ВЕСОМЕЕ НАЗВАННОЙ СРЕДЫ: у центробежной машины среда
        # чаще всего воздух и просто не выписана, а марка либо опознана
        # справочником, либо нет — тут догадок меньше.
        if центр and марка and воздух:
            разряд = 0
        elif центр and марка:
            разряд = 1
        elif центр and воздух:
            разряд = 2
        elif центр:
            разряд = 3
        elif воздух:
            ...
            разряд = 5
    дата = str(item.get("event_date") or "")
    return (разряд, 1 if not дата else 0, _obratnaya_data(дата))


def _obratnaya_data(дата: str) -> str:
    """Ключ для убывающей сортировки строкой: инвертируем каждую цифру."""
    ц = "".join(ch for ch in дата if ch.isdigit())
    # даты приходят и как 30.12.2014, и как 2014-12-30 — сводим к ГГГГММДД
    if len(ц) == 8 and дата[:4].isdigit():
        norm = ц
    elif len(ц) == 8:
        norm = ц[4:] + ц[2:4] + ц[:2]
    else:
        norm = ц.ljust(8, "0")[:8]
    return "".join(str(9 - int(з)) for з in norm)
