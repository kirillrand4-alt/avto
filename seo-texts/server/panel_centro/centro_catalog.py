...
def _dostovernost(item: dict) -> tuple:
    if item.get("rejected"):
        ...
        нет_среды = среда in _НЕ_НАЗВАНА
        опознана = bool(марка) and "не опознан" not in тип and "не установлен" not in тип
        if центр and воздух and опознана:
            разряд = 0
        elif центр and воздух:
            разряд = 1
        elif центр and нет_среды:
            разряд = 2
        elif воздух:
            разряд = 3
        else:
            разряд = 4
    # дата убывающе: пустую дату вниз внутри своего разряда
