...
не ушли ни НАШИ адреса почты, ни подпись, с которой письмо было отправлено
(адрес получателя показывать можно — владелец 20.08), и что отозванная ссылка
перестаёт работать.
"""
import json
import re
...
if код == 200:
    ...
    итог['ПРОВЕРКА'] = {
        ...
        'подпись_видна_НЕ_ДОЛЖНА': 'С уважением' in стр,
        'наш_ящик_виден_НЕ_ДОЛЖЕН': sorted(
            a for a in адреса if a.split('@')[-1].lower() in наши),
        'наших_доменов_знаем': len(наши),
