...
class Sender:
    ...
    def send_reply(self, *, to_email: str, subject: str, body: str,
                   ...
                   vlozheniya: Optional[list] = None) -> SendResult:
        ...
        mime_bytes = self._build_mime(headers, rendered, vlozheniya=vlozheniya)
    ...
    def _build_mime(self, headers: dict[str, str], rendered: RenderedMessage,
                    *, pixel_url: Optional[str] = None,
                    vlozheniya: Optional[list] = None) -> bytes:
        msg = EmailMessage()
        ...
        # ВЛОЖЕНИЯ (владелец 19.08: «сделай возможность прикреплять файлы как в
        # настоящей почте»). Кладём ПОСЛЕ заголовков: add_attachment сам
        # переводит письмо в multipart/mixed, сохраняя текст и HTML-альтернативу
        # внутри. Каждый файл читаем с диска в момент сборки — в очереди
        # подтверждений хранится только путь, а не содержимое.
        import mimetypes as _mt
        for в in (vlozheniya or []):
            путь = (в or {}).get("path") if isinstance(в, dict) else None
            if not путь or not os.path.exists(путь):
                logger.warning("вложение пропало с диска: %r", путь)
                continue
            имя = str((в.get("name") or os.path.basename(путь)))
            тип, _ = _mt.guess_type(имя)
            главный, _, под = (тип or "application/octet-stream").partition("/")
            with open(путь, "rb") as f:
                msg.add_attachment(f.read(), maintype=главный,
                                   subtype=под or "octet-stream", filename=имя)
