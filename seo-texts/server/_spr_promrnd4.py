# -*- coding: utf-8 -*-
"""promrnd: ПЕРЕСЪЁМ сайта правильным селектором (ссылка перед «Перейти на сайт
компании»). Мой прежний общий сборщик внешних хостов хватал баннер подкрышей.рф —
ровно та ошибка, из-за которой в нашей базе tenderguru.ru стоял у 505 компаний."""
import io
import json
import os
import re
...
n = 0
for i in ids:
    if i in done or time.time() - t0 > 420:
        continue
    time.sleep(1.2)
...
print(json.dumps({
    'карточек': len(recs), 'с_ИНН': len(инны), 'с_сайтом': sum(1 for x in recs if x['site']),
    'уникальных_доменов': len(дом),
    'домен_на_2+': sorted(((len(v), k) for k, v in дом.items() if len(v) > 1), reverse=True)[:6],
    'наших': len(наши), 'НЕ_наших': len(set(инны)) - len(наши),
    'наших_без_сайта': sum(1 for r in наши.values() if not r[3] and not r[4]),
    'находок': len(нах),
    'примеры': [[x['inn'], (наши[x['inn']][1] or '')[:28],
                 int(наши[x['inn']][2] or 0) // 1000000, x['site'][:40]] for x in нах[:10]],
