...
def _pdf_text(blob, страниц=25):
    ...
    Сканы (текстового слоя нет) отдадут пустоту — и это правильный ответ:
    распознавать картинки нам нечем, и делать вид, что прочли, мы не будем.
    """
    try:
        import pypdf
    except Exception:  # noqa: BLE001
...
def find_zakupki_contacts(inn, max_cards=12):
    ...
    for it in items[:max_cards]:
        ...
        try:
            ...
            пм = re.search(r'(?:Наименование|Предмет|Объект)\s+(?:закупк\w+|'
...
def _блоки(html):
    ...
    строк не нашлось.
    """
    строки = [m.group(1) for m in _TR_RE.finditer(html or '')]
    годные = [s for s in строки if _фио(_текст(s))]
    if годные:
        return [('таблица', s) for s in годные]
    карточки = [m.group(1) for m in _CARD_RE.finditer(html or '')]
    return [('карточка', c) for c in карточки if _фио(_текст(c))]
...
def people_from_html(html, url=''):
    ...
    for вид, блок in _блоки(html):
        ...
        if not (pm or тел or em):
            continue
        люди.append({'person': фио, 'post': _clean_post(pm.group(1)) if pm else '',
