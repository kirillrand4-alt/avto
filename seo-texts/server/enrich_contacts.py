...
# (XMLRIVER_CHANNELS).
#
# Дефолт поднят с 4 до 8 (владелец 16.08 показал график потоков за сутки): из десяти
# доступных каналов мы весь день держали один-четыре, то есть оплаченный канал
# простаивал больше, чем работал. Восемь — с запасом в два слота на процессы
# обогащения, которые ходят в тот же аккаунт: их всплески видно на том же графике.
