...
vk(1) | browser/гиперлокал(1) | regional(1/4) | frp(2, бюджет подтверждён) |
zakupki ЕИС(3) | google news(4) | xmlriver Я+G SERP(4, движок свипа) | hh вакансии(5) |
**rss(2)**. Item: {title, link, pubDate, source, tier, collector, query}.

**У RSS ДВЕ РАЗНЫЕ БАЗЫ, и их легко перепутать** (проверено 17.09, здесь стояло
«regional RSS 100+» — неверно):

* коллектор **`regional`** читает каталог `news-sources.json` с дропа, а в нём
  **14 федеральных лент** (lenta, rbc, tass, interfax, vedomosti, kommersant,
  izvestia, finmarket и ещё шесть), тиры 3-4. Ста лент там нет и не было;
* коллектор **`rss`** читает таблицу `donors` в enrich.db — это ленты САЙТОВ,
  найденные автодискавери по доменам, которые уже давали нам события. На
  17.09: 799 доменов, у 115 живая лента, 145 помечены `no-rss`, **539 вообще
  не проверялись**. Пополняется прогоном `{'rss_discover': 'db'}`.

Включать в прогон надо ОБА: список `collectors` без `rss` оставляет базу
донорских лент нетронутой, и это не видно ни по логам, ни по числу событий.
