          ...
          <div className="confirm-queue-list">
            {показ.length === 0 && (
              <div className="muted small" style={{ padding: "8px 10px" }}>
                {/* Ловушка двух фильтров: владелец 12.08 выбрал группу
                    «металлообработка» и направление Meyer — писем Meyer в
                    компрессорной группе нет, и список молча опустел при
                    заголовке «писем в группе: 97». Пустоту надо объяснять,
                    а не показывать. */}
                {напр !== "все" && list.length > 0 ? (
                  <>
                    в группе{группа ? ` «${группа}»` : ""} есть письма
                    ({list.length}), но среди них нет направления{" "}
                    {напр === "kc" ? "«КЦ»" : "«Meyer»"}.{" "}
                    <button type="button" className="confirm-empty-link"
                            onClick={() => выбратьНапр("все")}>
