...
ответ = (
    ...
    '+7 985 991 29 58\n\n'
    '----------------\n'
    'Кому: director@catering-technology.com (director@catering-technology.com);\n'
    'Тема: Контроль включений в готовой продукции;\n'
    '20.08.2026, 09:04, "Юрий Кузьмин, Meyer" <yk@sort-systems.ru>:\n\n'
    'Добрый день!\n'
...
итог = {
    ...
    'НАШ_менеджер_на_странице': [x for x in ('Кузьмин', 'Цейзер', 'Ляпин')
                                 if x in стр],
    'шапка_цитаты_ушла': 'Кому: director@catering' not in стр,
    'наше_письмо_не_дублируется': 'рентген-инспекция' not in стр,
    'страниц_у_контактов': sum(len(p.get('stranicy') or [])
                               for p in (карта.get('pochty') or [])) + sum(
        len([и for и in (t2.get('istochniki') or []) if и.get('url')])
        for t2 in (карта.get('telefony') or [])),
...
print(json.dumps({'ГЛАВНОЕ': {
    'наш_менеджер': итог['НАШ_менеджер_на_странице'],
    'наш_ИНН': итог['наш_ИНН_на_странице'],
    'шапка_цитаты_ушла': итог['шапка_цитаты_ушла'],
    'наше_письмо_не_дублируется': итог['наше_письмо_не_дублируется'],
    'подпись_клиента_цела': итог['подпись_клиента_цела'],
    'страниц_у_контактов': итог['страниц_у_контактов'],
