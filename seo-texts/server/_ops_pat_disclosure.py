...
def квотить(u):
    """Проценты для не-ASCII в пути.

    Замерено: тот же PDF отчёта об итогах голосования по СЫРОМУ адресу с
    кириллицей отдаёт 0 символов, а по квотированному — 24747. Пять «пустых»
    документов у Уралхиммаша были ровно этим.
    """
    try:
...
def провайдер(промпт, максимум=900):
...
def текст_дока(u):
    try:
        if re.search(r'\.pdf(?:\?|$)|\.docx?(?:\?|$)|\.rtf(?:\?|$)', u, re.I):
            return u, EC.doc_text(квотить(u)), 'doc_text'
        h, m, _meta = EC._fetch_site(квотить(u))   # .uif — HTML в cp1251
...
def компания(inn, имя, дом):
    ...
    if not текст.strip():
        ...
        return out
    части = [текст[i:i + 2800] for i in range(0, len(текст), 2800)][:5]
    out['частей_модели'] = len(части)
    беды = []

    def спросить(кусок):
        промпт = (
            f'Ниже кусок документов обязательного раскрытия компании {имя} '
            f'(ИНН {inn}): годовой отчёт, отчёт об итогах голосования, список '
            'аффилированных лиц. Выпиши названных ЛЮДЕЙ ЭТОЙ компании с '
            'должностью: члены совета директоров и правления, генеральный '
            'директор, главные специалисты. НЕ включай людей других '
            'организаций, аудиторов, регистраторов, нотариусов. ФИО в '
            'именительном падеже, должность — как в тексте. Верни СТРОГО JSON: '
            '{"people":[{"person":"Фамилия Имя Отчество","post":"должность"}]}'
            '\n\n' + кусок)
        for попытка in range(2):
            ответ, беда = провайдер(промпт, 900)
            if ответ:
                return ответ
            беды.append(беда)
            time.sleep(2)
        return ''
    ответы = []
    with ThreadPoolExecutor(max_workers=4) as п:
        ответы = list(п.map(спросить, части))
    if беды:
        out['сбои_провайдера'] = беды[:4]
    # Шлюз рубит поток примерно на 35-й секунде, поэтому JSON часто приходит
    # ОБОРВАННЫМ. Тянем пары person/post регуляркой: она берёт всё, что
    # успело прийти, а json.loads на обрезанном ответе не взял бы ничего.
    пар = re.compile(r'"person"\s*:\s*"([^"]{5,80})"\s*,\s*"post"\s*:\s*"([^"]{2,140})"')
    люди, видели = [], set()
    for ответ in ответы:
        for фио, пост in пар.findall(ответ or ''):
            к = фио.strip().lower()
            if к and к not in видели:
                видели.add(к)
                люди.append({'person': фио.strip(), 'post': пост.strip()})
    if not люди and not any(ответы):
        out['ошибка_провайдера'] = '; '.join(беды[:2]) or 'модель молчит'
        return out
    for ч in люди:
        ...
        # ссылка — тот документ, где фамилия ДЕЙСТВИТЕЛЬНО встречается:
        # модель адрес не возвращает, а приписывать наугад нельзя
        фам = фио.split()[0]
        ссылка = ''
        for u, t, _как in тексты:
            if t and фам in t:
                ссылка = u
                break
        out['люди'].append({'фио': фио, 'должность': пост,
                            ...
                            'тех': тех, 'адм': адм,
                            'подтверждено_в_документе': bool(ссылка),
                            'ссылка': ссылка[:110]})
