...
def проход(пауза, на_домен, лимит):
    ...
    # ОТБОР ИДЁТ ПО ВСЕЙ ОЧЕРЕДИ, А НЕ ПО ЕЁ НАЧАЛУ. Раньше сначала брался
    # срез очередь[:лимит], и только потом работала отбраковка по домену.
    # Очередь у нас на две трети mail.ru, поэтому из ста двадцати адресов
    # среза проходили ШЕСТЬ: три mail.ru и по мелочи. Проход впустую жёг
    # пять минут, а очередь не двигалась.
    #
    # Теперь набираем ровно так же вежливо - не больше `на_домен` адресов
    # одного домена за проход, - но идём по всей очереди, пока не наберём
    # лимит. Нагрузка на чужой сервер прежняя, польза от прохода в разы выше.
    счёт_домена = {}
    отобрано = []
    for адрес in очередь:
        домен = адрес.rsplit("@", 1)[-1]
        if счёт_домена.get(домен, 0) >= на_домен:
            continue                          # бережём и чужой сервер, и себя
        счёт_домена[домен] = счёт_домена.get(домен, 0) + 1
        отобрано.append(адрес)
        if len(отобрано) >= лимит:
            break
    print(f"   отобрано {len(отобрано)} адресов с {len(счёт_домена)} доменов "
          f"(не больше {на_домен} с домена)")

    сделано_сейчас = 0
    with open(МЕСТНЫЙ, "a", encoding="utf-8") as поток:
        for адрес in отобрано:
