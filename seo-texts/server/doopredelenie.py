...
Шесть путей (порядок = от дешёвых к дорогим, ранний выход на первом принятом):
  ...
  ПУТЬ 2            ЕГРЗ по объекту (заказчик/застройщик)            1 запрос OData, 0 ₽
  ПУТЬ 4            Федресурс: крупная сделка привязана к ИНН        сегодня недоступен
  ПУТЬ 5-SERP       более поздняя новость про тот же объект          xmlriver, ПЛАТНО
  ПУТЬ 6            «объект + район + отрасль» в SERP                xmlriver, ПЛАТНО

Пути 2 и 4 своей ходьбы в источник НЕ содержат: коллекторы ЕГРЗ и Федресурса
пишет другой агент, и реализацию мы не дублируем. Общая точка — функции
`poisk_v_egrz()` и `poisk_v_fedresurse()`: сначала ищут у соседского модуля
точечный `poisk`/`search`, иначе используют его же коллектор-ленту
(`collector_egrz.col_egrz` с серверными фильтрами по слову и региону). Нет модуля
— честно возвращают пусто с уликой. Федресурс сегодня для этой задачи не
применим: его коллектор ходит ОТ ИНН, а поиска от объекта у источника нет
(подробности — в docstring `poisk_v_fedresurse`).
...
на 1 107 боевых записях с ПРЯЧЕННЫМ именем), а имя находится у 3,1% безымянных
(23 из 735) бесплатным путём 5 и ещё около 1% — путём 2 через ЕГРЗ.
...
class Korpus(object):
    ...
    внутри корпуса: на боевых данных 16.09 (корпус 8 151 запись с именем) имя
    нашлось у 3,1% безымянных событий.
    ...
    выборке (1 106 записей с ПРЯЧЕННЫМ именем, корпус 8 146):
        ...
        порог df   охват   точность   спасено безымянных
          3         1,8%     75%          1,4%
          5         4,8%     83%          1,9%
          8         9,9%     88%          3,1%     <- сюда попадает правило N/1000
         12        14,3%     82%          4,1%
...
def strategiya_nash_korpus(sob, ctx):
    ...
    (8 151 запись с именем): имя нашлось у 3,1% безымянных событий (23 из 735),
    точность матчера 88% (проверка с прятанием имени на 1 107 записях; ещё 3% —
...
def strategiya_egrz(sob, ctx):
    ...
    Сколько даёт (живой прогон 16.09 по 60 безымянным событиям про стройку):
    имя нашлось у 3%. Причина низкой доли — не код, а разные языки: в реестре
    объект назван канцелярски («Строительство СИКН № 516 ЦПС Бахиловского
    месторождения»), в новости — житейски («завод картонной упаковки»), и общих
    предметных слов почти не бывает. ЕГРЗ силён как ИСТОЧНИК ранних событий
    (задача 1 ТЗ), а не как определитель имени по новости.
    """
