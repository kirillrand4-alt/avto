...
def занять_замок():
    """Захватить порт-замок.

    Возвращает сокет (замок наш), либо строку-причину, почему замка нет.
    Причина «занят» означает «раннер уже работает» и повод выйти; любая другая
    означает «порт непригоден», и выходить из-за неё НЕЛЬЗЯ.

    Нужен сторожу. Задача VpsRunner стояла на одном триггере — старт системы, —
    и умерший процесс поднять было нечем. Сторож раз в пять минут пробует
    запустить раннер; если тот жив, второй экземпляр упирается в занятый порт и
    тихо выходит. Порт, а не файл: замок снимается сам при любой смерти
    процесса, включая убийство без шанса прибрать за собой.

    Почему проверяем ЖИВОСТЬ, а не сам факт ошибки bind. 11.08 раннер лежал два
    с половиной часа при исправном стороже: тот запускался каждые пять минут и
    честно завершался с кодом 0, а процесса не оставалось. Причина — здесь:
    первая редакция считала ЛЮБУЮ ошибку bind доказательством живого соседа и
    выходила с нулём. Достаточно порту оказаться в зарезервированном Windows
    диапазоне (WSAEACCES) — и раннер запретил бы себе запускаться навсегда, а
    сторож рапортовал бы об успехе. Поэтому: не удалось занять — стучимся в
    порт. Ответили — сосед жив, выходим. Не ответили — порт непригоден,
    работаем без замка и говорим об этом в лог.
    """
    s = socket.socket()
    try:
    ...
    проба.settimeout(2)
    try:
        проба.connect(("127.0.0.1", ЗАМОК_ПОРТ))
        return "занят"
    except OSError:
        return f"порт непригоден ({причина})"
    finally:
        проба.close()
...
ЛОГ_ФАЙЛ = os.path.join(КОР, "runner.log")


def лог(м):
    """В консоль И в файл рядом со скриптом.

    Файл важнее консоли: раннер запускает планировщик, консоли у него нет, и
    11.08 он трижды умирал молча — причину приходилось угадывать по косвенным
    признакам, теряя по часу на догадку. Пишем сами, не полагаясь на обёртку:
    обёртку можно переписать и забыть про перенаправление.
    """
    строка = f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] {м}"
    try:
        print(строка, flush=True)
    except Exception:  # noqa: BLE001 - консоли может не быть вовсе
        pass
    try:
        with open(ЛОГ_ФАЙЛ, "a", encoding="utf-8") as f:
            f.write(строка + "\n")
            f.flush()
            os.fsync(f.fileno())
    except Exception:  # noqa: BLE001 - лог не должен ронять раннер
        pass
...
def разобрать_задания(видели):
    ...
    for имя in список():
        ...
        if not подпись_верна(задание):
            ...
            видели.add(имя)
            continue
        # Помечаем задание виденным И СОХРАНЯЕМ ОТМЕТКУ ДО выполнения.
        # 11.08 раннер полчаса подряд убивал сам себя: на дропе висело моё
        # старое задание «Stop-Process -Name python -Force», раннер поднимался,
        # брал его, исполнял — и умирал, не дойдя до записи «видел». Задача
        # планировщика через пять минут поднимала его снова, он снова брал то
        # же задание. Со стороны это выглядело как «раннер умирает сам».
        # Правило простое: задание, которое мы НАЧАЛИ, повторять нельзя — даже
        # если оно нас убило. Особенно если оно нас убило.
        видели.add(имя)
...
def main():
    ...
    if not a.no_lock:
        ...
        if замок == "занят":
            ...
            return 0
        if isinstance(замок, str):
            # Порт непригоден. Работаем БЕЗ замка: выйти здесь означало бы
            # запретить себе запускаться навсегда, а сторож рапортовал бы
            # об успехе — ровно это и случилось 11.08.
            лог(f"замок не взят: {замок} — работаю без него")
            замок = None
    ...
    while True:
      # Весь круг под защитой, а не только разбор заданий. 11.08 раннер умирал
      # молча через секунды после старта: любая непойманная мелочь в цикле
      # убивала процесс, а планировщик показывал «Готово» — неотличимо от
      # штатного завершения. Круг может упасть; раннер падать не должен.
      try:
        try:
        ...
        лог("круг упал целиком:\n" + traceback.format_exc()[-1200:])
      time.sleep(max(5, a.poll))


if __name__ == "__main__":
    # Ни один выход не должен быть молчаливым: причина всегда попадает в лог,
    # иначе снаружи «умер» и «отработал» выглядят одинаково.
    try:
        _код = main() or 0
        лог(f"раннер завершился штатно, код {_код}")
    except BaseException:  # noqa: BLE001 - в лог обязан попасть и KeyboardInterrupt
