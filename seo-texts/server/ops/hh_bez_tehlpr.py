...
# ДОЛЖНОСТИ. Первый полный прогон дал 99 предприятий, но внутри оказались
# «Судовой механик», «Механик транспортного цеха», «Автомеханик» и десятки
# слесарей: шаблон ловил голое слово «механик». Это ровно та ошибка, которую
# я утром чинил в каноне ролей (автомеханик и механик гаража уезжали в
# технические), и повторил её здесь в новом месте.
#
# Поэтому три уровня, а не один. Смешивать их нельзя: продавцу нужен тот, кто
# РЕШАЕТ по компрессорам, а не всякий, у кого в названии есть «механик».
_ТЕХ_ЛПР = re.compile(
    r'(?:главн\w*\s+(?:энергетик|механик|инженер|технолог|конструктор)|'
    r'зам\w*\s+главного\s+инженера|'
    r'начальн\w*\s+(?:цеха|участка|производств\w*|отдела|'
    r'энерго\w*|компрессорн\w*)|'
    r'ведущ\w*\s+инженер|инженер[\s-]*энергетик|энергетик\b|'
    r'инженер\s+КИПиА|начальник\s+службы)', re.I)
# наш человек по машине, но НЕ решает: машинист, слесарь, приборист
_ТЕХ_ИСПОЛНИТЕЛЬ = re.compile(
    r'(?:машинист\s+компрессорн\w*|слесар\w*|приборист|'
    r'электромеханик|техник\b)', re.I)
# чужие совсем: транспорт, суда, стройка
_ЧУЖИЕ = re.compile(
    r'(?:судов\w*|транспортн\w*\s+цех|автомехан|гараж\w*|\bАТЦ\b|'
    r'по\s+ремонту\s+(?:транспорт|автомобил)|водител|монтажник|'
    r'машинист\s+(?:крана|экскаватор|бульдозер)|токар|сварщик)', re.I)


def уровень(название):
    """'ЛПР' | 'исполнитель' | 'чужой' | '' — и пусть будет видно в файле."""
    н = название or ''
    if _ЧУЖИЕ.search(н):
        return 'чужой'
    if _ТЕХ_ЛПР.search(н):
        return 'ЛПР'
    if _ТЕХ_ИСПОЛНИТЕЛЬ.search(н):
        return 'исполнитель'
    return ''
...
def main():
    ...
    for i, ц in enumerate(цели, 1):
        ...
        техн = [(v, у) for v, у in все_техн if у in ('ЛПР', 'исполнитель')]
        итог['вакансий отброшено как чужие'] += sum(
    ...
    поля = ['uroven', 'inn', 'predpriyatie', 'rabotodatel_na_hh', 'vakansiya',
    ...
    print(f'  из них с наймом ТЕХНИЧЕСКОГО ЛПР: {len(предпр_лпр)} '
          f'— вот честное число способа')
    for у, n in с_ур.most_common():
        print(f'    {n:>5}  строк уровня «{у or "не определён"}»')
