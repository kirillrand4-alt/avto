...
if os.path.exists(ЖУРНАЛ):
    for s in io.open(ЖУРНАЛ, encoding="utf-8"):
        ...
        if z.get("ок") and z.get("review_id"):
            если_ок.add(inn)
        # ЧЕРНОВИК БРАКА — ТОЖЕ ТЕКСТ. partiya_gen кладёт удачное письмо в
        # «тело», а забракованное в «тело_брака» (ветка elif черновик), и этот
        # разбор смотрел только на первый ключ. Из-за этого мимо починки шли
        # 534 черновика, забракованных по заходу: механика у них исправна,
        # претензия только к первой фразе. Хуже того, «тело_брака» не считается
        # отработанным и в резюме самой генерации — компания возвращалась в пул
        # и переписывалась опусом заново, при готовом черновике на диске.
        if z.get("этап") == "сгенерировано":
            if z.get("тело"):
                последний[inn] = z
            elif z.get("тело_брака"):
                z = dict(z, тема=z.get("тема_брака"), тело=z.get("тело_брака"),
                         _из_черновика=True)
                последний[inn] = z
...
def назвать_предприятие(тема, тело, имя):
    ...
    if not кратко:
        return тема, тело
    # ИМЯ В ТЕКСТЕ СТОИТ В ПАДЕЖЕ. Сверка шла точной строкой, и «Гера» не
    # узнавалась в «в «Гере»» — починка приписывала имя второй раз, выдавая
    # «Вопрос по генерации азота в «Гере» в «Гера»». Сверяем по основе: у
    # русского склонения меняется хвост, а не начало.
    текст = f"{тема}\n{тело}".lower()
    основа = кратко.lower()
    if len(основа) >= 4:
        основа = основа[:-1]
