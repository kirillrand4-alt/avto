# -*- coding: utf-8 -*-
"""Сколько писем можем сгенерировать прямо сейчас — с учётом перегенерации.

Владелец 25.08 подгрузил 1665 компаний и спросил: «на сколько можем
сгенерировать, включая скипы — где причина не то что мы уже писали, а то
что письмо тогда не нравилось».

Поэтому к обычным кандидатам добавлены фирмы, чьи письма сняли ПО КАЧЕСТВУ
ТЕКСТА: линза, правила стиля, механическая сборка, старьё до 10.08,
административные снятия под партию. Такой фирме перегенерация помогает —
письмо будет другим. А снятия «компания не наша» (профиль, ОКВЭД,
направление) остаются снятыми: другим текстом их не исправить.
...
with store._lock:
    for (_инн,) in store._conn.execute(
            ...
            "WHERE c.status='skipped' AND ("
            # компания не наша — прежний набор, тут перегенерация ни при чём,
            # но фирма обязана вернуться в пул: направление пересчитается
            "      c.reason LIKE '%не наш%' "
            "   OR c.reason LIKE '%вне профиля%' "
            "   OR c.reason LIKE '%не покупатель%' "
            # письмо было плохим — вот это и просил владелец
            "   OR c.reason LIKE '%линза%' "
            "   OR c.reason LIKE '%правило %' "
...
print("приговоров «мёртв»: %d | компаний с готовым письмом: %d | "
      "возвращено в пул (профиль + плохое письмо): %d"
      % (len(_мёртвые), len(сделано_инн), len(_снятые)))

# Кто из кандидатов новый (подгружен сегодня), а кто вернулся на
# перегенерацию — владельцу важно различать: у новых письма ещё не было.
_сегодня = set()
with store._lock:
    for (_rid,) in store._conn.execute(
            "SELECT id FROM recipients "
            " WHERE substr(created_at,1,10)=date('now')").fetchall():
        _сегодня.add(int(_rid))
...
for rid in в_группе:
    ...
    годные.append((_d, свой, полей, роль,
                   "новый" if rid in _сегодня else
                   ("перегенерация" if inn in _снятые else "старый пул")))
    счёт["ГОДЕН"] += 1
...
_откуда = _C(г[4] for г in годные)
for к, н in _откуда.most_common():
    print("  %-22s %6d" % (к, н))
print("\n=== КАНДИДАТЫ ПО НАПРАВЛЕНИЯМ И ПРОИСХОЖДЕНИЮ ===")
_пара = _C((г[0], г[4]) for г in годные)
for (нап, отк), н in sorted(_пара.items()):
    print("  %-8s %-22s %6d" % (нап, отк, н))
