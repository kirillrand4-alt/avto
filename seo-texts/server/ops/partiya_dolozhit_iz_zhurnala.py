...
На письмо в журнале ДВЕ строки: «сгенерировано» (там тема и тело) и «итог»
(там review_id и статус очереди). Итоговой строки может не быть вовсе - это
и означает, что письмо до панели не доехало.

Имя первого этапа в журнале встречается в двух видах - «текст» и
«сгенерировано» (две сессии писали правку одновременно, 17.08). Этому
скрипту всё равно: он ищет письмо по наличию «тела» и по review_id, а на
поле «этап» не смотрит вовсе. Проверено на журнале со смешанными именами.
