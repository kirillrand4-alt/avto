...
п()
# ЛОВИМ ЛЮБУЮ ЦИФРУ, А НЕ ТОЛЬКО ЧИСЛО С ЕДИНИЦЕЙ. Прежний шаблон требовал
# после числа единицу измерения и пропустил «поле 2х3 метра» в письме #2130 -
# ровно тот вид утверждения, который и надо сверять с сайтом (размер,
# габарит, мощность). Печатаем с двумя словами контекста, иначе список цифр
# бесполезен: непонятно, к чему они.
числа = []
for м in re.finditer(r'\d[\d.,]*(?:\s?[хx]\s?\d[\d.,]*)?', f"{subj}\n{body}"):
    т = f"{subj}\n{body}"
    лево = т[max(0, м.start() - 26):м.start()].split()[-2:]
    право = т[м.end():м.end() + 22].split()[:2]
    числа.append(f"{' '.join(лево)} [{м.group(0)}] {' '.join(право)}")
