    ...
    for з in к_записи:
        ...
        if з['phone']:
            вставить('contact', поля_c, {
                ...
                # ДОБАВОЧНЫЙ СОХРАНЯЕМ. Он отрезался при разборе номера и молча
                # пропадал: в таблице не было колонки. А это, по замеру 3-й
                # сессии, единственный практический путь к технарю — городской
                # с добавочным есть прямой стол человека, а не коммутатор.
                # В файлах соседок таких номеров 94, у меня их было 0.
                'dobavochnyy': з['dob'],
                'source': источник, 'source_url': з['url'],
                'data_podtverzhdeniya': з['date'],
                'chem_podtverzhdena': з['quote'][:200],
