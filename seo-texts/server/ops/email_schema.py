...
_ВАРИАНТЫ = {'kh': ('h',), 'ts': ('c',), 'y': ('i',), 'ya': ('ia',),
             'yu': ('iu', 'u'), 'zh': ('j',), 'sch': ('shch', 'sh'),
             'ks': ('x',)}
# «ь» перед гласной пишут как «y»: Арсентьева -> arsentyeva, а не arsenteva.
# Замер на живой базе поймал это сразу на двух записях (chuva.ru), причём обе
# выглядели как «правило не распознано», хотя правило самое обычное.
_МЯГКИЙ = [(re.compile(r'ь([еёиюя])'), r'y\1')]


def лат(с, мягкий_y=False):
    с = (с or '').lower()
    if мягкий_y:
        for rx, зам in _МЯГКИЙ:
            с = rx.sub(зам, с)
    out = []
    for ч in с:
    ...
    for ф in варианты(p['ф']):
        ...
        for и in и_в:
            for о in о_в:
                ...
                if о1:
                    # фамилия_ио и фамилия.ио — самая частая непокрытая форма
                    # в замере: kurbatova_ma@, dikhtyar_oa@, gabyshev_il@
                    правила[f'{ф}_{и1}{о1}'] = 'фамилия_ио'
                    правила[f'{ф}.{и1}{о1}'] = 'фамилия.ио'
...
def свой_домен(inn):
    # site_domain, а не _domain: в companies.site схемы нет, и _domain на
    # голом «zavod.ru» отдаёт пустоту. Первый прогон именно из-за этого
    # отчитался «схема не выведена ни у одной компании» — доменов не было
    # вообще ни у одной из сорока.
    r = e.execute('SELECT site FROM companies WHERE inn=?', (inn,)).fetchone()
    д = (EC.site_domain((r or [''])[0] or '') or '').lower()
    if д:
        return д
    # Сайт известен не всем, но корпоративный домен часто виден в ИХ ЖЕ
    # адресах. Проверяющий агент показал: из шести компаний без сайта у пяти
    # домен так и находится — то есть 15% пула выбрасывались зря. Берём самый
    # частый непубличный домен среди адресов компании.
    счёт = {}
    for (а,) in e.execute('SELECT email FROM emails WHERE inn=?', (inn,)):
        d = (а or '').lower().split('@')[-1].strip()
        if d and '.' in d and d not in _ПУБЛИЧНЫЕ:
            счёт[d] = счёт.get(d, 0) + 1
    return max(счёт, key=счёт.get) if счёт else ''


def вывести_схему(inn, дом):
    """Правило почты компании: подтверждается минимум ДВУМЯ адресами с ФИО."""
    голоса = {}
    # ДЕДУПЛИКАЦИЯ ОБЯЗАТЕЛЬНА. Один и тот же адрес лежит и в emails, и в
    # people (по базе таких пар 88), и без схлопывания он давал ДВА голоса —
    # то есть в одиночку удовлетворял правилу «схема подтверждается минимум
    # двумя адресами». Поймано проверяющим агентом: на пяти фактических
    # правилах не выстрелило, но дыра в самой честности рубежа была настоящая.
    сырые = list(e.execute(
        'SELECT email, person FROM emails WHERE inn=? AND person<>""', (inn,)))
    сырые += [(r[0], r[1]) for r in e.execute(
        'SELECT email, person FROM people WHERE inn=? AND email<>"" '
        'AND person<>""', (inn,))]
    видели, строки = set(), []
    for адрес, фио in сырые:
        к = (адрес or '').lower().strip()
        if not к or к in видели:
            continue
        видели.add(к)
        строки.append((адрес, фио))
    примеры = {}
