...
def лучшая_должность(к):
    ...
    if 'техническая' in к['роли']:
        ...
        # Метка площадки сама по себе — тоже свидетельство. Соседняя сессия
        # ставит `rol` провайдером по смыслу комментария, и она читает
        # вывески, которых нет в моих шаблонах: «по тех. вопросам», «к ТЗ
        # обращаться», «для получения дополнительной технической информации».
        # Таких людей 61, из них 57 с телефоном — и без этой ветки они лежат
        # с ПУСТОЙ ролью, то есть неотличимы от секретаря.
        # Ровно тот случай, где канон подстрок проигрывает модели: чинить его
        # дописыванием правил под каждую новую формулировку бессмысленно.
        # ВАЖНО: `add_person` канонизирует роль ИЗ ДОЛЖНОСТИ сам, вычисленную
        # здесь роль он не принимает. Поэтому нельзя класть в должность живую
        # цитату основания: длинный текст ловится каноном по слову «техни» и
        # даёт ЛОЖНОГО главного инженера — на первом же прогоне так появилось
        # двадцать штук. Кладём нормализованную формулировку, которую канон
        # разбирает однозначно; сама цитата видна по ссылке на карточку.
        return 'контактное лицо по техническим вопросам', 'техконтакт'
    return '', 'общий'
...
def main():
    ...
    if not СУХОЙ:
        ...
        # ИДЕМПОТЕНТНОСТЬ. Ключ конфликта в people — (inn, person, post), и
        # если при повторном прогоне у человека появилась должность там, где
        # раньше была пустая строка, вставится ВТОРАЯ строка вместо правки
        # первой. Поэтому свои прежние записи без должности сносим: источник
        # tender.pro пишет только этот оп, чужого тут нет.
        # Сносим ВСЕ свои прежние записи, а не только без должности: при
        # смене правил у человека меняется и должность, и роль, а ключ
        # конфликта (inn, person, post) этого не переживает — остаются старые
        # строки с прежней ролью. Источник пишет только этот оп, чужого нет.
        снесено = db.cx.execute(
            'DELETE FROM people WHERE source=?', (ИСТОЧНИК,)).rowcount
        db.cx.commit()
        print(f'снято своих прежних записей без должности: {снесено}')
        sys.stdout.flush()
