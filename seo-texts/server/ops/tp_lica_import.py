...
def на_дроп(путь, имя):
    """Выложить файл на дроп. Имя ЯВНОЕ и уникальное — параллельные записи под
    одним именем тихо затирают друг друга (тот же класс, что совпадавшие id
    заданий; второй случай поймала соседняя сессия на fetch_url без name)."""
...
def _дата(т):
    """«13.07.2026» → сортируемое «2026-07-13». Пустое — в самый низ."""
    м = re.match(r'(\d{2})\.(\d{2})\.(\d{4})', (т or '').strip())
    return f'{м.group(3)}-{м.group(2)}-{м.group(1)}' if м else ''
...
def закупки():
    """{tender_id: (ключ, предмет, создан, до)} из tp-spisok.csv.

    Это ответ на вопрос владельца «кто может стать клиентом» — документом, а
    не догадкой по ОКВЭД. Соседняя сессия искала закупки по восьми ключам:
    компрессор, компрессорная, центробежный, турбокомпрессор, воздуходувка,
    турбоагрегат, нагнетатель, воздуходувный. Значит КАЖДОЕ предприятие в этой
    выгрузке уже публиковало закупку компрессорного — потребность датирована и
    проверяема, и это же готовый повод для звонка.
    """
    из = {}
    for r in читать('tp-spisok.csv'):
        тид = (r.get('tender_id') or '').strip()
        if тид:
            из[тид] = ((r.get('klyuch') or '').strip(),
                       (r.get('predmet') or '').strip(),
                       (r.get('sozdan') or '').strip(),
                       (r.get('do') or '').strip())
    return из
...
def main():
    ...
    if not СУХОЙ:
        ...
        # Повод для звонка: закупка компрессорного с датой. Пишем по одной
        # самой свежей закупке на компанию — не заваливаем сигналами.
        зак = закупки()
        по_инн = {}
        for к in люди.values():
            for тид in к['тендеры']:
                з = зак.get(тид)
                if not з:
                    continue
                ключ, предмет, создан, до = з
                прежн = по_инн.get(к['инн'])
                if not прежн or _дата(создан) > _дата(прежн[2]):
                    по_инн[к['инн']] = (тид, ключ, создан, предмет, до)
        for инн, (тид, ключ, создан, предмет, до) in по_инн.items():
            db.add_signal(инн, ИСТОЧНИК, event_type=f'закупка: {ключ}',
                          what=предмет, source_url=СТР.format(тид), ts=создан,
                          hotness=2)
        print(f'сигналов «закупка компрессорного»: {len(по_инн)}')
        sys.stdout.flush()
    ...
    # Выгрузка продажникам: человек, роль, телефон, повод и кликабельный
    # источник в одной строке. Кладём рядом с базой и на дроп — файлы сессии
    # слетают, дроп живёт на сервере.
    if not СУХОЙ:
        зак = закупки()
        путь = r'C:\sender\server\tp-lyudi-dlya-prodazhnikov.csv'
        порядок = {р: i for i, р in enumerate(EDB.EnrichDB.TECH_ROLES)}
        строки = []
        for к in люди.values():
            должность, роль = лучшая_должность(к)
            тид = к['тендеры'][0] if к['тендеры'] else ''
            ключ, предмет, создан, _до = зак.get(тид, ('', '', '', ''))
            б = из_базы.get(к['инн']) or {}
            строки.append([
                к['инн'], б.get('name') or к['компания'], б.get('city', ''),
                к['фио'], должность, роль,
                '; '.join(к['телефоны']), '; '.join(к['почты']),
                ключ, создан, предмет[:180], СТР.format(тид) if тид else '',
                'да' if к['инн'] in из_базы else 'нет'])
        строки.sort(key=lambda с: (порядок.get(с[5], 90 if с[5] == 'техконтакт' else 99),
                                   с[1]))
        with open(путь, 'w', encoding='utf-8-sig', newline='') as ф:
            в = csv.writer(ф, delimiter=';')
            в.writerow(['ИНН', 'компания', 'город', 'ФИО', 'должность', 'роль',
                        'телефоны', 'почты', 'ключ закупки', 'дата закупки',
                        'предмет закупки', 'источник', 'есть в базе обзвона'])
            в.writerows(строки)
            ф.flush()
            os.fsync(ф.fileno())
        print(f'выгрузка: {путь} ({len(строки)} строк)')
        try:
            на_дроп(путь, 'tp-lyudi-dlya-prodazhnikov.csv')
            print('выложено на дроп: tp-lyudi-dlya-prodazhnikov.csv')
        except Exception as e:  # noqa: BLE001
            print(f'на дроп не легло ({e}) — файл остался на сервере')
