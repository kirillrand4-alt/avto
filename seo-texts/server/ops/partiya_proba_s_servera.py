...
try:
    ...
    # ПУТЬ БАЗЫ ОБЯЗАТЕЛЕН ВТОРЫМ АРГУМЕНТОМ. Без него найти() смотрит только
    # в настройку service.enrich_db, а её нет - вернулся None, и 827 вердиктов
    # молча ушли в никуда («обновлено: 0»). Со вторым аргументом функция
    # находит enrich.db рядом с базой панели.
    n = _в_обогащение(_найти(cfg, БАЗА), для_обогащения)
    print("в enrich.db:", n)
except Exception as ex:                                        # noqa: BLE001
