...
def main():
    ...
    было = сделанные()
    # ТОЛЬКО ЮРЛИЦА (01.09). У индивидуального предпринимателя номер —
    # ОГРНИП в ПЯТНАДЦАТЬ цифр, а не ОГРН в тринадцать, и страницы
    # checko.ru/company/<номер> для него не существует: приходит 404. В
    # requisites такие строки лежат вперемешку и идут ПЕРВЫМИ, поэтому
    # прогон упирался в них, тратил по шесть попыток на каждую и до
    # свежего сбора не доходил вовсе — счётчик показывал «код 404: 11» при
    # нуле удачных.
    все_цели = [(и, о) for и, о in ряды if ВСЕ or str(и) not in было]
    юрлица = [(и, о) for и, о in все_цели
              if len(''.join(c for c in str(о or '') if c.isdigit())) == 13]
    пропущено_ип = len(все_цели) - len(юрлица)
    цели = юрлица[:ЛИМ]
    if пропущено_ип:
        print(f'пропущено ИП (ОГРНИП, не 13 цифр): {пропущено_ип}')
    ...
    def взять(url, i):
        ...
        for попытка in range(6):
            with _замок:
                ...
                # ВСЕ ОСТЫВАЮТ — ЖДЁМ БЛИЖАЙШЕГО, а не бьём в закрытые.
                # Мобильных выходов два-три, и без этого поток жёг попытки
                # по прокси, которые заведомо ответят 429.
                if not годные:
                    ближайший = min(_остыв.values()) if _остыв else 0
                    пауза = max(1.0, min(30.0, ближайший - time.time()))
                else:
                    пауза = 0
            if пауза:
                time.sleep(пауза)
                with _замок:
                    годные = [p for p in PX
                              if _остыв.get(p, 0) < time.time()]
            with _замок:
            ...
            if r.status_code == 429:
                with _замок:
                    _стат['429'] += 1
                # НЕ ЖДАТЬ ВНУТРИ ЗАПРОСА. Здесь стояло ожидание смены
                # адреса с пределом в 480 секунд — и это была ошибка: цикл
                # делает до шести попыток, то есть одна компания, поймавшая
                # 429, могла ждать до сорока восьми минут. Две компании не
                # прошли за семь минут ровно поэтому. Прокси уходит в
                # короткое остывание, а поток немедленно берёт другой
                # выход; у мобильного адрес всё равно сменится сам, и
                # остывание лишь не даёт долбиться в закрытую дверь.
                with _замок:
                    _остыв[px] = time.time() + (60 if МОБИЛЬНЫЕ else 90)
                time.sleep(0.5)
                continue
