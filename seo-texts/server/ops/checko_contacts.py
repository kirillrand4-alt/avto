...
def работа(t):
    ...
    with замок:
        ...
        # САЙТ ИЗ БЛОКА «Веб-сайт». Два урока, оба куплены за прогон:
        # 1) нельзя брать первую ссылку страницы — первой стоит `yastatic.net`,
        #    и 186 карточек уехали со статикой Яндекса вместо сайта завода;
        # 2) нельзя брать первое совпадение маркера — английское `website`
        #    встречается в мета-теге шапки (`<meta name="csrf-token">`), окно
        #    уезжает в заголовок, ссылок там нет, и сайт теряется. Ровно тот же
        #    урок уже записан выше про обрезку подвала: маркер ищем не первый,
        #    а тот, за которым есть содержимое.
        # Поэтому: ищем РУССКИЙ ярлык, перебираем ВСЕ его вхождения и берём то,
        # в окне которого нашлась годная ссылка.
        сайт = ''
        for мс in re.finditer(r'Веб-сайт|Сайт(?=[<:\s])', основа):
            окно = основа[мс.start():мс.start() + 700]
