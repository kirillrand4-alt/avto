# -*- coding: utf-8 -*-
"""Разобрать ОЧЕРЕДЬ ПОДТВЕРЖДЕНИЯ целиком: годное и косметику — в
автоотправку, критичное — снять на перегенерацию.
...
СЛЕД = r"C:\sender\_ops\perekinuto-ocheredi.jsonl"

вердикт = {}
for ф in (r"C:\sender\_ops\sud-ocheredi.jsonl",
          r"C:\sender\_ops\sud-vtoryh.jsonl",
          r"C:\sender\_ops\sud-vtoryh-2.jsonl"):
    try:
...
строки = c.execute(
    ...
    "   AND COALESCE(kind,'outbound') <> 'reply' ORDER BY id").fetchall()
c.close()
...
for r in строки:
    ...
    if в == "не отправлять":
        итог["судья: не отправлять — оставляем на снятие"] += 1
        continue
    if в == "не судили":
        итог["судья ещё не смотрел — не трогаем"] += 1
        continue
    try:
