    ...
    with open(путь, encoding='utf-8-sig', errors='replace', newline='') as ф:
        for r in csv.DictReader(ф, delimiter=';'):
            ...
            if ц and not номер_живой(ц):
                ...
                ц = ''
            # ЧИТАЕМ ПОМЕТКУ ИСТОЧНИКА, А НЕ ТОЛЬКО ЗНАЧЕНИЕ. Вторая сессия
            # нашла у себя 32 номера, которых в исходной карточке нет вовсе —
            # их выдумала модель при разборе. Её заслон такие номера ПОМЕЧАЛ
            # колонкой `telefon_est_v_tekste`, но писатель в панель пометку не
            # читал, и пять выдумок доехали до продавца личными мобильными
            # техников.
            #
            # У меня проверено: из 3 081 строки 5 номеров неподтверждённые, и
            # ни один в панель не попал — повезло. Полагаться на везение
            # нельзя: правило «пометка есть, но её никто не читает» стоит
            # ровно столько же, сколько отсутствие пометки.
            if ц and (r.get('telefon_est_v_tekste') or '').strip() not in ('1',
                                                                          'да'):
                отсев['номер не подтверждён текстом карточки'] += 1
                ц = ''
                тел = ''
