...
def main():
    ...
    # Судим ТОЛЬКО видимых: скрытые уже убраны по другим основаниям, и платить
    # за приговор по ним значит жечь деньги владельца впустую.
    очередь = []
    for инн, (имя, рег, окв) in компании.items():
        if инн in скрыты or инн in осуждены:
            continue
        ф = факты.get(инн, [])
