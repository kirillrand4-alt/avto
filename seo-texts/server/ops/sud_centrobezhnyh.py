...
def _список_целей():
    """Судить ИМЕННО этих, даже если они скрыты не судом.

    Обычный отбор пропускает скрытых кем угодно, кроме самого суда, и это
    правильно: платить за чужие ручные чистки незачем. Но список, присланный
    соседкой «к пересмотру», — прямая просьба вынести приговор, а не случайная
    выборка. Такие ИНН берём поимённо и фильтр скрытия для них не применяем.

    Файл берётся с дропа, колонка `inn`; годится и голый список в столбик.
    """
    имя = ''
    for i, а in enumerate(sys.argv):
        if а == '--spisok' and i + 1 < len(sys.argv):
            имя = sys.argv[i + 1]
    if not имя:
        return set()
    путь = os.path.join(r'C:\seostat\drop\drop-storage', имя)
    if not os.path.exists(путь):
        print('НЕТ ФАЙЛА СПИСКА:', путь)
        raise SystemExit(1)
    цели = set()
    for стр in io.open(путь, encoding='utf-8-sig', errors='replace'):
        for кус in re.split(r'[;,\t]', стр):
            ц = re.sub(r'\D', '', кус)
            if len(ц) in (10, 12):
                цели.add(ц)
                break
    print(f'список {имя}: ИНН к суду {len(цели)}')
    return цели


СПИСОК = _список_целей()
...
def main():
    ...
    for инн, (имя, рег, окв) in компании.items():
        if СПИСОК:
            # Прицельный прогон: судим только названных, и скрытие им не помеха.
            # А вот УЖЕ ОСУЖДЁННЫХ пропускаем и здесь: первый прицельный прогон
            # на шести карточках я не исключил, и вторым заходом заплатил за них
            # ещё раз. Список — это «кого судить», а не «сколько раз».
            if инн not in СПИСОК or инн in осуждены:
                continue
        else:
            if инн in осуждены:
                continue
