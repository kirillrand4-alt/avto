...
def карточка(tid):
    ...
    ядро = (c[:m.start()] if m else c)[:3000]
...
try:
    ...
    pass
for путь in (r'C:\sender\_ops\sales_base.json', r'C:\sender\server\sales_base.json'):
    try:
        b = json.load(open(путь, encoding='utf-8'))
    except Exception:  # noqa: BLE001
        continue
    for строки in b.values():
        for x in строки:
            i = str(x.get('inn') or '').strip()
            if i and i not in вид:
                вид.add(i)
