# -*- coding: utf-8 -*-
"""Что плавает в системном блоке письма КЦ: сверка префиксов по хешу.

То же, что chto_plavaet_v_prefikse.py, но по направлению КЦ — сегодняшние
партии были целиком КЦ (368 и 215 писем), и течь надо искать там.
...
# Берём трёх РАЗНЫХ получателей кц-шной группы.
...
for н, req in enumerate(взяли):
    статики.append(_разбор("письмо #%d" % (н + 1),
                           gen_prompt([req], факты, "kc", angle_base=н)))

print("\n=== ТЕ ЖЕ ПРОМПТЫ С ОДИНАКОВЫМ angle_base ===")
одинаковые = [gen_prompt([req], факты, "kc", angle_base=0) for req in взяли]
...
_разбор("судья (judge_prompt)", judge_prompt([(0, [проба, проба])], "kc"))
_разбор("верификатор (vf)", vf_prompt(
    [(0, проба["subject"], проба["body"])], "kc"))
...
print("\n=== ВЕРИФИКАТОР ПО РАЗНЫМ ПИСЬМАМ (в письме таких вызовов несколько) ===")
вф = []
for н, req in enumerate(взяли):
    имя = str(req.get("company_name") or req.get("имя") or "компания %d" % н)
    вф.append(_разбор("vf для %s" % имя[:18],
                      vf_prompt([(0, "Вопрос по компрессорам в «%s»" % имя[:20],
                                  "тело письма про %s" % имя[:20])], "kc")))
if len(вф) >= 2 and вф[0] and вф[1]:
    print("  статики vf %s" % ("СОВПАДАЮТ" if вф[0] == вф[1] else "РАСХОДЯТСЯ"))

print("\n=== ЧТО В СТАТИКЕ ЛЕЖИТ (первые и последние строки) ===")
if статики and статики[0]:
    строки = статики[0].split("\n")
    for с in строки[:6]:
        print("  | %s" % с[:120])
    print("  | … всего %d строк, %d знаков" % (len(строки), len(статики[0])))
    for с in строки[-8:]:
        print("  | %s" % с[:120])
