...
out = {'целей': len(todo), 'обработано': 0, 'ошибок': 0,
       ...
       'тел': 0, 'почт': 0, 'техЛПР': 0, 'людей': 0, 'примеры': []}
...
def записать(inn, тел, почта, фио, должность, источник, урл):
    ...
    # ЧЕЛОВЕКА пишем ДАЖЕ БЕЗ КОНТАКТОВ (вопрос владельца 29.07): «главный
    # инженер Иванов» без телефона — всё равно имя, которое спрашивают в
    # приёмной. Раньше такая запись падала в никуда: обе контактные таблицы
    # требуют либо номер, либо адрес.
    if (фио or '').strip():
        try:
            db.add_person(inn, фио, post=должность or '', phone=тел or '',
                          email=почта or '', source=источник, source_url=урл or '')
            out['людей'] = out.get('людей', 0) + 1
        except Exception:  # noqa: BLE001
            pass
...
def работа(t):
    ...
    try:
        ...
        if 'agg' in ИСТОЧНИКИ:
            # Тендерные агрегаторы. ЗАМЕР: 109 компаний -> НОЛЬ контактов, и
            # это честный ноль источника, а не сбой (разбор в комментарии к
            # enrich_contacts.find_tender_aggregator_contacts: страницы
            # качаются, ИНН на них есть, но контакты закрыты регистрацией).
            # Поэтому источник ВЫКЛЮЧЕН по умолчанию и включается только
            # явным --src agg — держим на случай платного доступа.
            for c in EC.find_tender_aggregator_contacts(inn, имя):
                with замок:
                    out['агрегаторов'] = out.get('агрегаторов', 0) + 1
                    записать(inn, c.get('phone'), c.get('email'),
                             c.get('person'), 'снабжение/закупки',
                             'тендер-агрегатор', c.get('url'))
                итог['агрегатор'] = итог.get('агрегатор', 0) + 1
