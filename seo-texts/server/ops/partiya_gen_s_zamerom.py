...
def _мой_raw(messages, model, max_tokens, thinking=True, effort=None):
    ...
    Режим рассуждения подменяем только для боевой модели письма: линзы на
    haiku идут как есть, иначе замер сравнивал бы разные вещи.
    """
    свой = str(model).startswith("claude-opus")
    if свой and РЕЖИМ != "kak_seychas":
        thinking = False
        effort = УСИЛИЕ
    промпт = ""
    try:
