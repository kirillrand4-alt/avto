...
def инн_на_странице(сайт, инн):
    """ИНН на главной ИЛИ на странице реквизитов — ВСЕ ПУТИ ПАРАЛЛЕЛЬНО.

    Последовательный обход девяти путей по 20 секунд давал до 180 секунд на
    одну компанию, и прогон падал по таймауту раньше, чем успевал что-то
    измерить. Пути независимы, значит их надо качать разом: стоимость
    становится равна САМОМУ МЕДЛЕННОМУ пути, а не их сумме. Указание
    владельца 30.07: «пути можешь оставить и 9, просто сделай больше
    параллельности».
    """
    ...
    def проба(путь):
        ...
        if re.search(r'(?<!\d)' + цифры + r'(?!\d)', re.sub(r'[\s\-]', '', html)):
            return путь or '/'
        return None
    with ThreadPoolExecutor(max_workers=len(_ПУТИ)) as пул:
        for где in пул.map(проба, _ПУТИ):
            if где:
                return True, где
    return False, ''
