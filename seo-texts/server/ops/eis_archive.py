...
def нормтел(s):
    """+7XXXXXXXXXX из строки телефона; пусто, если это не номер."""
    ц = re.sub(r'\D', '', s or '')
...
def схлопнуть(записи):
    ...
    по_тел = {}
    for i, з in enumerate(записи):
        t = нормтел(з.get('phone'))
        if not t or not t.startswith('+7'):
            continue
        j = по_тел.get(t)
        if j is None:
            по_тел[t] = i
...
def обработать(inn):
    ...
    # ЧТО ВЗЯЛ БЫ ПРЕЖНИЙ ПОДХОД: find_zakupki_contacts(max_cards=4) открывает
    # первые четыре позиции ленты, отсортированной по дате обновления.
    прежние = [u for u, _d, _t in (актуальные + архив)][:4]

    отсорт = sorted(пул.items(), key=lambda kv: kv[1][0], reverse=True)
    акт_список = [x for x in отсорт if x[1][2]][:КАРТ_АКТУАЛ]
    арх_список = [x for x in отсорт if not x[1][2]]
