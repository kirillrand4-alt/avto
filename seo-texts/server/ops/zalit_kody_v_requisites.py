...
# ОПТОВУЮ ТОРГОВЛЮ УБРАЛИ (владелец 01.09, после первого списка). Были
# названы ещё 46.21 «оптовая торговля зерном и кормами» (6276) и 46.38
# «оптовая торговля прочими пищевыми» (4680) — 10956 компаний. К моменту
# отмены не легло ни строки, откатывать было нечего.
КОДЫ = {
...
if ПРИМЕНИТЬ and новых:
    ...
    for н in range(0, len(новых), ПАЧКА):
        ...
        # ТЕРПЕНИЕ, А НЕ ВОСЕМЬ ПОПЫТОК. Писец в WAL может быть только
        # один, и соседний оп (sverka_prigovorov primenit) держит замок
        # десятками минут. Восьми попыток по 20 секунд не хватало ни на
        # одну пачку — заливка возвращала ноль строк. Ждём до часа на
...
всего = 0
for к in ("01.11", "01.11.1", "01.50", "01.41"):
    н = по_кодам.get(к, 0)
    всего += н
...
if ПРИМЕНИТЬ:
    ...
    print("   пачек всего было: %d по %d строк"
          % ((len(новых) + ПАЧКА - 1) // ПАЧКА, ПАЧКА))
print("")
