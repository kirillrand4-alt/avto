...
def _арг_пачка(по_умолчанию=5000):
    for а in sys.argv[1:]:
        if а.startswith("пачка=") or а.startswith("pachka="):
            try:
                return max(1, int(а.split("=", 1)[1]))
            except ValueError:
                pass
    return по_умолчанию
...
if ПРИМЕНИТЬ and новых:
    ...
    # РАЗМЕР ПАЧКИ — ЭТО ЧИСЛО ЗАХВАТОВ ЗАМКА, А НЕ СКОРОСТЬ ЗАПИСИ. Сама
    # вставка пятисот строк идёт миллисекунды; всё время уходит на ожидание
    # замка, который часовая сверка приговоров держит подолгу. Пачками по
    # 500 нужно полсотни окон, пачкой в 5000 — пять. Транзакция при этом
    # остаётся короткой, а INSERT OR IGNORE оставляет прогон идемпотентным.
    ПАЧКА = int(_арг_пачка())
