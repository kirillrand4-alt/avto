...
def _прочитать_журнал(путь, куда):
    """ИНН → множество имён, которыми его уже спрашивали.

    Раньше журнал давал просто «спрашивали», и цель больше не переспрашивали
    никогда. Но запрос строится ИЗ ИМЕНИ, а имя у нас поменялось: пришли
    официальные КРАТКИЕ наименования, и «ГУСП МТС "ЦЕНТРАЛЬНАЯ" РБ» — это
    совсем другой запрос, чем полная форма на двести знаков. Спрашивать
    повторно тем же именем незачем, другим — нужно.
    """
    ...
    with open(путь, encoding='utf-8', errors='replace') as ж:
        for строка in ж:
            ...
            и = д.get('инн')
            if и:
                куда.setdefault(и, set()).add((д.get('имя') or '').strip().lower())
...
def main():
    ...
    имена = {}
    есть_сайт = set()
    for и, н, к, с in q("SELECT inn,COALESCE(name,''),COALESCE(short_name,''),"
                        "COALESCE(site,'') FROM companies"):
        # краткое официальное имя предпочтительнее: выдача по нему точнее, а
        # полная форма на двести знаков часто не находит ничего
        имена[и] = (к.strip() or н)
        if с.strip():
            есть_сайт.add(и)
    опрошены = {}
    ...
    работа, без_имени, тем_же = [], 0, 0
    for c in цели:
        if c['inn'] in есть_сайт:
            continue
        имя = (имена.get(c['inn']) or '').strip()
        # в журнал имя пишется обрезанным до 70 знаков — сравниваем так же,
        # иначе длинное имя не совпадёт само с собой и цель переспросится зря
        if имя and имя[:70].lower() in опрошены.get(c['inn'], set()):
            тем_же += 1
            continue
        if not имя:
            имя = (c.get('poln') or '').strip()
            if имя[:70].lower() in опрошены.get(c['inn'], set()):
                тем_же += 1
                continue
    ...
    print(f'целей в файле: {len(цели)}; уже с сайтом: '
          f'{len(есть_сайт & {c["inn"] for c in цели})}; '
          f'пропущено (тем же именем уже спрашивали): {тем_же}', flush=True)
