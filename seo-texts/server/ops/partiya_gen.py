...
НА ПИСЬМО ПРИХОДИТСЯ ДВЕ СТРОКИ ЖУРНАЛА: «текст» - сразу после генерации,
ДО первого обращения к базе, и «итог» - после постановки в очередь. Так
письмо переживает и блокировку базы, и падение потока: текст уже на диске,
а в очередь его доложит ops/partiya_dolozhit_iz_zhurnala.py.
"""
import io
import json
import os
