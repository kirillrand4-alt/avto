...
def _один(g):
    ...
    # ТЕКСТ ПИСЬМА В ЖУРНАЛ ДО ВСЕГО. Владелец 17.08: «ты их сохраняешь
    # надеюсь?» - не сохранял. В журнал шли только имя, цена и статус, а сам
    # текст жил лишь в оперативной памяти до записи в очередь. Заблокированная
    # база - и оплаченное письмо исчезало навсегда. Теперь текст ложится в
    # durable-журнал ПЕРВЫМ делом: даже если очередь недоступна, письмо можно
    # положить туда потом, не платя за него второй раз.
    if L:
