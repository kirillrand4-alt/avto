...
# ХВОСТ КАРТОЧКИ, ПРИКЛЕЕННЫЙ К ИМЕНИ. Первая редакция резала
# «Контракт №», «Договор №», «Объекты закупки», «Номер извещения» — но
# у карточек 223-ФЗ хвост другой: «Цена договора 119 998». Из-за него
# нормализованное имя не совпадало ни с чем, и ВСЕ заказчики выглядели
# новыми для базы. Цифра «промышленных новых 57 из 57» была артефактом
# обрезки, а не находкой.
_ХВОСТ = re.compile(r'\s+(?:Контракт\s*№|Договор\s*№|Цена\s+договора|'
                    r'Объекты\s+закупки|Номер\s+извещения|Сумма|'
                    # БЕЗ \b НА КОНЦЕ: после «№» границы слова нет (оба
                    # символа не буквенные), и хвост «Контракт № 17Z» не
                    # срезался, хотя «Цена договора» срезалась. Проверять
                    # надо на ОБОИХ видах хвоста, а не на одном.
                    r'Дата\s+заключения|Статус).*$', re.I | re.S)
