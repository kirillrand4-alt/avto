...
def одно(rid):
    ...
    строка = json.dumps({
        ...
        # Причины брака нужны поимённо: «генерация забракована» не говорит,
        # кто забраковал — механический гейт (детерминированный, от модели не
        # зависит) или верификатор с линзой (зависят). Без этого выбор
        # «вернуть опус или чинить правило» — гадание.
        "fails": res.get("fails"),
