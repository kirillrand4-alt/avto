...
def main():
    ...
    for и, (значение, причина) in скрытые.items():
        ...
        if not новый:
            # ПРИГОВОРА НЕТ — И ЭТО ДВА РАЗНЫХ СЛУЧАЯ, а не один.
            #
            # Первая версия писала всем «ждёт пересуда, приговор снят», и я
            # доложил владельцу «592 ждут приговора». Это было неверно.
            # Замер по причинам скрытия: из 592 ни одно дело суд не открывал
            # вовсе — 143 садовый инструмент и вихревые, 108 без единого
            # подтверждающего факта, 67 ликвидированные юрлица, остальные
            # школы и больницы по ОКВЭДу. Суд их не судил и судить не должен.
            #
            # Отличаем по причине скрытия: подпись суда есть — дело
            # действительно переоткрыто и ждёт; подписи нет — убрано по другому
            # основанию, и это не очередь, а закрытый вопрос.
            if _НАШЕ_СКРЫТИЕ.search(причина):
                остаток['переоткрыто судом, ждёт приговора'] += 1
            else:
                остаток[f'скрыто не судом: {(причина or "без причины")[:44]}'] += 1
            continue
