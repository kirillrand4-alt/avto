...
# ОДНА ЗАПИСЬ НА ПИСЬМО. С 17.08 журнал пишется дважды: «текст» - сразу
# после генерации, до обращения к базе, и «итог» - после постановки в
# очередь. Считать обе строки значило бы удвоить и число писем, и
# потраченные деньги. «Итог» замещает свой «текст» и встаёт на его место;
# «текст» без итога остаётся - это письмо, чей поток умер на постановке
# в очередь, и прятать его из отчёта нельзя. Старый журнал (строки без
# этапа) считается как раньше, по строке на письмо.
пары, попытка = {}, {}
...
без_итога = sum(1 for x in z if x.get("этап") == "текст")
if без_итога:
