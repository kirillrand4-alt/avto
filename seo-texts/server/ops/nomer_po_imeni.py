...
def main():
    ...
    def одно(инн):
        ...
        with _ЗАМОК:
            ...
            # НАХОДКИ ПИШЕМ В ЖУРНАЛ ПОИМЁННО. Сперва я журналировал только
            # счётчики, и разбор «кому какой номер достался» существовал лишь
            # в возвращаемом выводе — то есть исчезал при рестарте песочницы.
            _журнал({'инн': инн, 'страниц': len(страницы),
                     'нашли': len(нашли), 'находки': нашли,
    ...
    # ОДИН НОМЕР НА МНОГИХ — НЕ ЛИЧНЫЙ. На странице структуры перечислены
    # восемь начальников цехов и ОДИН телефон здания; окно вокруг каждой
    # фамилии честно ловит его же. Записать это как личный номер восьмерых
    # значит соврать восемь раз. Номер, доставшийся трём и более людям на
    # одном предприятии, пишем БЕЗ привязки к человеку — как номер
    # организации, чем он и является.
    счёт = Counter()
    for н in находки:
        счёт[(н['инн'], re.sub(r'\D', '', н['номер'])[-10:])] += 1
    личных = общих = 0
    for н in находки:
        общий = счёт[(н['инн'], re.sub(r'\D', '', н['номер'])[-10:])] >= 3
        н['вид'] = 'номер организации' if общий else 'личный'
        личных += 0 if общий else 1
        общих += 1 if общий else 0
    итог['записано как личный'] = личных
    итог['записано как номер организации (один на многих)'] = общих
    if ПРИМЕНИТЬ:
        видано_общих = set()
        for н in находки:
            if н['вид'] == 'личный':
                db.add_phone(н['инн'], н['номер'], person=н['фио'],
                             role=н['роль'],
                             source='сайт: номер рядом с фамилией',
                             source_url=н['url'])
            else:
                к = (н['инн'], re.sub(r'\D', '', н['номер'])[-10:])
                if к in видано_общих:
                    continue
                видано_общих.add(к)
                db.add_phone(н['инн'], н['номер'],
                             source='сайт: общий номер со страницы структуры',
                             source_url=н['url'])
        db.cx.commit()

    стало = q('SELECT COUNT(*) FROM phone_contacts').fetchone()[0]
    # СКОЛЬКО РЕАЛЬНО ЛЕГЛО. Ключ phone_contacts — (ИНН, номер, ссылка), и
    # семь человек с ОДНИМ номером с одной страницы схлопываются в одну
    # строку: шесть привязок исчезают молча. Поэтому число «нашли» и число
    # строк в базе расходятся, и верить надо второму.
    в_базе = q("SELECT COUNT(*) FROM phone_contacts "
               "WHERE source LIKE 'сайт: %номер%'").fetchone()[0]
    print()
