...
phone_contacts/emails + core_sites_stream.jsonl. Резюмируемо. 12 потоков
...
НАЧАЛО = time.time()
ПОТОК = r'C:\seostat\drop\core_sites_stream.jsonl'
...
for ln in io.open(_ЦЕЛЕВОЙ, encoding='utf-8', errors='replace'):
    ...
    if not m or m.group(1) in цели:
        continue
    i = m.group(1)
    r0 = e.execute('SELECT phones FROM companies WHERE inn=?', (i,)).fetchone()
    известные = set()
    try:
        for t in json.loads((r0[0] if r0 else '') or '[]'):
            ц = re.sub(r'\D', '', str(t))[-10:]
            if ц:
                известные.add(ц)
    except Exception:
        pass
    цели[i] = {'их_тел': известные}
row_map = {}
for i in list(цели):
