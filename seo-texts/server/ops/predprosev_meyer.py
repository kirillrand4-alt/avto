...
ДВУХ станков:
  ...
  * рентген-инспекция - поиск плотных посторонних включений (металл, стекло,
    камень, кость) в продукте, в том числе уже упакованном.

ПОКУПАТЕЛЬ - ЛЮБОЕ пищевое, напиточное и агропроизводство, а также
переработка вторсырья, пластика, стекла и минерального сырья.

ЖИДКИЙ ИЛИ УПАКОВАННЫЙ ПРОДУКТ - НЕ ПРИЧИНА ОТКАЗАТЬ. Это прямое указание
владельца 26.08, и оно сильнее рассуждений о том, что «сепаратор сыпучему
потоку, а тут бутылки»:
  * молокозавод выпускает не только молоко - сыры, масло, творожные изделия,
    сухое молоко и сыворотку; рентген проверяет упакованное, сепаратор -
    порошок;
  * винодельня годится тоже: одна из них вчера была заинтересована в покупке.
Пищевому заводу ставь «мейер», даже если по первому взгляду кажется, что
станок ему не подходит: что именно у него на линиях, знает он сам.

НЕ ПОКУПАТЕЛЬ по профилю: торговля, дистрибуция, интернет-магазин, оптовые
базы, логистика, склад-фулфилмент, аренда, проектирование, экспертиза,
оценка, консалтинг, госорган, банк, клиника.
...
ВТОРОЕ НАПРАВЛЕНИЕ. Если производство НЕПИЩЕВОЕ - металлообработка,
электроника, электротехника, химия, краски, стройматериалы, мебель,
деревообработка, машиностроение, производство тары и упаковки - Meyer им не
нужен, но нужен сжатый воздух: ответь «кц».

СТРОГО ПРО «НИКУДА» - это РЕДКИЙ вердикт. Ставь его ТОЛЬКО когда своего
производства нет вовсе: чистая торговля, логистика, аренда, услуги,
госорган. Есть завод, цех, линия, розлив, обжиг, литьё, сварка, покраска,
сборка, экструзия, фасовка - это «мейер» или «кц», но не «никуда».
...
# Страховка от разрушительной ошибки — «никуда» убирает компанию из ОБОИХ
# направлений, а «кц» стоит одного лишнего письма.
...
# ОТРИЦАНИЕ ПЕРЕД СЛОВОМ ОТМЕНЯЕТ ЕГО. Первая версия страховки читала
# «своего производства НЕТ» как довод ЗА производство и переводила чистых
# логистов и оптовиков в «кц»: на замере 60 компаний так ошиблись четыре
# раза из шести. Смотрим двадцать знаков перед словом.
# Отрицание ищем с ОБЕИХ сторон: «не производство» и «производства нет»
# значат одно и то же. Справа берём только ближайшее слово — в «производство
# упаковки, а не фасуют продукт» это «не» относится к фасовке, а не к цеху.
_ОТРИЦАНИЕ_СЛЕВА = re.compile(r"\b(не|нет|без|отсутству\w*)\b\s*\S*\s*$", re.I)
_ОТРИЦАНИЕ_СПРАВА = re.compile(r"^\w*\s+(не|нет|отсутству\w*)\b", re.I)


def est_svoy_ceh(почему: str) -> bool:
    """Есть ли в объяснении довод ЗА своё производство (а не против)."""
    т = str(почему or "")
    for м in _ЕСТЬ_ЦЕХ.finditer(т):
        слева = т[max(0, м.start() - 20):м.start()]
        справа = т[м.end():м.end() + 16]
        if (not _ОТРИЦАНИЕ_СЛЕВА.search(слева)
                and not _ОТРИЦАНИЕ_СПРАВА.search(справа)):
            return True
    return False
...
def партия(часть):
    ...
    for в in данные.get("verdicts", []):
        ...
        if вердикт == "никуда" and est_svoy_ceh(почему):
            вердикт = "кц"
