# -*- coding: utf-8 -*-
"""Судья по ОЧЕРЕДИ ПОДТВЕРЖДЕНИЯ: те же проверки, но источник — все
письма в статусе pending, независимо от того, какой прогон их поставил.
...
СЛЕД = r"C:\sender\_ops\sud-ocheredi.jsonl"
...
строки = c.execute(
    ...
    " WHERE cr.status='pending' AND COALESCE(cr.body,'')<>'' "
    "   AND COALESCE(cr.kind,'outbound')<>'reply' ORDER BY cr.id").fetchall()
c.close()
шаг = max(1, len(строки) // СКОЛЬКО)
проба = [строки[i] for i in range(0, len(строки), шаг)][:СКОЛЬКО]
print("в очереди подтверждения %d, беру %d, модель %s"
      % (len(строки), len(проба), МОДЕЛЬ))
