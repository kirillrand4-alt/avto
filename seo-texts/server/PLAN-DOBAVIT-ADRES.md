...
## 7. ЧТО СДЕЛАНО ПО ЭТОМУ ПЛАНУ (5 августа, сессия выполнения)

**Порядок соблюдён: сначала бой → репозиторий, потом правка.** `preflight_panel.py`
показал 8 файлов, где репо было МЕНЬШЕ боевого (ai_letter, ai_quota, api/app,
confirm, infopanel, postoffice, sender, store) плюс `gender_agree.py` только на
сервере. Все сняты с `C:\sender\sender` раннером (`panel_file_put get`), сверены
по размеру и sha, залиты в репозиторий отдельным коммитом ДО правки. Проверка на
потери: ни одной функции и ни одного маршрута репо в бою не пропало — бой строго
шире. Правка легла поверх боевых версий, так что 5 серверных маршрутов и фильтр
направления КЦ/Meyer сохраняются.

**Сделано по пунктам плана**

| Пункт | Статус | Отличие от плана и почему |
|---|---|---|
| 1.1 `find_recipient_by_email` | сделано | нормализация — через `_normalize_recipient_identity` (тот же канон, что `upsert_recipient`), а не голый strip/lower |
| 1.1 `confirm_update_panel` | НЕ понадобился | в бою уже есть `store.confirm_set_panel` с той же семантикой (перезапись panel_json только для pending) — второй метод был бы дублем |
| 1.2 `add_recipient_email` | сделано | аудит пишется через реальный `append_audit` (метода `add_audit` в store нет); добавлены две проверки, которых в плане не было: статус карточки сверяется ДО записи в базу (иначе на решённой карточке контакт заводился бы впустую) и карточка без ИНН отклоняется (объём владельца — «с привязкой к ИНН», без него проверка «чужая компания» не работает) |
| 1.3 эндпоинт + модель | сделано | фича-флаг: `deps.settings_flag` в коде нет — читаем как `allow_out_of_base` (panel_settings → config → дефолт ВКЛ) внутри движка, чтобы тумблер действовал и для CLI; выключенная фича = `ManualRecipientDisabled` → 403, комплаенс-блок → 409, формат/статус/дедуп → 400 |
| 1.4 довесок по ФЗ-38 | сделано | `store.delivery_emails_for_recipient` (send_log → messages.recipient_id) + `Suppression.add_delivery_aliases`, вызывается из `unsub.handle_one_click` и трёх точек `imap_watcher` (отписка текстом, hard bounce, жалоба). Ограничение названо в коде: ответы в тред пишут send_log без `message_id` и в выборку не попадают |
| 2.1–2.2 фронт | сделано | как в плане: поле «новый адрес» + кнопка в `.confirm-routing`, сброс при смене карточки, Enter внутри поля не отправляет письмо (глобальные хоткеи игнорируют INPUT) |
| 6.8 паритет CLI | сделано командой, а не записью расхождения | `sender confirm-recipient <id> <email> [--add]` зовёт тот же движок; в PARITY.md две строки, в MANUAL-SEND.md — таблица «две разные ручки» |

**Чем проверено (локально, без боя)**

* `sender/tests/test_confirm_add_recipient.py` — 14 тестов: заведение контакта
  под ИНН карточки и попадание в контакты, повтор без дублей, чужой ИНН → 409,
  стоп-лист → 409, мусорный адрес → 400, решённая карточка → 400 (база не
  тронута), карточка без ИНН → 400, тумблер → 403, без токена → 401, коллизия
  очереди → 400, старая ручка осталась закрытой, и два теста ФЗ-38: отписка
  закрывает адрес доставки, а без подмены ничего лишнего не суппрессится.
* `sender/tests/test_confirm_parity.py::test_add_recipient_parity_cli_web` —
  CLI и панель дают одинаковое состояние базы.
* `sender/web/tests/confirm_add_recipient.test.tsx` — кнопка шлёт адрес в новую
  ручку, до «@» неактивна, 409 показывается причиной.
* Весь сьют: `python3 -m pytest sender/tests/ -q` — зелёный; фронт:
  `npx tsc -b --noEmit` и `npm run test` — зелёные.
* `test_change_email_rejects_foreign` (запрет из п. 6.1) остался зелёным.

**Что НЕ сделано и почему**

* Пункты 3 (сборка `panel-update.zip` с `WITH_WEB=1`, заливка на дроп) и 4
  (боевые пробы, живой approve) — выкладка. Она меняет боевую панель и требует
  решения владельца; код к ней готов, `preflight_panel.py` теперь вернёт 0.
* Риски 3 и 4 из п. 5 (статистика по домену исходного получателя, выбор ящика
  по `mx_provider` строки получателя) этой правкой, как и было записано в плане,
  не чинятся.
