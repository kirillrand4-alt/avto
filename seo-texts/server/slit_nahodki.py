...
# ЗАМЕР 30.08, из-за которого числа стали другими. Разложить находки по гейту —
# 2,9 секунды, прочитать сайты — полсекунды, записать строку — ОДНА миллисекунда
# (WAL, коммит дёшев). То есть восемь тысяч строк пишутся полторы минуты, из
# которых восемьдесят секунд — моя собственная передышка. Тормозил не SQLite, а
# осторожность, заведённая когда за базой стоял голодный мост Зенки.
# Зенка закрыта, очередь за базой короткая — открываем кран, оставив паузу
# короткой, но ненулевой: окно другим писателям всё равно нужно.
ПАЧКА = int(os.environ.get('SLIV_PACHKA', 500))
ПЕРЕДЫШКА = float(os.environ.get('SLIV_PEREDYSHKA', 0.2))
ЗА_ПРОГОН = int(os.environ.get('SLIV_ZA_PROGON', 50000))
