...
# СТРАНИЦА-СПИСОК. Владелец 30.08 посмотрел глазами на компании, у которых
# набралось по восемьсот адресов: каталог участников выставки (niva-expo.ru),
# дилерская сеть r_keeper, комментарии посетителей kraspivo.ru, список врачей
# bareo.ru. Всюду адреса принадлежат НЕ той компании, под чьим ИНН записаны.
#
# Он же поправил первую версию правила: «у прокомпрессора тоже больше 15 адресов
# на странице». Верно — настоящая контактная страница крупной компании даёт
# сотни адресов, и это законно. Отличает не количество, а РАЗБРОС ДОМЕНОВ:
#   ldc.ru/kontakty      1 879 адресов, 100% на своём домене, 4 чужих домена
#   urfu.ru/about/units    309 адресов,  99% своих
#   vkmspb.com/shop2.php 1 020 адресов,   0% своих, 729 РАЗНЫХ чужих доменов
#   niva-expo.ru           829 адресов,   0% своих, 509 чужих доменов
# Ни у одной компании не бывает семисот почтовых доменов. Поэтому отсекаем
# страницу, только если адресов много И меньше половины из них на своём домене.
СТРАНИЦА_МНОГО = 15   # адресов с одной страницы у одной компании
СТРАНИЦА_СВОИХ = 0.5  # ниже этой доли своего домена — это справочник
...
def разложить(предел=0, c=None):
    ...
    все_стр = c.execute("select inn, email, coalesce(source_url,'') "
                        'from nahodki_pochta').fetchall()
    ...
    # страницы-списки: считаем по ВСЕЙ выборке, как и портальность доменов
    стр_адреса = collections.defaultdict(list)
    for inn, email, url in все_стр:
        стр_адреса[(str(inn), url)].append(email)
    списки = set()
    for (inn, url), адр in стр_адреса.items():
        if len(адр) <= СТРАНИЦА_МНОГО:
            continue
        свой = сайты.get(inn, '')
        своих = sum(1 for e in адр if _родня(e.split('@')[-1], свой))
        if своих / len(адр) < СТРАНИЦА_СВОИХ:
            списки.add((inn, url))
