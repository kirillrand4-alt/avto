...
def _сайты():
    c = sqlite3.connect('file:%s?mode=ro' % ENRICH.replace('\\', '/'), uri=True,
                        timeout=60)
    д = {}
    for i, s, cs in c.execute("select inn, coalesce(site,''), coalesce(cand_site,'') "
                              'from companies'):
        x = _домен(s or cs)
        if x:
            д[str(i)] = x
...
def разложить(предел=0, c=None):
    ...
    телефоны = c.execute('select inn,phone,source_url from nahodki_telefon '
                         'where coalesce(slito,0)=0').fetchall()
    if своё:
        c.close()
    ...
    for inn, email, role, role_src, ctx, src, url, skryt in строки:
        inn = str(inn)
        сайт = сайты.get(inn, '')
        дом = email.split('@')[-1]
        стр = _домен(url)
        ...
        if skryt:
            ...
            if len(дом_инн[дом]) >= ПОРТАЛ_ИНН:
                ...
                причина = 'чужой домен, редкий — под вопросом'
        if причина:
            отсев[причина] += 1
    ...
    for inn, phone, url in телефоны:
        inn = str(inn)
        сайт = сайты.get(inn, '')
        причина = ''
        if not сайт:
            причина = 'сайт компании неизвестен'
        elif not _родня(_домен(url), сайт):
            причина = 'страница не своего сайта'
...
def слить(ждать_минут=15, предел=0):
    ...
    if not _свободна():
        итог['итог'] = ('база занята все %d минут — ничего не лили, находки целы'
                        % ждать_минут)
        н.close()
        return итог
    ...
    for з in годные:
        try:
            db.add_email(з['inn'], з['email'], role=з['role'], source=ИСТОЧНИК,
