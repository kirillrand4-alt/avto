...
Подкоманды:
  ...
  apply --file X          — записать классифицированные сигналы в enrich.db
        ...
        for x in капекс[:статей]:
            ...
            with lock:
                if текст:
                    ...
                    сч['пусто'] += 1
                # ЯДРО БОЛЬШЕ НЕ СЧИТАЕТСЯ ПРИВЯЗКОЙ. Третья сессия
                # проверила все 66 привязок «компания↔новость» по настоящему
                # тексту постов и нашла 16 мисматчей, из них 9 — тёзки,
                # прошедшие именно по уровню «ядро». Сам код это и предупреждал
                # («ядро — слабое (тёзки!)»), но строка приёма брала его
                # наравне с ИНН и полным именем.
                # Эталон её разбора: ПАО «ХИМПРОМ» (Чувашия) получило пост про
                # завод «в Волгограде, на площадке БЫВШЕГО Химпрома» —
                # совпало одно слово, компания другая, город другой.
                # Ни в одном из 31 проверенного поста ИНН не встретился ни
                # разу: источник — новостные паблики, а не страницы компаний,
                # поэтому сильнейшее доказательство недоступно в принципе, и
                # тем более нельзя добирать слабейшим.
                if прив['уровень'] in ('инн', 'полное_имя'):
                    сч['привязка_ок'] += 1
                elif прив['уровень'] == 'ядро':
                    # не выбрасываем молча: тёзку надо видеть и уметь оспорить
                    сч['отклонено_тезка'] = сч.get('отклонено_тезка', 0) + 1
                if наше:
                    сч['наше_оборудование'] += 1
            # Гейт по региону: если в тексте прямо назван ДРУГОЙ регион, а
            # наш не упомянут — это подозрение, и оно должно быть видно в
            # выгрузке. Не отсев: в половине новостей регион не называют
            # вовсе, и отсев по молчанию выбросил бы верные привязки.
            рег = (c.get('region') or '').strip()
            рег_ток = [w for w in re.findall(r'[А-Яа-яЁё]{5,}', рег)][:2]
            текст_низ = (текст or x['passage'] or '').lower()
            регион_совпал = (any(w.lower()[:6] in текст_низ for w in рег_ток)
                             if рег_ток else None)
            статьи.append({'регион_совпал': регион_совпал,
