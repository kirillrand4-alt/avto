...
def прогон(процессов=8, минут=0, предел=0, источник='razobrano'):
    ...
    сейчас = time.strftime('%Y-%m-%dT%H:%M:%S')
    с_прошлой_записи = 0
    # ГРОМКИЙ ПРОГРЕСС. Полдня я гадал, работает прогон или стоит: отметки
    # «сделано» ложатся в базу пачками, и между ними процесс молчит. Печатаем
    # шаги прямо в лог, с промывкой — тогда видно фазу, а не только результат.
    print('очередь построена: %d целей, источник %s' % (len(цели), источник),
          flush=True)
