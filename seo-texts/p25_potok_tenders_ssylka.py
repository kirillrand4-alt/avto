...
# ПЕРВЫЙ ЗАХОД ОТВЕРГ 38 706 СТРОК со словами «площадка не ЕИС либо номер не цифровой».
# Формулировка моя, и она склеивала две разные причины — поэтому была бесполезна, пока я не
# посмотрела сами значения:
#
#     platform:    etpgpb 37 428 | eis 1 691 | tenderpro 1 513 | svod_loc 993 | epb 742
#     reg_number:  ГП###### 11 740 | 11 цифр 11 075 | 6 цифр 10 349 | АП###### 2 717
#
# ЕИС там меньше пяти процентов. Основная площадка — ЭТП ГПБ, и её номер записан с буквенной
# приставкой. Форму адреса беру НЕ из головы, а из рабочих ссылок, уже лежащих в базе:
#
#     https://etpgpb.ru/procedures/etp/738892/      <- короткая, без словесного хвоста
#     https://etpgpb.ru/procedure/tender/etp/738892-postavka-turbokompressora-.../
#
# Короткая форма без хвоста в базе есть и работает, её и строю: хвост из названия я бы
# сочиняла, а он у площадки свой.
# ВТОРАЯ ПОПРАВКА, после проверки двадцати ссылок ЭТП ГПБ: открылись все двадцать, и ни на
# одной нет ИНН. Две причины, обе видны в самих адресах и в наших же прежних записях:
#
#   1. Под `platform=etpgpb` лежат номера РАЗНЫХ реестров. `0345300017223000468` и
#      `31401066685` — это реестровые номера ЕИС, а не идентификаторы ЭТП ГПБ, и адрес вида
#      `/procedures/etp/0345300017223000468/` отдавал 200 на любую строку. Значит регистр
#      определяет ФОРМА НОМЕРА, а не колонка площадки.
#   2. Сам ЭТП ГПБ — приложение на Nuxt, и в нашем же сборщике площадок записано: «выдача
#      появляется только после гидратации», ожидание 30 000 мс. Простым запросом карточку
#      не увидеть в принципе, поэтому его строки проверяются браузером отдельно и в поток
#      сейчас не идут.
