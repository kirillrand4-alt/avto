...
# обрезается: разбор падает, ошибка приходит ПУСТОЙ — ни статуса, ни длины, ни тела.
# Замер: 2 предприятия × 5 страниц = 79 КБ, разбирается; 6 × 20 = мегабайты, 759 пачек подряд
# упали и собрано ноль. Поэтому пачка держится маленькой, а параллельность высокой — раннер
# держит 30 воркеров, и десять коротких заданий лучше одного длинного.
