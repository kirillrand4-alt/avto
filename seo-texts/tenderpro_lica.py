                        ...
                        for ch in k.get('lyudi') or []:
                            ...
                            for t in tels:
                                ...
                                if t and not est:
                                    ...
                                    # пометку никто не читал: сборка панели берёт колонку
                                    # `telefon` как есть. Замер 03.08: 32 таких номера, и НИ
                                    # ОДИН не встречается в карточке — ни в комментарии, ни в
                                    # размеченных телефонах площадки. Пять из них доехали до
                                    # панели ЛИЧНЫМИ МОБИЛЬНЫМИ, то есть продавец позвонил бы
                                    # постороннему человеку и назвал его главным энергетиком.
                                    # Имя оставляем: оно из текста и проверяемо. Номер — нет.
                                    sch['vydumka'] += 1
                                    t = ''
