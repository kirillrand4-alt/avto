...
# Отчество — самый дешёвый и надёжный признак того, что в хвосте назван ЧЕЛОВЕК.
_OTCH = re.compile(r'[А-ЯЁ][а-яё]+(?:ович|евич|ьевич|овна|евна|ьевна|ична|инична)\b')
_FIO_INIC = re.compile(r'[А-ЯЁ][а-яё]{2,}\s+[А-ЯЁ]\.\s?[А-ЯЁ]\.|[А-ЯЁ]\.\s?[А-ЯЁ]\.\s?[А-ЯЁ][а-яё]{2,}')


def za_rubezhom(comment):
    """Добавляет ли полный текст что-то за прежним обрезом 1 500 знаков.

    ПОПРАВКА 03.08 по обратной проверке (пункт 16). Прежде признаком считались только телефон и
    техническое слово — ИМЯ ЧЕЛОВЕКА признаком не было. Замер по корпусу: 3 052 карточки
    разобраны только по обрезку, из них 1 241 длиннее 1 500 знаков; 945 не дали ни одного
    человека, ХОТЯ У 831 в карточке есть телефон, а у 37 ФИО стоит только за 1400-м знаком —
    модель их физически не видела. Теперь ФИО в хвосте тоже повод переспросить.
    """
    c = comment or ''
    if len(c) <= 1500:
        return False
    ...
    return bool(TEL_LYUBOY.search(hvost) or TEH_SLOVO.search(hvost)
                or _OTCH.search(hvost) or _FIO_INIC.search(hvost))
