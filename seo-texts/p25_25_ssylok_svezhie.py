...
def sudit(z, t):
    ...
    if re.search(r'/epz/organization/view', z.get('url') or ''):
        vid = 'ИНН'
    if vid == 'машина':
        # ДВА ВОПРОСА, А НЕ ОДИН. Факт парка утверждает «у ЭТОГО предприятия ЕСТЬ ЭТА
        # машина», а проверка спрашивала только «есть ли на странице слово машины». Разница
        # стоила мне всех прежних цифр «ДОКАЗЫВАЮТ N из 5».
        #
        # Поймано двумя независимыми приборами в один час:
        #   • мой отрицательный контроль `etpgpb.ru/procedures/?search=щварцкопфер` получил
        #     вердикт ДОКАЗЫВАЕТ — выдача площадки показывает компрессорные процедуры на
        #     любой запрос, и «слово машины» проходит на ней даром;
        #   • 1-я сессия (запись 139) взяла ВОСЕМЬ МОИХ ссылок и спросила их двумя вопросами:
        #     машина названа 8 из 8, наш ИНН на странице 0 из 8.
        #
        # Оба верны, и мерки разные: «такая машина закупалась» и «машина у этого
        # предприятия» — разные утверждения, а в парк я кладу второе.
        # У ПОИСКА ПО КОДУ СВОЙ ВОПРОС: СТОИТ ЛИ ЗАПРОШЕННЫЙ КОД НА СТРАНИЦЕ.
        # Проба ЭТП ГПБ, четыре адреса подряд:
        #     настоящий ГП131308   8 948 знаков, свой код True,  слово машины True
        #     настоящий ГП415801   9 032 знака,  свой код True,  слово машины True
        #     ВЫДУМАННОЕ слово     8 924 знака,  свой код False, слово машины TRUE  <- пробой
        #     ВЫДУМАННЫЙ ГП999999  9 379 знаков, свой код False, слово машины False
        # То есть на выдуманное слово площадка всё равно показывает компрессорные процедуры,
        # и признак «слово машины» проходит даром. Признак «свой код на странице» различает
        # 4 из 4. Спрашиваю именно его — а слово машины остаётся вторым условием.
        m_kod = re.search(r'[?&](?:search|searchString|query_field|name|keywords)=([^&#]+)',
                          z.get('url') or '')
        if m_kod:
            kod = _up.unquote(m_kod.group(1))
            if len(kod.split()) == 1 and len(re.sub(r'\D', '', kod)) >= 5:
                if kod.lower() not in t.lower():
                    return False, 'запрошенный код на странице НЕ найден — выдача не про него'
        est_mash = bool(MASH.search(t))
        cifry = re.sub(r'\D', '', t)
        inn = str(z.get('inn') or '')
        imya = re.sub(r'^(ООО|АО|ПАО|ЗАО|ОАО|ФГУП|ГУП|МУП)\s*', '',
                      str(z.get('imya') or '').strip(' "«»')).strip(' "«»')
        est_inn = bool(inn) and inn in cifry
        est_imya = len(imya) >= 6 and imya.lower()[:24] in t.lower()
        if not est_mash:
            return False, 'машины на странице нет'
        if est_inn or est_imya:
            return True, ('машина + ИНН на странице' if est_inn
                          else 'машина + название предприятия на странице')
        # Машина есть, а чья — не сказано. Это НЕ доказательство факта парка, и считать его
        # доказательством значит завышать собственные числа.
        return False, 'машина есть, а ЧЬЯ — на странице не сказано'
    if vid == 'ИНН':
...
def rabotnik():
    while True:
        ...
        if not t or len(t) < 120 or oshibka_stranicy:
            ...
            est, chem = sudit(z, t)
            # ИСХОД НАЗЫВАЕТ ПРИЧИНУ, а не сваливает всё в «искомого нет». «Машина есть, а
            # чья не сказано» и «машины на странице нет вовсе» — разные беды: первая лечится
            # добором второй ссылки на ИНН, вторая означает негодный источник.
            ishod = ('ДОКАЗЫВАЕТ' if est
                     else 'МАШИНА ЕСТЬ, ЧЬЯ — НЕ СКАЗАНО'
                     if chem.startswith('машина есть, а ЧЬЯ')
                     else 'ОТКРЫЛАСЬ, ИСКОМОГО НЕТ')
        with zamok:
