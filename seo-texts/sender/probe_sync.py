...
class ProbeSync:
    ...
    def забрать(self, письма: list = None) -> dict:
        ...
        для_обогащения: list = []
        снято = новых = отклонено = без_изменений = 0
        ...
        for с in строки:
            ...
            стояло = self.probe.cached(адрес)
            if not стояло:
                новых += 1
            ...
            легло = True if то_же else self.probe._save(
                ...
                str(з.get("mx") or ""))
            if легло is False:
                # Поверх приговора доставки. Письмо на этот адрес уже ушло и
                # отбилось — «приму» от сервера этого не отменяет: домен
                # принимает любой адрес и разбирается потом. Именно так 18.08
                # kk@vebfabrika.ru вернулся в работу (код 250 в 04:45 поверх
                # «нет ящика»). Дальше ведём себя по приговору, а не по
                # ответу работника: иначе «есть» уедет в обогащение и снова
                # выпустит адрес в отбор кандидатов.
                стоял = self.probe.cached(адрес) or {}
                вердикт = str(стоял.get("verdict") or вердикт)
                з["answer"] = str(стоял.get("answer") or з.get("answer") or "")
                з["code"] = стоял.get("code", з.get("code"))
                отклонено += 1
            свод[вердикт] = свод.get(вердикт, 0) + 1
