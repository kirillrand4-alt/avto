...
## Постмастера (можно уже сейчас — DNS-подтверждение писем не требует)

Кабинеты мониторинга доставляемости у провайдеров. Ключевой факт (Mail.ru,
дословно из справки): «статистика ведётся по домену из DKIM-подписи (тег d)» —
поэтому добавлять надо ВСЕ домены-отправители (и Я360, и VK, все 11 активных)
в ОБА кабинета: письма с яндекс-доменов в ящики Mail.ru учитывает постмастер
Mail.ru, и наоборот.

- **postmaster.mail.ru** (доставка в ящики Mail.ru/VK — ~51% базы):
  войти под админ-аккаунтом Mail.ru → «Добавить домен» → подтверждение.
  Способы: HTML-файл / meta-тег / **DNS-запись** — наш вариант (у двойников
  нет сайтов, только редиректы; DNS «можно использовать даже при отсутствии
  сайта»). Код индивидуальный — вносится TXT-записью в зону у регистратора,
  как DMARC. Внутри: доставлено/в спам, жалобы «это спам» (FBL), репутация,
  динамика день/неделя/месяц. Наши гейты смотрят те же метрики:
  complaint <0.1% (стоп >0.3%), bounce 2-3%. Интеграция в сендер готова:
  `postoffice.py` — получить API-токен постмастера → env POSTOFFICE_TOKEN
  (пути API сверяются при первом live-вызове, заложено в модуле).
- **У Яндекса постмастера НЕТ** (проверено 2026-07-24: «Почтовый офис»
  postoffice.yandex.ru закрыт, домен не резолвится; Yandex Cloud Postbox —
  сервис ОТПРАВКИ, не мониторинга, нам не нужен). Мониторинг доставки в
  Яндекс-ящики: (а) DMARC-отчёты Яндекса в наш dmarc@ (агрегаты по SPF/DKIM),
  (б) свои bounce/жалобы через IMAP + гейты, (в) смоук-письма на контрольные
  ящики Яндекса (инбокс/спам глазами), (г) open_rate из нашего пикселя.

Порядок: добавить все 11 активных доменов в оба кабинета сейчас (статистика
начнёт наполняться с первыми письмами прогрева); токен Mail.ru-постмастера —
в env боевого сервера для postoffice.py.
