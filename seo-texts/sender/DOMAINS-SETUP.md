...
(генерируется `tools/gen_redirects_nginx.py` из реестра; порядок ввода — в
шапке конфига: A-записи → conf.d → certbot webroot → включить 443). Подходит
любой свой сервер с белым IP (например, тот же, где живёт панель сендера).
