...
- **Почтовый офис Яндекса** postoffice.yandex.ru (ящики Яндекса — ~23% базы
  + свои корпы): войти под Яндекс-аккаунтом админа → указать домен. Если
  домен уже подтверждён в Яндекс Вебмастере этого аккаунта — доп-верификация
  не нужна; иначе DNS TXT. Внутри: прочитано/не прочитано/спам/удалено,
  время прочтения. API у сервиса есть (yandex.ru/dev/postoffice) — клиент
  в сендере пока только для Mail.ru, Яндекс — вручную (бэклог).
