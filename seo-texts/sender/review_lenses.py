...
def default_caller(prompt: str, max_tokens: int = 2000,
    ...
    # ВЫБОР МОДЕЛИ - решение владельца 16.08, и оно НЕ про цену: тарифы
    # opus-4-8 и fable-5 на роутере идентичны (замер контрольной парой:
    # $0.040392 против $0.040342 на одинаковой форме, ~$6/M вход, ~$30/M
    # выход). Основная - opus-4-8, потому что fable, по наблюдению владельца,
    # «часто придумывает то, чего нет» - а весь канон писем стоит на «факты
    # только из карточки», и склонность к выдумке здесь дороже любой силы
    # модели. Fable остаётся запасным на случай трёх подряд отказов опуса.
    # model=... — вызов проверки (судья, верификатор, линза). Письмо пишет
    # opus, а проверки читают готовый текст по списку правил и отвечают
    # «ok/не ok»: держать на этом самую дорогую модель незачем (владелец
    # 18.08 — «проверки, для них opus избыточен»). Умолчание не меняем:
    # без явной модели всё как было.
    # МОДЕЛЬ ЛИНЗЫ - РУБИЛЬНИКОМ, А НЕ ПРАВКОЙ КОДА. Владелец 25.08 по
    # журналу шлюза: линзы качества шли на опусе по $0.061 за вызов против
    # $0.007-0.015 у sonnet, вчетверо дороже. Линза читает готовое письмо
    # по списку правил и отвечает «ok/не ok» - то же решение, что 18.08
    # приняли по проверкам и по гейту адресата. Умолчание не трогаем: без
    # LENS_MODEL всё как было, чужие вызовы ведут себя по-прежнему.
    model = model or os.environ.get('LENS_MODEL') or 'claude-opus-4-8'
    # ЗАПАСНАЯ — НЕ ФАБЛ. Здесь стоял claude-fable-5, и это худший выбор из
    # возможных сразу по двум причинам. Он вдвое дороже опуса ($10/$50
    # против $5/$25), а главное — у него рассуждение НЕ ВЫКЛЮЧАЕТСЯ: запрет
    # thinking он не принимает и думает всегда. Мы платим за это по ставке
    # выхода. 19.08 в журнале шлюза это видно прямо: при сыпавшемся прогоне
    # фабловские вызовы шли по $0.10-0.16 с выходом 3700-5800 токенов и
    # пометкой «Рассуждение: high», тогда как наши обычные проверки стоят
    # $0.02-0.05 и помечены «отключено».
    # opus-4-7 — та же цена, что у основной, и запрет рассуждения работает.
    fallback = os.environ.get('LENS_FALLBACK_MODEL', 'claude-opus-4-7')
