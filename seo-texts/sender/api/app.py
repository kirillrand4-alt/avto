        ...
        for r in rows:
            ...
            if isinstance(panel, dict) and isinstance(panel.get("letter"), dict):
                ...
                try:
                    ...
                    # Тем же телом кормим и строку письма: по ней открывается
                    # окно «Править». Иначе редактор видит там служебную метку
                    # ИМЯ_ОТПРАВИТЕЛЯ (её ставит генератор, потому что имени
                    # ящика в момент генерации ещё нет) — для неё это выглядит
                    # как брак письма. В БД тело остаётся сырым: правка идёт по
                    # видимому тексту, а подстановка идемпотентна.
                    r["body"] = body
                except Exception:  # noqa: BLE001 - показ не роняем
                    pass
