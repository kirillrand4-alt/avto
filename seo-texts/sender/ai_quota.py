...
class AiQuota:
    ...
    def candidates(self, campaign_id: int, limit: int) -> list:
        ...
        # ГЕЙТ ПО РОДУ ДЕЯТЕЛЬНОСТИ - ПОСЛЕДНИМ И ОКНАМИ. Он один здесь стоит
        # денег и времени: две линзы через провайдера на каждую компанию без
        # кэшированного вердикта. Стоял он выше и судил ВЕСЬ запас, а запас -
        # это limit*10.
        #
        # Замер 17.08 на боевой панели: candidates(10, 14) шла 403 секунды,
        # то есть 140 компаний по ~2.9 с. Кнопка «Сгенерировать в очередь» при
        # этом выглядит сломанной - HTTP-запрос панели отваливается задолго до
        # конца, и владелец видит, что «ничего не началось».
        #
        # Судить весь запас незачем: отдаём мы limit штук. Сортировка по
        # накалу уже прошла, поэтому берём окно сверху, судим только его и
        # добираем следующим окном, если гейт много вырезал. Окно вдвое
        # больше нужного - с запасом на отсев. Вердикты кэшируются в базе
        # гейта, так что повторные нажатия дешевеют сами.
        готовые: list = []
        окно = max(1, limit * 2)
        сдвиг = 0
        while len(готовые) < limit and сдвиг < len(out):
            часть = out[сдвиг:сдвиг + окно]
            сдвиг += len(часть)
            не_покупатели = self._not_buyers([r for r in часть if r.inn])
            if не_покупатели:
                logger.info("target_gate: отсеяно %s адресатов (не покупатели)",
                            len(не_покупатели))
            готовые.extend(r for r in часть
                           if str(r.inn) not in не_покупатели)
        return готовые[:limit]
    ...
    def _izbytochnyy_zahod(self, campaign_id, доля: float = 0.34,
        ...
        СМОТРИМ НА ПОСЛЕДНИЕ ПИСЬМА, А НЕ НА ВСЮ ИСТОРИЮ. Первая редакция
        брала 200 штук, то есть всю кампанию целиком - а там лежит старая доля
        87%, накопленная до правки. Подсказка срабатывала бы ещё сотню писем
        подряд и после того, как заходы уже разошлись: квота 34%, а не ноль, и
        загонять форму в ноль так же неверно, как держать её на 87%
        (владелец 17.08: «в ноль то не надо прям резать»).

        Окно в 40 писем делает из этого петлю обратной связи: как только
        свежие письма разнообразны, доля падает ниже квоты и подсказка
        замолкает сама.
    ...
    def _request(self, r) -> dict:
        ...
        # Имя контакта из карточки: инициализируем ДО блока `if card`,
        # иначе при отсутствии карточки переменной не будет вовсе.
        _имя_из_карточки = ""
        ...
        if card:
            ...
            # ИМЯ ТОЖЕ БЕРЁМ ЗДЕСЬ. Замер 17.08 по 120 последним письмам
            # кампании 10: имя есть в карточке у 10 писем и НИ У ОДНОГО в
            # колонке recipients.contact_name - она пуста поголовно, имя
            # живёт только в обогащении. А генератор письма читает именно
            # колонку, и _request возвращал contact_name=None.
            #
            # Из-за этого именных приветствий не было и не могло быть: 73
            # письма прошли все заслоны надёжности имени, поздоровалась
            # модель в двух. Правка формулировки промпта («ОБЯЗАТЕЛЬНО
            # поздоровайся») работала вхолостую - ей нечего было называть.
            #
            # Контакт ищем по ТОМУ ЖЕ ящику, что и роль, одним проходом.
            if not extra.get("role") or not (r.contact_name or "").strip():
                email_l = (r.email or "").strip().lower()
                for c in (card.get("contacts") or {}).get("emails") or []:
                    if (c.get("email") or "").strip().lower() == email_l:
                        if c.get("role") and not extra.get("role"):
                            extra["role"] = c["role"]
                        if c.get("person"):
                            _имя_из_карточки = str(c["person"]).strip()
        ...
        # РОД ДЕЯТЕЛЬНОСТИ БЕЗ КАРТОЧКИ ОБОГАЩЕНИЯ. `activity` до сих пор
        ...
        return {"company_name": r.company_name, "okved": r.okved,
                ...
                "contact_name": (r.contact_name or "").strip()
                                or _имя_из_карточки or None,
                "mode": "auto", "extra": extra,
    ...
    def _not_buyers(self, recipients: list) -> set:
        ...
        ВМЕСТЕ С НЕЙ ОТДАЁМ ПАСПОРТ САЙТА. Одной строки гейту мало, и это не
        теория: разбор десяти снятых 20.08 показал, что примерно половина
        снята зря, и в паспорте той же компании лежало опровержение.
        ОКБ «ССП-ЕКБ» гейт назвал складской логистикой — в паспорте корпуса
        испарителя, шестерни, шнеки, изделия из чугуна. «Феррум-Трейд»
        назвал торговой компанией без производства — цитата с их сайта в
        паспорте: «занимается изготовлением металлоконструкций по чертежам
        заказчика». Строка activity лоссовая, паспорт снят с того же сайта
        и говорит о деле.
        """
        гейт = self._gate()
        ...
        for r in recipients:
            ...
            компании.append({
                ...
                "pasport": self._pasport_dlya_geyta(str(r.inn)),
            })
        try:
    ...
    # Поля паспорта, по которым видно ДЕЛО, а не вывеску. Ровно те же, что
    # кладёт в промпт письма _паспорт_кратко в ops/partiya_gen.py: если гейт
    # и генератор судят по разному, гейт снимает тех, кому письмо написалось
    # бы верно.
    _ПОЛЯ_ПАСПОРТА = ("цитата", "продукция", "оборудование_линии", "сырьё",
                      "масштаб", "мощности")

    def _pasport_dlya_geyta(self, inn: str) -> str:
        """Паспорт сайта одной строкой для гейта адресата. Нет — пусто."""
        try:
            д = self._site_facts(inn) or {}
        except Exception:  # noqa: BLE001 - гейт переживёт отсутствие паспорта
            return ""
        куски = []
        for k in self._ПОЛЯ_ПАСПОРТА:
            v = д.get(k)
            if not v:
                continue
            куски.append(f"{k}: " + (v if isinstance(v, str)
                                     else "; ".join(map(str, v))[:220]))
        return "\n".join(куски)[:900]
