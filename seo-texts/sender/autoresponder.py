...
# Задача #65: у ответа НЕТ своего подвала. Тело кончается «С уважением,» —
# ровно как AI-письмо первого касания, — а полную подпись (имя менеджера из
# ящика, «Компрессор Центр», ООО «Руспром» с ИНН) дописывает отправка тем же
# Sender._apply_signature. Один подвал — одно место сборки; раньше ответ уходил
# с обезличенным «Менеджер направления / prokompressor.ru», не похожим на
# подпись первого касания.
_SIGNATURE_TPL = 'С уважением,'
...
def qa_reply(subject: str, body: str) -> list[str]:
    ...
    # Финал письма — «С уважением,»: юр-байлайн «ООО «Руспром», ИНН» дописывает
    # отправка единой подписью (#65), в теле его больше нет — и не должно быть,
    # иначе он задвоится.
    if not body.rstrip().endswith('С уважением,'):
        problems.append('Финал не «С уважением,» — подпись отправки не приклеится.')
    if 'ООО «Руспром»' in body:
        problems.append('Байлайн «ООО «Руспром»» в теле — задвоится с подписью отправки.')
