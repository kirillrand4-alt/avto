...
class Orchestrator:
    ...
    def _division_queue_block(self, msg_in, campaign) -> "str | None":
        ...
        Тумблер «слать вне базы» здесь НАМЕРЕННО не действует: он про ручную
        отправку оператором (экран подтверждения + рубеж перед SMTP), а это
        автоматический планировщик. Ослаблять автопуть — отдельное решение
        владельца, и здесь ничего не умирает: письмо просто не ставится."""
        cards = self._cards
