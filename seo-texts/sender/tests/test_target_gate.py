...
# --- конкурент не покупатель, а арендодатель покупатель (26.08) ------------ #

def test_v_promte_gate_est_pravilo_pro_konkurenta():
    """Владелец 26.08: «уже второй раз конкурент проскакивает». Правила не
    было вовсе — «Энергоремкомплект» ответил «мы тоже занимаемся поршневыми
    компрессорами»."""
    from sender.target_gate import ПРОДАВЕЦ, СКЕПТИК
    for промпт in (ПРОДАВЕЦ, СКЕПТИК):
        низ = промпт.lower()
        assert "конкурент" in низ
        assert "дилер" in низ or "торгует нашим оборудованием" in низ
        assert "запчаст" in низ


def test_arenda_ostalas_pokupatelem():
    """Поправка владельца 19.08 сильнее нового правила: арендодатель
    компрессоры ПОКУПАЕТ, чтобы сдавать."""
    from sender.target_gate import ПРОДАВЕЦ, СКЕПТИК
    assert "АРЕНДА ОСТАЁТСЯ ПОКУПАТЕЛЕМ" in ПРОДАВЕЦ
    assert "Аренда сюда НЕ" in СКЕПТИК
    # старая формулировка про парк спецтехники на месте, её не затёрли
    assert "АРЕНДА И ПАРК СПЕЦТЕХНИКИ" in ПРОДАВЕЦ


def test_smezhnyy_tovar_ne_delaet_konkurentom():
    """НКТ, блок-контейнеры и муфты — другой рынок: на них суд 26.08
    ошибался, пока правило не назвали явно."""
    from sender.target_gate import ПРОДАВЕЦ
    низ = ПРОДАВЕЦ.lower()
    assert "насосно-компрессорные трубы" in низ
    assert "муфты" in низ
    assert "в собственном цеху" in низ
