...
KC_BODY = (
    ...
    # БЕЗ СЧЁТЧИКА КЕЙСОВ. Владелец 19.08: «убрать из первого письма
    # упоминания о кейсах опубликованных» — запрет стал безусловным, и
    # прежняя заготовка «78 опубликованных кейсов» валила гейт, а с ним
    # два теста про разделение направлений, к кейсам отношения не имевших.
    "Я подбираю компрессорное оборудование в Компрессор Центре.\n\n"
...
def test_tehlinza_delit_partiyu_po_napravleniyu():
    ...
    # Делим по голове ТЕХНОЛОГА: она у направлений разная и ничего чужого не
    # называет. Прежний признак — слово «фотосепаратор» — перестал делить
    # 28.08, когда линза покупателя научилась предлагать переставить письмо
    # на соседнее направление: чтобы его предложить, она обязана его назвать,
    # и «фотосепаратор» появился в компрессорном промпте тоже.
    мейеровских = [p for _, p in спрошено if "пищевого и перерабатывающего" in p]
    компрессорных = [p for _, p in спрошено
                     if "машиностроительного профиля" in p]
