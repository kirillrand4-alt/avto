...
MEYER_BODY = (
    ...
    "на таком объеме ручная переборка обычно перестает справляться.\n\n"
...
def test_generic_pismo_poluchaet_svoyu_mehaniku_zahoda():
    ...
    for б in блоки:
        ...
        assert "не начинать со «Смотрел профиль" in б
    ...
    мех = мблок.split("МЕХАНИКА ЗАХОДА для этого письма (только она):")[1]
...
def test_zahod_shtamp_v_pervoy_fraze_brakuetsya():
    """Просьбу в промпте модель игнорирует — нужен заслон в коде.

    12.08 запрет «не начинай со Смотрел профиль» добавили в промпт, выкатили
    и перегенерировали шесть писем: все шесть снова начались этими словами.
    Так же когда-то было с концовкой — починил только код, не просьба.
    """
    from sender.ai_letter import gate
    ...
    плохое = ("Добрый день!\n\nСмотрел профиль «Гладиум»: металлические двери, "
              ...
              "С уважением,")
    fails = gate("Вопрос по оборудованию", плохое, mode="GENERIC", division="kc")
    assert any("заход-штамп" in str(f) for f in fails), fails

    # Тот же оборот НЕ в первой фразе — законен, письмо не бракуем.
    ...
    assert not any("заход-штамп" in str(f) for f in ok), ok
