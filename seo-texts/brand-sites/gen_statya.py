...
RAUNDOV = 3
OBEM = (12000, 20000)          # знаков без тегов, как в эталонах копирайтера
...
- объём {OBEM[0]}-{OBEM[1]} знаков без тегов.
...
def proverit(html, sh, gazovaya):
    ...
    t = _tekst(html)
    p = []
    if not (OBEM[0] <= len(t) <= OBEM[1]):
        p.append(f'объём {len(t)} знаков, норма {OBEM[0]}-{OBEM[1]}')
