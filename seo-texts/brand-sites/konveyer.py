...
def main():
    ...
    # СДЕЛАНА ТА, У КОТОРОЙ ЛЕЖИТ ФАЙЛ, А НЕ ТА, ПРО КОТОРУЮ ЕСТЬ ЗАПИСЬ.
    #
    # Раньше здесь стоял просмотр ВСЕГО журнала: одна запись «чисто» или
    # «нужен разбор» за любой день навсегда убирала страницу из очереди.
    # Пока файлы только прибывают, разницы нет. Но как только страницу
    # ОТЗЫВАЮТ - а отзывать пришлось, доводка испортила четыре, - она
    # выпадает совсем: файла нет, а запись в журнале осталась, и конвейер
    # считает её готовой. Страница теряется навсегда и при этом выглядит
    # стоящей в очереди. Ровно та ловушка, о которой предупреждает шапка
    # otstavit_brak.py, только зашедшая через журнал, а не через файл.
    #
    # Поймано на berg и ekomak модульных азотных, enger--osushiteli,
    # zif--mks, remeza--osushiteli: очередь взяла 7 страниц вместо 9.
    #
    # Правило теперь опирается на диск, а не на историю: страница сделана,
    # если её ПОСЛЕДНИЙ вердикт годен И финальный файл лежит на месте.
    # Журнал только дополняет, решает наличие файла.
    posledniy = {}
    if os.path.exists(ZHURNAL):
        for s in open(ZHURNAL, encoding='utf-8'):
            try:
                z = json.loads(s)
                if z.get('slug'):
                    posledniy[z['slug']] = z.get('itog')
            except Exception:
                pass
    # «Нужен разбор» - это ЗАКОНЧЕННАЯ страница с пометкой, а не брак:
    # гейты её пропустили, спорят две линзы. Гонять её заново по всей
    # цепочке бессмысленно - второй прогон даст ту же пометку и второй
    # раз возьмёт деньги.
    est_fajl = {os.path.basename(p).split('.')[0]
                for p in _glob.glob(os.path.join(DIR, 'statyi-final', '*.html'))}
    sdelano = {s for s, itog in posledniy.items()
               if itog in ('чисто', 'нужен разбор') and s in est_fajl}
