...
Четыре снимка одной страницы: верх с оглавлением, первый призыв,
таблица, последний призыв. Ширина настольная.
...
def kadry(put, br, shirina=1280, vysota=900):
    """Четыре экрана, наведённые НА БЛОКИ, а не на доли высоты.
    ...
    try:
        ...
        # ЧЕТЫРЕ КАДРА, И ПЕРВЫЙ - ВЕРХ СТРАНИЦЫ.
        #
        # Наведя первый кадр на призыв, я выронил оглавление: оно стоит
        # ВЫШЕ первого призыва. Осмотр 52 раза написал «оглавление
        # отсутствует» - и снова был прав относительно того, что ему
        # показали. Починив одну слепую зону, я сделал другую.
        #
        # Спрашиваем про четыре вещи - показываем четыре: верх с
        # оглавлением, первый призыв, таблицу, последний призыв.
        celi = [None, 'p.cta', '.tablica-prokrutka', 'p.cta:last-of-type']
        zapas = [0, .35, .6, .92]
        vsego = st.evaluate('() => document.body.scrollHeight')
        for i, sel in enumerate(celi):
            nashli = False if sel is None else st.evaluate(f"""() => {{
