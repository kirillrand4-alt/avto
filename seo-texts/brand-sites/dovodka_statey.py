...
def pochemu_nelzya(citata, zamena, html, tronuto, sh=None):
    ...
    # КОЭФФИЦИЕНТ ПОДБОРА ПРАВКЕ НЕ ПОДЛЕЖИТ.
    #
    # 0,05 / 0,06 / 0,07 м³/мин подачи компрессора на 1 м³/ч азота - наша
    # земля, выведенная из реальных пар «генератор + компрессор» в прайсе.
    # Линза teh_razmernost прочитала это как отношение около 1:1, объявила
    # физически неверным и заменила 0,06 на 6, не переведя час в минуты:
    # 1 м³/ч это 0,0167 м³/мин, и настоящее отношение здесь 3,6 к одному.
    #
    # Общее правило «новое число должно быть в задании» такую правку
    # пропустило: шестёрка в задании есть, она встречается там десятками.
    # Поэтому здесь нужен именно ИМЕННОЙ запрет, а не сверка со списком.
    #
    # Ценой этого стали две страницы: ekomak--azotnaya-stanciya-modulnaya
    # с «коэффициентом 6» и berg--azotnaya-stanciya-modulnaya, где проза
    # получила «5, 6, 7 м³/ч», а таблица сохранила 0,06 и 0,07 - три
    # системы единиц в одной колонке.
    if sanity._KOEF_RYADOM.search(citata) and not sanity._KOEF_RYADOM.search(zamena):
        return 'правка трогает коэффициент подбора под генератор азота'
    if sanity._KOEF_RYADOM.search(citata):
        bylo_k = [sanity._ch60(m.group(1)) for m in sanity._KOEF_RYADOM.finditer(citata)]
        stalo_k = [sanity._ch60(m.group(1)) for m in sanity._KOEF_RYADOM.finditer(zamena)]
        if bylo_k != stalo_k:
            return ('правка меняет коэффициент подбора '
                    f'{bylo_k} -> {stalo_k}, а он у нас известен')
