...
# --- числа --------------------------------------------------------------
#
# Первая версия этой проверки пометила 86 ТЗ из 87 и оказалась бесполезной:
# в шуме тонули настоящие находки. Ругалась она на законное - на «78% азота
# в воздухе» (физическая константа, правилами разрешена), на ступени чистоты
# «95%, 99,9%, 99,999%» (нормативный ряд), на «64% позиций с ресивером»
# (посчитано из payload, просто не лежит там готовым числом) и даже
# на порог 40%, который ТЗ цитирует из моей же инструкции.
#
# Ловить надо не всякое число, а ОДИН сорт - сравнительные и экономические
# утверждения, которые запрещены правилами (сорта 4 и 6): «тише на 2-4 дБА»,
# «КПД выше на 3-5%», «дороже на 15-20%», «экономия до 35%», «окупается
# за 2 года». У них нет источника и быть не может: наши данные таких величин
# не содержат, а инженер заказчика такую страницу прочитает.

# Число становится уликой, только если рядом стоит слово сравнения или денег.
SRAVNENIE = re.compile(
    r'(тише|громче|выше|ниже|больше|меньше|дешевле|дороже|экономи\w*|'
    r'окупа\w*|срок\w* службы|ресурс|КПД|производительн\w* выше|'
    r'на \d+[.,]?\d*\s*%|в \d+[.,]?\d*\s*раза?)', re.I)
# Строки, где число законно даже рядом со сравнением.
ZAKONNO = re.compile(
    r'знак|символ|ISO|ГОСТ|СанПиН|СП |ТР ТС|раздел|пункт|8573|15150|'
    r'запрещ|нельзя|не писать|не указыв|брак|пример недопуст|'
    r'порог|пересечен|скелет|блок\w* H2|'
    r'янв|фев|мар|апр|мая|июн|июл|авг|сен|окт|ноя|дек', re.I)
# Проценты чистоты газа - нормативный ряд, не сочинённая цифра.
CHISTOTA = re.compile(r'9[0-9][.,]?\d*\s*%|99[.,]9+\s*%|2[01]\s*%|78\s*%')


def levye_chisla(text, znaem):
    """Сравнительные и экономические числа без источника.

    znaem оставлен в сигнатуре: число из payload не улика, даже если стоит
    рядом со словом сравнения («на 203 позиции больше» - законно).
    """
    # Раздел 10 - это список «что нельзя писать», он ЦИТИРУЕТ запрещённые
    # формулировки целиком: «доли рынка, средние сроки окупаемости, у 30%
    # предприятий». Сканировать его значит ругаться на правильно написанный
    # запрет. Раздел 5 - лексика, там проценты про баланс текста, не про технику.
    m = re.search(r'^#{1,3}\s*10\.', text, re.M)
    text = text[:m.start()] if m else text
    m5 = re.search(r'^#{1,3}\s*5\..*?$(.*?)(?=^#{1,3}\s*6\.)', text, re.M | re.S)
    if m5:
        text = text[:m5.start(1)] + text[m5.end(1):]
    bad = {}
    for line in text.splitlines():
        if ZAKONNO.search(line) or not SRAVNENIE.search(line):
            continue
        for m in CHISLO.finditer(line):
            hvost = line[m.end():m.end() + 12]
            if not EDINICY.match(hvost):
                continue
            s = m.group(1).replace(' ', '').replace(' ', '')
            if s in znaem or len(s) < 2:
                continue
            ed = EDINICY.match(hvost).group(1)
            if ed == '%' and CHISTOTA.search(line[max(0, m.start() - 2):m.end() + 2]):
                continue
