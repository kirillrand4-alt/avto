...
def main():
    ...
    for gaz, po_pod in po_gazu.items():
        ...
        if not otn:
            continue
        # РАЗМЕН, О КОТОРОМ НАДО ПОМНИТЬ: от базы 98% ступени
        # подтверждались 13-18 значениями подачи, от 99% - только 5-8,
        # потому что позиция ровно на 99% есть не у всякой подачи.
        # Числа тоже другие: 98% -> 99,999% давало x2,58, от 99% выходит
        # x2,26. Оба верны, просто это разные утверждения. Единая база
        # на странице важнее лишних наблюдений: страница с двумя разными
        # базами уже один раз родила подлог, а тонкая база лечится
        # печатью разброса рядом с величиной.
        #
