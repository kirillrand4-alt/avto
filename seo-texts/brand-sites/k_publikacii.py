...
def _zagolovki_k_publikacii(html):
    ...
    def oglavlenie(m_):
        ...
        # ЯКОРНЫЕ ССЫЛКИ СПИСКОМ, А НЕ СТРОКОЙ ЧЕРЕЗ ЗАПЯТУЮ.
        #
        # Замечание владельца по снимку: «не очень удобные якорные ссылки».
        # И правда: девятнадцать заголовков, слитых в один абзац запятыми,
        # глазом не сканируются - непонятно, где кончается пункт, а на
        # телефоне это два экрана сплошного подчёркнутого текста.
        #
        # Списком нельзя тегом ul: вёрстка сайтов рисует у него свои
        # тире-маркеры и ломается. Поэтому разделители убираем совсем,
        # а каждую ссылку делает строкой CSS (display:block), и он же
        # раскладывает их в две колонки на широком экране.
        blok = re.sub(r'</a>\s*[;,]\s*(?=<a )', '</a>', blok)
        blok = re.sub(r'</a>\s*[;,.]\s*(?=</p>)', '</a>', blok)
        blok = re.sub(r'</a>\s*<br>\s*(?=<a )', '</a>', blok)
...
def podgotovit(html, domen):
    ...
    # ДВЕ КНОПКИ ПОДРЯД - ЭТО ОДНА КНОПКА СЛИШКОМ.
    #
    # На первом экране задание ставит ДВЕ дорожки заявки: «получить КП»
    # и «рассчитать станцию». Обе настоящие и обе нужны, но собранные
    # в столбик они дают две серые плашки с двумя кнопками одна под
    # другой - читается как задвоение, а не как выбор. Замечание владельца
    # по снимку: «на фото 2 кнопки подряд».
    #
    # Сливаем пару в ОДНУ плашку: оба предложения текстом, обе кнопки
    # в один ряд под ними. Ни одного слова не теряется, а выбор становится
    # виден как выбор.
    # СЛИВАЕМ ПАРУ, А НЕ ЦЕПОЧКУ. Первый вариант крутил замену в цикле,
    # и после слияния пары результат снова оказывался рядом со следующей:
    # шесть призывов схлопнулись в одну плашку-простыню на шесть фраз.
    # Поэтому один проход и потолок на длину: две фразы читаются, четыре
    # уже нет.
    POTOLOK_ZNAKOV = 420

    def slit_paru(m_):
        a, ka, b, kb = m_.group(1), m_.group(2), m_.group(3), m_.group(4)
        chisto = re.sub(r'<[^>]+>', '', a + ' ' + b)
        if len(chisto) > POTOLOK_ZNAKOV:
            # Слить нельзя - выйдет простыня. Но и две кнопки столбиком
            # оставлять нельзя, замечание владельца именно про это.
            # Тогда вторая плашка опускается до обычного абзаца: текст
            # призыва на месте, а кнопка у пары одна.
            return f'<p class="cta">{a}</p>\n<p class="cta-knopka">{ka}</p>\n<p>{b}</p>'
        # Первая фраза часто идёт без точки на конце, и при слиянии
        # получалось «...в течение часа Рассчитать станцию под задачу».
        # Ставим точку, если её нет.
        levo = a.rstrip()
        if not levo.endswith(('.', '!', '?', ':', ';')):
            levo += '.'
        return (f'<p class="cta">{levo} {b}</p>\n'
                f'<p class="cta-knopka">{ka} {kb}</p>')

    # ГРУППА НЕ ИМЕЕТ ПРАВА ПЕРЕСЕЧЬ КОНЕЦ АБЗАЦА.
    #
    # С обычным «.*?» и DOTALL шаблон при неудаче расширял первую группу
    # дальше своего </p> и находил «пару» через полстраницы, сметая всё
    # между ними в один абзац. Текст при этом уцелевал - и потому подвох
    # не виден по объёму, - а плашки и кнопки исчезали: 423 кнопки стали
    # 256. Ловится только счётом кнопок, а не счётом знаков.
    #
    # «(?:(?!</p>).)*» - это «что угодно, но не доходя до </p>».
    NE_DO_KONCA = r'((?:(?!</p>).)*)'
    html = re.sub(
        r'<p class="cta">' + NE_DO_KONCA + r'</p>\s*'
        r'<p class="cta-knopka">' + NE_DO_KONCA + r'</p>\s*'
        r'<p class="cta">' + NE_DO_KONCA + r'</p>\s*'
