...
комбинат» с «Комбинатом хлебопродуктов». Поэтому:
  * запрос идёт ТОЛЬКО ИМЕНЕМ, а регион служит ФИЛЬТРОМ по адресу найденных карточек.
    Дописывать регион в строку поиска нельзя: проверено, «Магнитогорский металлургический
    комбинат Челябинская область» даёт НОЛЬ карточек — и это поймал положительный контроль,
    иначе прогон отчитался бы «таких предприятий в России нет»;
  * карточка принимается, только если после отсева она осталась ОДНА;
  * если в регионе несколько, но имя совпадает знак в знак ровно с одной — берём её
    («Северсталь»: три карточки в Вологодской области, точное имя у одной);
  * если в регионе НЕТ ни одной, но в стране имя совпадает знак в знак ровно с одной —
    берём и помечаем «регистрация в другом регионе». Это обычный федеральный заказчик:
    ОАО «РЖД» строит в Амурской области, а зарегистрировано в Москве. На пробе из 20 строк
    это правило подняло разрешение с 6 до 11;
