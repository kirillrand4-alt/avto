...
import glob as _g
FAYLY = sorted(_g.glob(os.path.join(D, 'PARK-EIS-GOD-*-3S.jsonl')))


def _kartochka(n):
    """11 знаков — 223-ФЗ, 19 — 44-ФЗ. Проверено на 12 живых ссылках: номер и предмет
    видны на странице, капчи нет."""
    if len(n) == 11:
...
inny, novye = set(), set()
if not FAYLY:
    raise SystemExit('нет файлов PARK-EIS-GOD-*-3S.jsonl')
bylo_inn = {r[0] for r in cur.execute('select distinct inn from fakt where v_parke=1')}
for _f in FAYLY:
  for ln in open(_f, encoding='utf-8', errors='replace'):
      if not ln.strip():
          continue
      vs += 1
      try:
      ...
      # у 3-й в потоке ссылка — поиск по номеру; карточку строим сами,
      # форма проверена на 12 живых ссылках (номер и предмет видны на странице)
      url = _kartochka(nomer) if nomer else ''
      ...
      cur.execute(
          ...
          (inn, (r.get('org_imya') or r.get('zakazchik_iz_lenty') or '')[:200], tip,
           ...
           '3-я сессия, ЕИС годовым срезом; принято 1-й', '', dedup,
...
cur.execute('insert into zhurnal_vlivaniya values (?,?,?,?,?,?)',
            (time.strftime('%Y-%m-%d %H:%M:%S'), 'ПОТОК 3-й СЕССИИ: ЕИС годовые срезы',
...
for r in cur.execute("""select tip, vid_fakta, count(*), count(distinct inn) from fakt
   where kto like '%3-я сессия, ЕИС годовым срезом%' group by 1,2 order by 3 desc limit 12"""):
