...
BAZA = r'C:\sender'
OUT = os.path.join(BAZA, 'park_brendy.jsonl')
PROG = os.path.join(BAZA, 'park_brendy_progress.json')
OTVAL = os.path.join(BAZA, 'park_brendy_otval.jsonl')   # урок 2-й: счётчик выхода
                                                     # не знает про отвал
...
ZAPROSY = json.load(open(os.path.join(BAZA, 'park_brendy_zaprosy.json'), encoding='utf-8'))
...
GAZ = re.compile(
    ...
    r'ресивер|осушител|(винтов\w{0,4}блок)|компрессорн')
# чужое под теми же словами: медицина, стоматология, быт, авто, садовое
NE_RASHOD = re.compile(
    r'стоматолог|аквариум|(медицинск\w{0,4}компрессор)|(компрессор\w{0,4}(матрас|ингалятор|'
    r'небулайзер))|(садов\w{0,4}воздуходувк)|ранцев|(бытов\w{0,4}компрессор)|'
    r'(автомобильн\w{0,4}компрессор)|(холодильн\w{0,4}(витрин|ларь|шкаф))|'
    r'(компрессор\w{0,4}холодильник\w{0,4}бытов)')
