...
MODELI_1 = [('Atlas Copco ZH', r'\bZH[\s-]?\d{2,4}\b', False),
            ...
            # ЦК — по прямому решению владельца. На вопрос «есть ли ещё центробежники, кроме
            # ТВ» я нашёл шесть серий: Н-370-18-1, ЦБН/НЦ, PCL, ГТК, ТГ и ЦК. Владелец
            # посмотрел и ответил: «оставляй только там, где есть ЦК в названии, всё
            # остальное это для транспортировки природного газа». Так и сделано: серии
            # магистральных газоперекачивающих агрегатов (их держат Газпром трансгаз и
            # Газпром добыча) в покраску не идут, ЦК идёт.
            #   «Компрессор ЦК 135/8, зав. № 83015, поз. 2/1 цех 25 корпус 138»
            #   «Капитальный ремонт компрессора ЦЕНТРОБЕЖНОГО позиция ЦК-135/8»
            #   «пуска компрессора ЦК-301 (с заменой ЧРП)» — НОВАТЭК-Усть-Луга
            ('ЦК', r'\bЦК[\s-]?\d{2,3}\b', True)]
...
_RYADOM = _re.compile(r'центробежн|турбокомпресс|воздуходув|газодожимн|atlas\s*copco'
                      r'|нагнетател|газодувк|ingersoll|cameron|samsung|hanwha|hanwa|centac|turbo|pcl',
                      _re.I)
_VINT = _re.compile(r'винтов', _re.I)
# БЫТОВОЙ КОМПРЕССОР С ТЕМ ЖЕ НОМЕРОМ. У «Пармы» и «Сибртеха» модель пишется как
# «К-1500/24ВМ», где 24 — литры ресивера, и мой поиск К-1500 ловил их как центробежные:
# у Ростелекома это оказалась «Пластина клапанная для компрессора Парма К-1500/24ВМ», а у
# другого предприятия — «Компрессор ПАРМА К-1500/24БМ безмасляный ДЛЯ НАКАЧКИ ШИН».
# Настоящая центробежная машина пишется индексом через дефис: К-250-61-5, К-1500-62-1.
# Отсекаем по двум признакам сразу: дробь с числом сразу после номера и имя бытовой марки.
_BYTOVOY = _re.compile(r'парма|сибртех|fubag|patriot|elitech|накачк\w* шин|для шин'
                       r'|ресивер\w*\s*\d{2}\s*л\b', _re.I)


def _bytovoy_odnofamilec(tekst, m):
    """True, если совпадение — бытовой компрессор, а не центробежная машина.

    Дробь после номера («К-1500/24ВМ» — 24 литра ресивера) подозрительна только у К-моделей
    и только когда рядом НЕ сказано «центробежный»: у ЦТК дробь законна, владелец сам
    предупредил — «ЦТК-275, иногда встречается как ЦТК-275/9». Первый заслон был грубее и
    съел настоящую ЦТК-275/9 у Евразруды.
    """
    okno = tekst[max(0, m.start() - 110):m.end() + 110]
    if _BYTOVOY.search(okno):
        return True
    if m.group(0).upper().lstrip().startswith('ЦТК'):
        return False
    if _re.match(r'\s*/\s*\d', tekst[m.end():m.end() + 8]):
        return not _re.search(r'центробежн|турбокомпресс', okno, _re.I)
    return False
...
for _inn, _kuski in _teksty.items():
    ...
    # Склейка НЕ обрезается: обрезка по 20 000 знаков теряла модель у крупных предприятий —
    # у Газпрома, Галополимера и ФГУП «ЦК» стояло за обрезом, и они оставались непокрашенными
    # при том, что модель в базе есть. Держим до 400 000 знаков: это верх по самому
    # многословному предприятию, а не круглое число наугад.
    _tekst = ' '.join(_kuski)[:400000].replace('K', 'К')
    _nayd = []
    for _imya, _r in _2:
        for _m in _r.finditer(_tekst):
            if not _bytovoy_odnofamilec(_tekst, _m):
                _nayd.append(_imya)
                break
