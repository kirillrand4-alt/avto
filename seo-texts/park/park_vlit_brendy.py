# -*- coding: utf-8 -*-
"""Приём закупок ПО СЛОВАРЮ БРЕНДОВ в park.db.

Бренды взяты с сайта владельца (prokompressor.ru) и дополнены словарём по десяти
сегментам: винтовые импорт и РФ, поршневые, центробежные, ПКС, МКС, азот, кислород,
ВРУ, воздуходувки — 1 032 бренда, 7 987 написаний, 7 636 серий. Сегменты азота,
кислорода и ВРУ на сайте владельца не представлены, их закрывает словарь.
...
FAYL = os.path.join(D, 'park_brendy_inn.jsonl')
...
for ln in open(FAYL, encoding='utf-8', errors='replace'):
    ...
    # САДОВЫЙ ИНСТРУМЕНТ ПОД НАШИМ СЛОВОМ («воздуходувка ранцевая бензиновая»)
    # — не наша машина. Правило общее и лежит в park_build, чтобы не разъезжалось
    # по вливалкам: раньше оно стояло только в приёме чужого потока.
    ...
    cur.execute(
        ...
        (inn, (r.get('org_imya') or r.get('zakazchik_iz_lenty') or '')[:200], tip,
         ...
         '', '', '', '', '', (r.get('summa') or ''), '', '',
         2, 'E: закупка по бренду, класс машины отсюда не следует', predmet[:500],
         'запрос по марке/бренду в ЕИС; ИНН снят с карточки закупки', 'vysokaya',
         '1-я сессия, запрос по бренду (ЕИС)', '', dedup,
        ...
        if len(tel) == 10:
            cur.execute('insert or ignore into contact_source(inn,vid,znachenie,person,dolzhnost,'
                        ...
                        (inn, 'telefon', tel, fio[:200], 'контактное лицо закупки', raz[1],
                         ...
                         '1-я сессия, контакт с карточки ЕИС (запросы по брендам)'))
            kt += 1
        if '@' in mail:
            cur.execute('insert or ignore into contact_source(inn,vid,znachenie,person,dolzhnost,'
                        'istochnik,source_url,domen,pervoistochnik,data_nablyudeniya,quote,kto) '
...
cur.execute('insert into zhurnal_vlivaniya values (?,?,?,?,?,?)',
            (time.strftime('%Y-%m-%d %H:%M:%S'), 'СЛОВАРЬ БРЕНДОВ: закупки ЕИС с ИНН',
...
for r in cur.execute("""select tip, vid_fakta, count(*), count(distinct inn) from fakt
   where kto like '%запрос по бренду%' group by 1,2 order by 3 desc limit 12"""):
