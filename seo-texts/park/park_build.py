...
_POSREDNIK = re.compile(
    r'агентств\w*\s+государственн\w*\s+заказ|комитет\w*\s+по\s+регулированию|'
    r'управлени\w*\s+по\s+закупкам|центр\w*\s+организации\s+закупок|дирекци\w*\s+закупок|'
    r'министерств\w*\s+по\s+регулированию\s+контрактной\s+системы|'
    r'департамент\w*\s+государственн\w*\s+заказа', re.I)
