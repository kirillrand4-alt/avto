...
_PRINCIP = [(r'винтов', 'винтовой'), (r'поршнев', 'поршневой'),
...
_POSREDNIK = re.compile(
    r'^администраци|^адм\b|'
    r'агентств\w*\s+(государственн\w*\s+заказ|по\s+госзаказ)|'
    r'комитет\w*\s+по\s+(регулированию|закупк)|'
    r'(управлени|департамент)\w*\s+(по\s+)?(закупк|государственн\w*\s+заказ)|'
    r'центр\w*\s+(организации\s+)?закупок|центр\w*\s+управления\s+закупк|'
    r'дирекци\w*\s+(по\s+)?закупок|центр\w*\s+централизованных\s+закупок|'
    r'министерств\w*\s+по\s+регулированию', re.I)


def posrednik(nazvanie):
    """Заказчик — уполномоченный орган, а не эксплуатант машины.

    «Министерство по регулированию контрактной системы в сфере закупок Воронежской области»
    объявляет закупку компрессоров ДЛЯ ДРУГОГО учреждения: ИНН в карточке его, а машина
    встанет у того, для кого закупают. Такой ИНН сел бы в парк вместо настоящего владельца,
    и продавец звонил бы в министерство.

    Заслон был написан один раз в вливалке одного потока (`park_vlit_3s_eis_tik3.py`) и в
    остальные не попал — та же история, что с садовыми воздуходувками. Здесь он один на всех.

    Список расширен по находке 10.08: всё, что влилось ДО появления заслона, принесло в парк
    75 уполномоченных органов — департаменты и агентства госзаказа, региональные центры
    закупок, 20 администраций, «ИНТЕР РАО-Центр управления закупками». Проверено, что это не
    эксплуатанты: надзорная запись ЭПБ выдаётся именно эксплуатанту, и НИ У ОДНОГО из 75 её
    нет — все держатся только на закупках.
    """
    return bool(_POSREDNIK.search(nazvanie or ''))


def razobrat_predmet(predmet):
    """Предмет закупки -> (тип машины, вид факта, принцип) или (None, None, None).

    Правило жило в `park_vlit_3s_eis.py` и было скопировано бы в каждую новую вливалку
    чужого потока — ровно так однажды разъехался заслон садового инструмента. Теперь оно
    здесь одно, и все вливалки зовут его отсюда.
    """
    n = re.sub(r'\s+', '', (predmet or '').lower().replace('ё', 'е'))
    tip = next((k for rx, k in _KANON if re.search(rx, n)), None)
    if not tip:
        return None, None, None
    princip = next((k for rx, k in _PRINCIP if re.search(rx, n)), '')
...
def kartochka_zakupki(nomer):
    """Реестровый номер -> адрес КАРТОЧКИ закупки (не поиска).

    11 знаков — 223-ФЗ, 19 — 44-ФЗ. Форма проверена глазами на живых ссылках: номер и
    предмет видны на странице. Поисковая форма `extendedsearch/results.html?searchString=`
    выдачу рисует скриптом — ни ИНН, ни предмета в теле нет, доказательством не является.
    """
    n = re.sub(r'\D', '', nomer or '')
    if len(n) == 11:
...
_NOMER_V_POISKE = re.compile(r'searchString=(\d{11}|\d{19})(?:&|$)')


def nomer_iz_poiska(u):
    """Номер закупки из поисковой ссылки 3-й сессии — чтобы построить по нему карточку."""
    m = _NOMER_V_POISKE.search(u or '')
