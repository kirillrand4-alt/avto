...
def stroka_cheloveka(stroki, chel, chuzhie=()):
    ...
    for s in godny:
        ...
        if chasti[1:] and all(c.lower() in nizh for c in chasti[1:] if len(c) > 2):
            ...
            tochnye.append(s)
    # Полосу считаем у КАЖДОЙ подходящей строки и берём самую содержательную.
    # По длине СТРОКИ выбирать нельзя ни в ту, ни в другую сторону: самая длинная —
    # это слипшаяся страница, а самая короткая оказалась пунктом меню, откуда пришла
    # полоса «Безруких» — одна фамилия без имени и без должности. Полоса же обрезана
    # чужим именем, поэтому длинная полоса безопасна и просто содержит больше.
    polosy = [p for p in (polosa_v_stroke(s, fam, chuzhie) for s in (tochnye or godny)) if p]
    if not polosy:
        return '', 'строки человека на странице нет'
    return max(polosy, key=len), ''


# Работодатель, НАЗВАННЫЙ в полосе. Без этого «отвязать» нечем: строка
# «Ларёв Павел Николаевич, главный инженер – первый заместитель генерального директора»
# не называет предприятия вовсе, и отсутствие в ней нашего имени ничего не доказывает.
RABOTODATEL = re.compile(
    # Границы слова обязательны: без них «АНО» находилось внутри фамилии «Ив-ано-в»,
    # и человек без места работы получал работодателя из собственного имени.
    r'\b(?:ООО|ОАО|ЗАО|АО|ПАО|НАО|ГУП|МУП|ФГУП|АНО|НКО|ФКУ|ФБУ|ГБУ|МБУ|МАУ|'
    r'концерн|холдинг|корпораци\w*|группа\s+компаний)\b\s*[«"“]?[^»"”,;]{2,60}|'
    r'[«"“][^»"”]{2,60}[»"”]|'
    r'\b(?:завод\w*|комбинат\w*|фабрик\w*|шахт\w*|рудник\w*|карьер\w*|институт\w*|'
    r'университет\w*|академи\w*|техникум\w*|колледж\w*|управлени\w*|департамент\w*|'
    r'министерств\w*|администраци\w*|дворец\w*|дом\w*(?:\s+\w+){0,2}\s+'
    r'(?:культур\w*|творчеств\w*|техник\w*|молод[её]жи)|центр\w*|'
    r'школ\w*|больниц\w*|поликлиник\w*|музе\w*|библиотек\w*|театр\w*|'
    r'филиал\w*|представительств\w*)\b[^,;.]{0,40}', re.I)


def nazvan_rabotodatel(polosa):
    """Что названо местом работы в полосе. Пусто — значит не названо ничего."""
    for m in RABOTODATEL.finditer(str(polosa or '')):
        kusok = m.group(0).strip(' \t«»"“”,;:-–—')
        if len(kusok) >= 4:
            return kusok[:90]
    return ''
    ...
    for c in celi:
        ...
        if not h or h.startswith('__ОШИБКА__'):
            ...
            if beda:
                ...
                gde_rabotaet = nazvan_rabotodatel(polosa)
                if not gde_rabotaet:
                    # В полосе не назван НИКАКОЙ работодатель. Тогда отсутствие в ней
                    # нашего имени не улика, и привязка остаётся спорной.
                    itog = 'в полосе человека место работы не названо'
                    sch[itog] += 1
                else:
