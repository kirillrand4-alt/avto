...
TEG = re.compile(r'<[^>]+>')
# СТРОЧНЫЕ ТЕГИ РВУТ СЛОВА. ЕИС подсвечивает найденный кусок предмета своим тегом:
# «...станц<span class="...">ия</span> для заправки кислородн<span>ых</span> баллонов».
# Замена ЛЮБОГО тега пробелом даёт «станц ия», «кислородн ых», «компрессор а поршнев ого» —
# и дальше рвётся всё, что смотрит на слово целиком: подтверждение словом, словарь видов,
# заслон вливания.
#
# РАЗМЕР БЕДЫ ИЗМЕРЕН, А НЕ ОЦЕНЁН НА ГЛАЗ. Признак: слово машины НЕ находится в предмете,
# но находится, если убрать все пробелы.
#     PARK-EIS-TIK19  11 строк из 1 745      PARK-EIS-TIK18  0 из 2 009
#     park_ingest_3d   9 строк из 3 266
# Примеры: «Запасные части для кислород ной станции Novair Premium 270 HF»,
# «Поставку запасных частей для кислород ного генератор а MZ630».
#
# ЧЕСТНАЯ ПОПРАВКА К СЕБЕ: сперва я записала сюда, что из-за этого в парк не вошли два
# предприятия, — и это НЕ подтвердилось при проверке. У «Поставка компрессор а поршнев ого»
# заказчик — «управление по закупкам администрации», его отсекает заслон посредника, и
# правильно. У «монтаж станц ии для заправки кислородн ых баллонов» слово машины ищется
# образцом `кислородн\w*станц`, то есть требует СОСЕДСТВА слов, а здесь «станции» стоит
# ПЕРЕД «кислородных» — склейка тегов этого не чинит. Итог: дефект настоящий и измеренный,
# но те две строки не вошли по другим причинам, и одна из них — узость образца видов.
#
# Строчные теги убираю БЕЗ пробела, блочные — с пробелом.
TEG_STROCHNYY = re.compile(r'</?(span|b|i|em|strong|mark|u|small|sub|sup|a|font)\b[^>]*>',
                           re.I)


def bez_tegov(s):
    """Видимый текст: строчные теги склеивают слово, прочие разделяют."""
    s = TEG_STROCHNYY.sub('', s or '')
    return re.sub(r'\s+', ' ', TEG.sub(' ', s)).strip()
...
def adres(slovo, stranica):
    return ('https://zakupki.gov.ru/epz/order/extendedsearch/results.html'
            ...
            # РАЗМЕР СТРАНИЦЫ. Срез «компрессор» за 2022: счётчик ЕИС 4 900, а разобрано
            # 200 при двадцати страницах — то есть на странице ДЕСЯТЬ карточек, и весь
            # мой постраничный обход был впятеро мельче, чем я считала. Проверила
            # четыре формы на одном слове: `recordsPerPage=50` даёт 50 карточек, слово
            # в тексте остаётся, первые номера те же — фильтр не сломался. `pageSize=50`
            # не действует. Прирост ровно в пять раз.
            + ('?fz44=on&fz223=on&searchString=%s%s&recordsPerPage=50&pageNumber=%d'
...
for slovo in SLOVA:
    ...
    for st in range(1, STRANIC + 1):
        ...
        if slovo not in schetchiki:
            tx = bez_tegov(re.sub(r'<script.*?</script>', ' ', html,
                                  flags=re.S | re.I))
            m = SCHET.search(tx)
...
# СЧЁТЧИК СТОИТ ПОСЛЕ ЗАСЛОНА, а не до него. Прежняя строка брала ВЕСЬ поток, включая
# извещения, где слово запроса нигде не подтвердилось, и печатала «новых ИНН 319» при
# «с ИНН 224» — новых больше, чем всего с ИНН, то есть счётчик считал не то, что подписано.
# Проверка вручную по тому же файлу: разных ИНН 415, с машиной В ПРЕДМЕТЕ 178, а новых для
# парка после отсева непрофильных учреждений — 60. Это и есть добыча.
_nov_syro = {o['inn'] for o in potok if o.get('inn_novyy')}
_nov = {o['inn'] for o in podtv if o.get('inn_novyy') and o.get('inn')}
print('  НОВЫХ ИНН, где слово подтверждено %5d  (без заслона было бы %d — это не добыча)'
      % (len(_nov), len(_nov_syro)))
