...
def main():
    ...
    for r in out:
        ...
        if len(c) >= 10:
            po_nomeru[c[-10:]].add(r['inn'])
    obshchie = {k for k, v in po_nomeru.items() if len(v) > 1}
    snyato = 0
    for r in out:
        c = re.sub(r'\D', '', r.get('lichnyy_nomer') or '')
        if len(c) >= 10 and c[-10:] in obshchie:
            r['nomera_predpriyatiya'] = ((r.get('nomera_predpriyatiya') or '') + ' | '
                                         + r['lichnyy_nomer']).strip(' |')[:70]
            r['vid_lichnogo'] = 'номер у нескольких предприятий — не личный'
            r['lichnyy_nomer'] = ''
            snyato += 1
    if snyato:
        print(f'  АУДИТ: номеров у нескольких предприятий {len(obshchie)}, снято строк {snyato}')
