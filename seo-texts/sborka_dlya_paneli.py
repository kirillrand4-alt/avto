...
SADOVAYA = re.compile(
    ...
    r'мойк\w* высокого давления|высоторез|мотопомп|'
    # Вторая половина признака добавлена 04.08 по карточке владельца: «Воздуходувка
    # АККУМУЛЯТОРНАЯ Denzel RB180-36, 36В» проходила прежний заслон насквозь — в тексте нет
    # ни бензина, ни триммера. Промышленная воздуходувка не бывает аккумуляторной, ранцевой,
    # переносной или «Li-Ion 6000mAh».
    r'аккумулятор\w*|переносн\w*|ручн(?:ая|ой|ые|ого)\b|бесщ[её]точн\w*|li-?ion|мач\b|'
    r'denzel|greenworks|ryobi|einhell|sturm|\bhammer\b|кратон|калибр|вихрь|elitech|элитех|'
    r'зубр|интерскол|\bbort\b|orkan|скор\w*\.?\s*потока воздуха', re.I)


# НЕ НАША МАШИНА, НАЗВАННАЯ НАШИМ СЛОВОМ. Владелец показал факт «Закуп лебедки электрической
# и нагнетателя смазки» с типом «центробежная»: слово «нагнетатель» совпало, а нагнетатель
# смазки — это ручной шприц для солидола. Из такого не следует, что на предприятии есть
# центробежник, — определение мусора, данное владельцем.
#
# Классы собраны по живым цитатам, а не выдуманы:
#   нагнетатель смазки/масла/раствора/бетона   ручной инструмент и строительная техника
#   центробежный НАСОС                          та же приставка, другая машина
#   вентилятор (воздуходувка)                   так реестр ЭПБ пишет синоним, объект — вентилятор
#   турбокомпрессор дизеля, наддувочный воздух  автомобильный ТКР
#   компрессор кондиционера/холодильника        бытовая техника
#
# ЗАСЛОН ОТ ПЕРЕУСЕРДСТВОВАНИЯ: если в том же тексте названа И наша машина («поставка насосов
# и воздуходувок»), факт остаётся. Иначе правило съело бы настоящие закупки, где наша машина
# идёт списком вместе с чужими.
NE_NASHA = re.compile(
    r'нагнетател\w*\s+(?:смазк|масл|раствор|бетон|кле|герметик|топлив|мастик|жидкост|краск|'
    r'цемент|шпакл|битум)'
    r'|(?:центробежн\w+|вихрев\w+|шестерён\w+)\s+насос|насосн\w+ агрегат'
    r'|вентилятор\w*\s*\(\s*воздуходувк'
    r'|турбокомпрессор\w*\s+(?:дизел|двигател|наддув)|наддувочн\w+ воздух'
    r'|компрессор\w*\s+(?:холодильн|кондиционер|сплит|морозильн|бытов)'
    r'|компрессор\w*\s+кондиционера', re.I)
NASHA_MASHINA = re.compile(
    r'воздуходувк|воздуходувок|газодувк|газодувок|турбокомпрессор|турбовоздуходувк|'
    r'центробежн\w+\s+компрессор|компрессорн\w+\s+(?:установк|станци)|нагнетател\w*\s+воздух',
    re.I)
