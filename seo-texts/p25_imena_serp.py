...
# ОСНОВА ОТЧЕСТВА — {2,}, А НЕ {3,}. Узкий шаблон требовал трёх букв между заглавной и
# окончанием и молча терял самые ходовые отчества: Юрьевич, Юрьевна, Львович, Львовна —
# в них между «Ю» и «евич» ровно две буквы. Проверено прямым замером: из восьми живых
# написаний узкий шаблон теряет ПЯТЬ. Потеря невидима — человек просто не находится, и
# это выглядит пустой страницей, а не ошибкой разбора.
# Нашёл враждебный проверяющий, читая канал, который писался по этим же правилам.
FIO = re.compile(
    r'\b([А-ЯЁ][а-яё]{2,}(?:-[А-ЯЁ][а-яё]{2,})?)\s+'
    r'([А-ЯЁ][а-яё]{2,})\s+([А-ЯЁ][а-яё]{2,}(?:ович|евич|ьевич|инич|овна|евна|ьевна|ична))\b')
FIO_OBR = re.compile(
...
NE_FAMILIYA = re.compile(
    ...
    # «Сейчас Леонид Федорович» — наречие, разобранное как фамилия. Поймано глазами
    # в списке на переспрос: «Сейчас» стоит с большой буквы в начале предложения
    # перед именем, и шаблон ФИО принял его за фамилию.
