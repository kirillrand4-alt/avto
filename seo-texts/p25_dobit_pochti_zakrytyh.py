    ...
    def odin(c):
        ...
        for d in docs:
            ...
            if fam.lower() not in tekst.lower():
                continue
            m = TEL.search(tekst)
            # ОКНО ЦИТАТЫ — ЭТО НЕ СТРАНИЦА, и 600 знаков стоили мне неверного ответа
            # владельцу и 1-й сессии. По Паначеву я сказала «страница не называет ни
            # должности, ни предприятия», а на той же странице дальше стояло «Место
            # работы: ПАО "Уралкалий", главный энергетик-начальник отдела главного
            # энергетика СКРУ-1». Не было В ОКНЕ, а не на странице.
            #
            # Прямое следствие для счёта: приёмка добора снимает признак технического
            # круга, если должности нет в цитате, — так признак потеряли 43 человека.
            # Часть из них наверняка технические, просто должность стояла за границей.
            # Окно расширено вчетверо и записан признак, ЧТО именно в нём нашлось.
            mesto = bool(re.search(r'место\s+работ|должност|занимает', tekst, re.I))
            nashli.append({'ssylka': url, 'pochemu': poch,
                           'telefon': m.group(0) if m else '',
                           'est_mesto_raboty': mesto,
                           'znakov_na_stranice': len(tekst),
                           'citata': tekst[:2500]})
