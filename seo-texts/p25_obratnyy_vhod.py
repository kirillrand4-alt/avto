...
VHODY = [r'C:\sender\_ops\3s_p25_vhod_2s.csv', r'C:\sender\_ops\3s_p25_vhod_3s.csv',
         ...
         # ПЯТЫЙ ВХОД, И ОН ОТМЕНЯЕТ САМ ПРИНЦИП ЭТОГО СПИСКА. Собран не по именам
         # файлов, а ПОИСКОМ: все 232 файла на дропе и в рабочей папке, где есть
         # колонка с ИНН и колонка с ФИО. 1 567 имён на 131 предприятии, 588 из первой
         # сотни по продажам, 968 названы двумя и более источниками независимо.
         #
         # Час назад я записала «обратный ход исчерпан, ждать нечего». Он не был
         # исчерпан ни минуты: спрошено 1 126 имён при 73 342 добытых и ни разу не
         # спрошенных. Исчерпан был ЭТОТ СПИСОК — четыре файла из ста пятидесяти шести.
         # Правило «канал широкий, а список целей узкий» я за смену записала трижды —
         # про реестр ЭПБ, про площадку, про людей с карточек — и трижды не применила
         # к собственному главному каналу. Впредь новые источники добавляются сюда не
         # именем файла, а поиском по колонкам.
         r'C:\sender\_ops\3s_p25_vhod_sploshnoy.csv']
