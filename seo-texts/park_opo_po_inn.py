...
TIP = (r'(?:Площадк\w+|Участок|Участк\w+|Цех\b|Цеха\b|Сеть\s+газо\w+|Систем\w+\s+газо\w+|'
       ...
       r'Карьер\w*|Рудник\w*|Шахт\w+|Транспортн\w+\s+участок|Комплекс\w*|'
...
STOP = re.compile(r'\s*' + RIMSK + r'\s*(?:[-–]?\s*(?:го|ый|ой|й))?\s*класс\w*\s*опаснос'
                  ...
                  r'|\s*\(\s*№?\s*А\s?-?\s?\d{2}\s?-\s?\d{5}', re.I)
...
NE_OBEKT = re.compile(r'расходомер|клапан|щит\b|датчик|прибор учёт|прибор учет|'
