...
# ИМЯ ЖУРНАЛА МОЖНО ЗАДАТЬ ЦЕЛИКОМ. Понадобилось для второй задачи (P25): там свой журнал
# `P25-ZHURNAL-3.md`, и я выкладывал его ПРЯМОЙ ЗАЛИВКОЙ, минуя этот модуль. Вторая заливка
# затёрла первую запись — ровно тот дефект, который 1-я сессия нашла у моего файла людей в
# ту же минуту. Правило владельца «журнал только дозаписывается» держится не намерением, а
# механизмом: скачать с дропа, дописать в конец, выгрузить.
MOY = os.environ.get('ZHURNAL_FAYL') or f'ZHURNAL-{MOY_NOMER}.md'
