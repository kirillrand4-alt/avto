...
INDUSTRY = [
    ...
    ('нефтедобыча и нефтепереработка',
     r'НЕФТЕГАЗ|НПЗ|НЕФТЬ|ЛУКОЙЛ|ТАТНЕФТЬ|СЛАВНЕФТЬ|ГАЗПРОМНЕФТЬ|РОСНЕФТЬ|АНХК|\bННК\b|'
     r'НЕФТЕОРГСИНТЕЗ|НЕФТЕХИМСЕРВИС|СУРГУТНЕФТЕГАЗ|УДМУРТНЕФТЬ|ТАНЕКО|НЕФТЕПЕРЕРАБ|\bРН-'),
    ('химия, нефтехимия, удобрения',
     ...
     r'СИНТЕЗ|КАУЧУК|СИНТАНОЛ|КОРУНД|УДОБРЕН|ЦИАН|ТОМЕТ|КЗСК|ЭКТОС|ПРОМСИНТЕЗ'),
    ('чёрная металлургия и кокс',
     r'МЕТКОМБИНАТ|МЕТАЛЛУРГ|ЕВРАЗ|ММК|НЛМК|СЕВЕРСТАЛЬ|МЕЧЕЛ|КОКС|ТРУБН|ТРУБОПРОКАТ|ТМК|'
     r'ЧТПЗ|ПНТЗ|\bСТЗ\b|ЗСМК|НТМК|ОЭМК|УРАЛВАГОНЗАВОД|ЧМК|АЛЧЕВСК|\bДМЗ\b|ФЕРРОСПЛАВ'),
    ('цветная металлургия',
     r'РУСАЛ|НОРИЛЬСК|КОЛЬСКАЯ ГМК|УГМК|ЭЛЕКТРОЦИНК|ЧЦЗ|ЦИНК|МЕДЬ|МЕДНОГОРСК|СВЯТОГОР|'
     r'КАРАБАШМЕДЬ|ГЛИНОЗЕМ|АЛЮМИНИ|ТИТАН|ПОЛЮС|ЮГМК|МЕДЕПЛАВ|МЕДНО|\bСУМЗ\b|НИКЕЛ'),
    ('горная добыча и обогащение',
     r'ГОК|РУДНИК|РАЗРЕЗ|ШАХТА|ОБОГАТИТЕЛ|ЦОФ|\bОФ\b|ГОФ|КАРЬЕР|ОКАТЫШ|ППГХО|УГОЛ|КУЗБАСС|'
     r'РАСПАДСК|СУЭК|ЯКУТУГОЛЬ|ЮЖНЫЙ КУЗБАСС|АЛРОСА|АЛМАЗ|ГХК|ГМК'),
    ('энергетика (ТЭЦ, ГРЭС)',
    ...
    ('целлюлоза, бумага, дерево',
     r'ЦБК|ЦБП|БУМАЖ|ЦЕЛЛЮЛОЗ|ИЛИМ|СВЕЗА|КРОНО|\bЛЕС|КАРТОН|КБК|\bЛПК\b|СЛПК|ФАНЕР'),
    ('пищевая промышленность и АПК',
     r'МЕЛЬКОМБИНАТ|ХЛЕБ|САХАР|МОЛОК|МЯСО|ПИВО|ДРОЖЖ|КРАХМАЛ|СПИРТ|МАСЛОЖИР|ЭФКО|ЭЛЕКОМ|'
     r'МУКОМОЛ|МЕЛЬНИЦ|ЭЛЕВАТОР|КОМБИКОРМ|\bМЭЗ\b|ЗЕРНО|КАРАВАЙ|НИВА|\bХПП\b|КОНДИТЕР|'
     r'ФЕРРЕРО|МАКФА|РОТ ФРОНТ|\bКДВ\b|НЕСТЛЕ|ПРОВИМИ|АСТОН|ЮГ РУСИ|КОЛХОЗ|\bСПК\b|ПТИЦ|'
     r'КОРМ|МУКИ|СОЛОД|ХОЛОДИЛЬНИК|ПИЩЕВ|АГРО|МАСЛОЭКСТРАКЦ|БИОТЕХ|МИРФУДС|СИНЕРГИЯ'),
    ('газопереработка (ГПЗ, ГПП)',
     r'\bГПЗ\b|\bГПП\b|ГАЗОПЕРЕРАБ|ПУРГАЗ|ВНИИГАЗ'),
    ('машиностроение и оборонка',
     r'\bОДК\b|УМПО|САТУРН|КРАСМАШ|ПОЗИС|ВЕРФИ|ЗАВОД ИМЕНИ|\bБОЗ\b|ТУРБОКОМПРЕССОР|'
     r'МАШЗАВОД|МАШИНОСТРО|ДАМЕЛЬ|АВАНГАРД|ЧТЗ|УРАЛТРАК|АГРЕГАТ'),
    ('атомная промышленность',
     r'\bУЭХК\b|ЭЛЕКТРОХИМИЧЕСК|РОСАТОМ|АТОМ'),
    ('стройматериалы и цемент',
     r'ЦЕМЕНТ|ХАЙДЕЛЬБЕРГ|ЖБИ|КИРПИЧ|СТЕКЛ|ИЗВЕСТ'),
]


def industry_of(name, texts):
    """
    Сначала пробуем по названию юрлица — это надёжнее. Если название ничего
    не говорит («ООО "МФ"», «ЗАО "Ерофеев"»), только тогда смотрим на текст
    объектов; там сигнал грязный (слово «азот» есть в описании трубопровода
    на любом заводе), поэтому такие случаи помечаются отдельно.
    """
    nm = name.upper().replace('Ё', 'Е')
    for label, pat in INDUSTRY:
        if re.search(pat, nm):
            return label, 'название'
    blob = ' '.join(texts).upper().replace('Ё', 'Е')
    for label, pat in INDUSTRY:
        if re.search(pat, blob):
            return label, 'текст объектов'
