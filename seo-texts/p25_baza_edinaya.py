...
KANALY = [
    ...
    # ПЛОЩАДКИ КАК КАНАЛ КОНТАКТОВ. На карточке процедуры ТЭК-Торга и Росэлторга рядом с
    # предметом стоит контактное лицо с телефоном: у ТЭК-Торга 56 названных людей из 63
    # строк, у Росэлторга 56 телефонов из 66. Схема их файлов совпадает с тем, что читает
    # этот сборщик (`inn`, `chelovek`, `telefon`, `pochta`, `istochniki`), поэтому канал
    # добавляется без перевода — и именно поэтому канал НАЗВАН отдельно: провенанс не
    # смешивается, у площадки своя надёжность и свои ссылки.
    ('карточка процедуры ТЭК-Торга', 'PARK-TEKTORG-ZAKUPKI-3S.jsonl'),
    ('карточка процедуры Росэлторга', 'PARK-ROSELTORG-ZAKUPKI-3S.jsonl'),
