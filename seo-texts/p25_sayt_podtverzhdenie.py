...
def sokrashchenie(yadro, sam):
    """Сайт зовёт себя АББРЕВИАТУРОЙ нашего же названия — это он и есть.

    Поймано на разборе восемнадцати «сайтов группы»: `ao-gns.ru` у АО «ГЛАВНОВОСИБИРСКСТРОЙ»
    зовёт себя «ОАО ГНС». Ни в домене, ни в самоназвании слова «главновосибирскстрой» нет, и
    правило самоназвания честно сказало «зовёт себя иначе» — а это тот же завод, просто
    сложносокращённо: ГлавНовосибирскСтрой. Признак: короткое слово из самоназвания идёт
    ПОДПОСЛЕДОВАТЕЛЬНОСТЬЮ букв нашего ядра. Проверка нарочно слабая и работает только в одну
    сторону — она способна лишь ОТМЕНИТЬ вывод «сайт группы», вернув прежний слабый исход."""
    yad = re.sub(r'[^а-яё]', '', (yadro or '').lower())
    if len(yad) < 6:
        return False
    for slovo in re.findall(r'[а-яёa-z]{2,6}', (sam or '').lower()):
        if slovo in ('оао', 'ооо', 'пао', 'зао', 'ао', 'гк', 'тм', 'официальный', 'сайт'):
            continue
        i, nachalo = 0, -1
        for ch in slovo:
            i = yad.find(ch, i)
            ...
            # БУКВЫ ДОЛЖНЫ ИДТИ С ПРОПУСКАМИ, а не подряд. Иначе правило ловит общее отраслевое
            # слово: «фарма» — подпоследовательность «нанофармадевелопмент», но она же и
            # непрерывная подстрока, и «Изварино Фарма» от этого нашим заводом не становится.
            # Аббревиатура сложносокращённого имени всегда разрежена: ГНС в ГлавНовосибирскСтрой
            # это буквы 0, 4 и 8.
            if len(slovo) >= 3 and (i - nachalo) > len(slovo):
                return True
    return False
