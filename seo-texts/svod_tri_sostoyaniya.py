...
def main():
    ...
    # ЭТП ГПБ, СЛОВАРНЫЙ обход по всей площадке — единственный канал, находящий предприятия ВНЕ
    # нашей очереди. Обходы по компаниям (Tender.pro, ЭТП ГПБ) ходят по нашему же списку ИНН и
    # структурно не могут выйти за него: замер дал 0 и 1 предприятие вне очереди соответственно,
    # а этот — 1 135 из 1 290.
    # ИНН добран справочником площадки по slug ещё на сборе, поэтому здесь берётся готовым.
    for r in chitat(os.path.join(C, 'etpgpb-po-slovam.csv')):
        inn = (r.get('inn') or '').strip()
        pred = r.get('predmet') or ''
        if not re.fullmatch(r'\d{10}|\d{12}', inn):
            poteri['ЭТП ГПБ по словам: ИНН не нашёлся в справочнике площадки'] += 1
            continue
        nash = nash_predmet(r, pred)
        fakty.append({
            'inn': inn, 'predpriyatie': (r.get('zakazchik') or '').strip()[:120],
            'sostoyanie': 'покупает' if nash else 'есть на площадке',
            'marki': ' | '.join(marki_iz(pred)[:4]) if nash else '',
            'data': (r.get('data') or '').strip(), 'chto_za_data': 'дата процедуры',
            'chem_dokazano': f'ЭТП ГПБ, процедура {(r.get("nomer") or "").strip()}',
            'tekst': re.sub(r'\s+', ' ', pred)[:300],
            'ssylka': (r.get('ssylka') or '').strip()[:200],
            'istochnik': 'ЭТП ГПБ, поиск по словам',
        })
