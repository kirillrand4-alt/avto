...
STRANIC = 1   # последовательный обход под одним профилем не успевает больше за один заход
...
STROKA = re.compile(r'№\s*(\d{6,9})\s+(.{5,220}?)\s+(\d{2}\.\d{2}\.\d{4}\s+\d{2}:\d{2})')
ORG = re.compile(r'((?:ООО|АО|ПАО|ЗАО|ОАО|НАО|ГУП|МУП|ФГУП|АК)\s*[«"„][^»"“]{2,90}[»"“]|'
                 r'(?:ООО|АО|ПАО|ЗАО|ОАО)\s+[А-ЯЁ][^,;]{2,60})')
SCHET = re.compile(r'Актуально\s*•\s*([\d\s ]{1,9}).{0,40}?В архиве\s*•\s*([\d\s ]{1,12})')


def razobrat(html):
    """Лот беру ИЗ ПЛОСКОГО ТЕКСТА ленты, а не из ссылки. Ссылки на лот в разметке НЕТ.

    Два разбора подряд дали ноль названий, и причина оказалась не в тегах: вхождений
    `/tender/` в странице РОВНО НОЛЬ. Мои 93 «лота» приходили из запасной ветки — из
    номеров вида «№ 4555289» в тексте, — а название я честно оставляла пустым.

    В тексте строка ленты выглядит так:

        Запрос предложений № 4555289 Компрессоры , запасные части/ АО «ОРК» ПАО "Русолово"
        09.08.2026 08:34 14.08.2026 17:00

    То есть между номером и первой датой лежит название И организатор. Беру этот отрезок,
    отрезаю от него организатора по правовой форме с кавычками — и получаю обе колонки.
    Ссылку на карточку строю от номера (`/market/tender/<номер>/`), и она проверяется
    отдельно, как всякая собранная ссылка.
    """
    out = []
