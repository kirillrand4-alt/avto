...
def qa(html: str, page: dict) -> list:
    ...
    # УТЁКШИЙ СЛУЖЕБНЫЙ ОТВЕТ МОДЕЛИ.
    # Азотно-кислородная страница уехала в архив с секцией «Реализованные проекты», внутри
    # которой лежал не кейс, а рассуждение модели: «ни один из трёх проектов в базе знаний
    # не относится… рекомендация: пропустить секцию… уточните, и я подберу». Модель повела
    # себя правильно (двухгазового кейса в базе действительно нет и выдумывать его нельзя),
    # но её ответ попал в тело статьи. Прошлая версия проверки сканировала только текст
    # внутри <p> и <li>, а утечка пришла голым текстом между тегами - и не нашлась.
    # Поэтому здесь сканируется ВЕСЬ текст страницы.
    # Ловим ОБРАЩЕНИЕ К ОПЕРАТОРУ и ссылки на служебные источники, а не любое «уточните»:
    # фраза «уточните у заказчика требуемую чистоту» - это ровно то, чего требует ТЗ
    # («после каждого технического блока давать вывод: какие данные запросить»), и первая
    # версия проверки забраковала все пять страниц именно на ней.
    leak = re.findall(r'(?i)[^.!?]*(?:\bв базе знаний\b|\bв исходных данных\b|'
                      r'\bя подберу\b|уточните,? и я\b|как языковая модель|'
                      r'пропустить секцию|заменить её заглушкой|'
                      r'использовать их[^.!?]{0,40}нельзя)[^.!?]*[.!?]', text)
    for frag in leak[:3]:
        issues.append(f'служебный ответ модели в теле статьи: «{frag.strip()[:110]}…»')
