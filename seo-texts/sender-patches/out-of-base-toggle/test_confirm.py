...
def _confirm_with_toggle(store, suppression, allow):
    return ConfirmSend(
        _Cfg(**{"confirm.mode": "all", "confirm.allow_out_of_base": allow}),
        store, suppression, cards=_CardsStub())


def test_out_of_base_blocked_by_default(store, suppression):
    cs = _confirm_with_toggle(store, suppression, allow=False)
    cid, rid, mid = seed_message(store)
    cs.submit(email="lead@zavod.ru", inn="4201000625", campaign_id=cid,
              recipient_id=rid, message_id=mid, subject="s", body="b")
    rv = cs.pending(campaign_id=cid)[0]["id"]
    with pytest.raises(ConfirmBlockedError):
        cs.approve(rv, operator="op")


def test_out_of_base_allowed_when_toggle_on(store, suppression):
    cs = _confirm_with_toggle(store, suppression, allow=True)
    cid, rid, mid = seed_message(store)
    cs.submit(email="lead@zavod.ru", inn="4201000625", campaign_id=cid,
              recipient_id=rid, message_id=mid, subject="s", body="b")
    rv = cs.pending(campaign_id=cid)[0]["id"]
    assert cs.approve(rv, operator="op") is True
