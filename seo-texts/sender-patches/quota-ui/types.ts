...
/** Один день расписания дневной квоты AI-генерации (карточка на «Кампаниях»). */
export interface QuotaDay {
  date: string;        // "2026-07-25"
  quota: number;       // план на день (0 = квота не задана)
  generated: number;   // ok: письмо ушло в очередь подтверждений
  rejected: number;    // брак: не прошло гейт/линзы
  attempts: number;    // ok + брак — именно это съедает квоту
  remaining: number;   // сколько ещё догенерируем сегодня
  weekday: number;     // ISO 1=Пн .. 7=Вс
}

/** Итог одного прогона «сгенерировать сейчас». */
export interface QuotaRunResult {
  campaign_id: number; date: string; quota: number; before: number;
  planned: number; generated: number; rejected: number; candidates: number;
  reason: string; errors: string[];
}

/** Состояние фонового прогона (durable в panel_settings). */
export interface QuotaRunState {
  running?: boolean;
  stale?: boolean;          // служба перезапустилась посреди прогона
  started_at?: string;
  finished_at?: string;
  date?: string;
  actor?: string;
  error?: string | null;
  result?: QuotaRunResult | null;
}
...
export interface QuotaView {
  ...
  today: string;
  days: QuotaDay[];
  candidates_left: number;   // оценка: кому в сегменте ещё не писали
  run: QuotaRunState;
}
