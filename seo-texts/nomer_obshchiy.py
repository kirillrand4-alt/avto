...
def postroit(pary):
    ...
    """
    karta = collections.defaultdict(lambda: {'inn': set(), 'lyudi': set(), 'vstrechalsya': 0})
    for p in pary:
        inn, tel = p[0], p[1]
...
def sudba(k, karta, ishodnoe=''):
    ...
    if n_inn > 1:
        v = (f'ОБЩИЙ: номер стоит у {n_inn} предприятий — это приёмная, коммутатор или '
             f'линия площадки, личным быть не может')
    elif n_lyud > 2:
        v = (f'ПОДОЗРИТЕЛЬНЫЙ: одно предприятие, но {n_lyud} разных людей — похоже на '
