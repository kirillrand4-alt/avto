...
# ЗАСЛОН НА ГОЛЫЙ ПОЗИЦИОННЫЙ НОМЕР. Класс поймал провайдер, а не я: из 300 разобранных им
# серий 159 получили «не наша машина», и глазами видно, что это не чужие машины, а позиции —
# «К-3/2», «К057», «К2», «АК001-007», «Ц6», «К-345/1». Мой прежний заслон ловил только
# ХВОСТЫ («поз.», «зав. №», «инв. №»), а голый код от серии «К-250-61-5» не отличал.
# Признак: одна-две буквы плюс не больше трёх цифр (и, возможно, дробь) — у настоящей серии
# машин цифр больше и есть параметры (К-250-61-5, ТВ-80-1,6, ЦК-135/8).
# ФОРМА КОДА САМА ПО СЕБЕ РЕШАТЬ НЕ МОЖЕТ — это итог двух сужений и одной сверки.
# Суженный до К/АК/Ц заслон помечает 208 строк. Провайдер, спрошенный по ним отдельно,
# СОГЛАСИЛСЯ со 163 и ВОЗРАЗИЛ по 38, и возражения по существу: К-250-61, К-500-61,
# К-605-182, К-402, К-400, К601, К101, К20, К12, К-24 — настоящие центробежные серии,
# К30/10 и К-5/7 — поршневые. То есть «К» плюс три цифры бывает и позицией, и серией,
# и никакой шаблон этого не различает.
# Поэтому шаблон понижен в звании: он даёт ПОДОЗРЕНИЕ, а вердикт выносит второй прибор.
# Снимается строка только там, где ОБА согласны.
#
# ПЕРВЫЙ ВАРИАНТ ЗАСЛОНА БЫЛ ШИРОК, И ЭТО ПОКАЗАЛА СВЕРКА С ПРОВАЙДЕРОМ, а не моё чутьё.
# Он снимал 284 строки, из них провайдер согласился с 96, не смотрел 179, и по ДЕВЯТИ прямо
# возразил: ТВ-152, ТВ-252, ТВ-552, ТВ-652 — турбовоздуходувки (центробежные), ВК-430,
# ВК-205, ВК100-8 — винтовые компрессоры, ВП-4 — поршневой. То есть префиксы ТВ, ВК, ВП это
# НАСТОЯЩИЕ индексы машин, и снимать их нельзя. Оставляю К, АК, Ц — те, где провайдер
# подтвердил позиционный характер (К2, АК-0, АК-1, К-132, Ц6).
...
VYDUMKI = ['ЩВ-999', 'поз. 12', 'зав. № 4471', 'насос НМ-1250', 'вентилятор ВДН-18',
           ...
           'адсорбер А-12',
           # добавлены после находки провайдера: голые позиционные коды
           'К-3/2', 'К057', 'К2', 'АК001-007', 'Ц6', 'К-345/1']
...
for r in stroki:
    ...
    elif POZICIONNYY.match(ob):
        verd = (r.get('mashina_provajder') or '').strip()
        if verd == 'НЕ наша':
            zaslony['СНЯТО: позиционный номер (шаблон И провайдер согласны)'] += 1
            primery['СНЯТО: позиционный номер (шаблон И провайдер согласны)'].append(ob)
        elif verd:
            zaslony['ОПРАВДАНО провайдером: это машина, шаблон ошибся'] += 1
            primery['ОПРАВДАНО провайдером: это машина, шаблон ошибся'].append(
                '%s (%s)' % (ob, (r.get('princip_provajder') or '')[:12]))
        else:
            zaslony['подозрение на позицию, второй прибор не спрошен'] += 1
            primery['подозрение на позицию, второй прибор не спрошен'].append(ob)
...
for v in VYDUMKI:
    ...
    otsek = bool(POZ.match(v) or CHUZH.search(v) or POZICIONNYY.match(v))
