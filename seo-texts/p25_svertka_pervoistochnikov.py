...
# ГРАНИЦЫ СЛОВА ОБЯЗАТЕЛЬНЫ. Без них «АО» находится внутри слова «зАОкупкам», и первый же
# прогон объявил чужим юрлицом «акупкам ТМЦ» — то есть заслон начал резать верные строки,
# причём с уверенной формулировкой. Форма собственности пишется прописными и стоит отдельным
# словом; регистр здесь не игнорируем именно поэтому.
YURLICO = re.compile(r'(?<![А-Яа-яЁёA-Za-z])(?:ООО|АО|ПАО|ОАО|ЗАО|ГУП|МУП|ФГУП|НПО|НПП|ФКП)'
                     r'(?![А-Яа-яЁёA-Za-z])\s*[«"\']?([А-ЯЁA-Z][^»"\'\n,;.]{2,44})')
...
def main():
    ...
    # СВЕРЯЕМ ВСЕ МОБИЛЬНЫЕ, А НЕ ТОЛЬКО С НАЗВАННОЙ РОЛЬЮ. Узкий отбор исчерпался на 30
    # строках, а мобильных 258, и у 121 из них роль ПУСТАЯ — это ровно те, за кем ходит
    # прицельный поиск должности. Проверять цепочку у них надо тем более: если человек
    # приписан не тому предприятию, найденная ему должность закрепит ошибку, а не исправит.
    # МОБИЛЬНЫЕ ИСЧЕРПАНЫ (258 сверено), расширяю на всех С НАЗВАННОЙ РОЛЬЮ. Городской
    # с добавочным — рабочий контакт для панели ничуть не меньше мобильного, и цепочку у него
    # проверять надо тем же порядком: имя, должность, принадлежность юрлицу.
    celi = [r for r in rows
            if (r['vid_nomera'] == p25_nomer.MOBILNYY
                or r['rol'] in ('техническая', 'снабжение', 'руководство'))
