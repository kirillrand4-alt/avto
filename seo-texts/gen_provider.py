...
_UPDATED_BASE_DEFAULT = '2026-08-21'


def updated_date(slug):
    """Детерминированная по слагу дата «Обновлено» в окне ~4 мес до опорной даты,
    чтобы сотни страниц не были «обновлены» одним днём (сигнал автонаполнения).

    Опорная дата - в переменной PROVIDER_UPDATED_BASE, иначе константа ниже.
    Константу надо двигать: до 20.08.2026 тут стояло 15.07.2026, и любая генерация
    после этой даты проставляла дату задним числом - до четырёх с половиной месяцев
    просрочки на всех страницах сразу. Дату держим фиксированной, а не «сегодня»,
    чтобы перегенерация страницы не меняла её каждый раз."""
    import hashlib, datetime, os
    base = datetime.date.fromisoformat(
        os.environ.get('PROVIDER_UPDATED_BASE', _UPDATED_BASE_DEFAULT))
...
# в бесконечные ping-кадры, текста нет. Пока так — подменяем на рабочую.
#
# Проверено 27.07.2026: fable-5 и opus-5 — только ping до дедлайна; opus-4-8 (2.0с),
# haiku-4-5 (1.6с), sonnet-4-6 (1.9с), sonnet-5 (10с) отвечают штатно.
#
# ПЕРЕПРОВЕРЕНО 19.08.2026: **fable-5 ожил** — короткий вопрос 3 с, тяжёлый промпт
# на 19 784 символа 125 с с корректным ответом. Убран из списка: правило владельца
# гонит тяжёлую работу именно на fable, а месяц все вызовы молча уходили на opus-4-8
# по устаревшей записи. opus-5 по-прежнему мёртв (молчит 90 с, только ping) —
# остаётся. Список стоит перепроверять раз в пару недель, состояние шлюза дрейфует.
_DEAD_DEFAULT = 'claude-opus-5'
