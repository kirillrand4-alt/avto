...
Вывод у раннера сохраняется ХВОСТОМ, поэтому: примеры печатаются первыми, ЧИСЛА — в самом
конце, и печатаются они в КАЖДОМ прогоне. Раздел примеров выбирается аргументом:
...
ZAVOD_V_IMENI = re.compile(
    r'\bзавод|комбинат|фабрик|\bГОК\b|\bНПЗ\b|\bТЭЦ\b|\bГРЭС\b|\bАЭС\b|\bШахт|рудник|'
    r'металлург|цемент|карьер|птицефабрик|молокозавод|мясокомбинат|'
    r'производствен\w+\s+объединен|\bПО\s|агрегатн', re.I)
...
def kody(okv):
    """Поле ОКВЭД — СПИСОК через | (или , ;). Возвращаю список кодов."""
    return [c.strip() for c in re.split(r'[|,;]+', str(okv or '')) if c.strip()]
...
def stroka_primera(d):
    return ('  · %s | ОКВЭД %s | %s\n      деят: %s'
            % (d['inn'], (d['okv'] or '(пусто)')[:60], (d['nazv'] or '(без названия)')[:64],
               (d['act'] or '(описания нет)')[:170]))
