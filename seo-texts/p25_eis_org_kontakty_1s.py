# -*- coding: utf-8 -*-
"""КОНТАКТЫ ПО ПАРКУ СОСЕДА. Они выложили 5 149 ИНН, из них 3 066 у меня отсутствуют.

Разделение работ, о котором договорились: они расширяют парк, я обогащаю. Файл
`PARK-VYDACHA-PREDPRIYATIYA.csv` наконец лежит на дропе (я просила четыре тика), и мой
самый быстрый канал — карточка организации ЕИС — применим к нему без изменений: 1 390
целей он прошёл за 154 секунды и дал 162 телефона.

Всё прочее — заслоны, накопление файла, вид номера — как в основном сборщике.

СТАРОЕ ОПИСАНИЕ основного прогона ниже, оно же объясняет заслоны.
...
VYHOD = os.path.join(OPS, 'PARK-EIS-ORG-KONTAKTY-1S-3S.jsonl')
POTOKOV = 8
...
# ЦЕЛИ — ИЗ ФАЙЛА СОСЕДА, а не из моего парка: беру их ИНН, которых у меня нет.
vidno = set()
# ФАЙЛ СОСЕДА ЛЕЖИТ НА ДРОПЕ, А НЕ НА СЕРВЕРЕ. Первый заход дал «целей 0» — и это был
# честный ноль прибора: скрипт искал файл рядом с собой, а его там нет. Качаю с дропа.
_ih = os.path.join(OPS, 'PARK-VYDACHA-PREDPRIYATIYA.csv')
if not os.path.exists(_ih):
    try:
        _op = urllib.request.build_opener(urllib.request.ProxyHandler({}))
        _syr = _op.open(urllib.request.Request('%s/PARK-VYDACHA-PREDPRIYATIYA.csv' % drop,
                                               headers=tok), timeout=300).read()
        io.open(_ih, 'wb').write(_syr)
        print('файл соседа скачан с дропа: %d байт' % len(_syr))
    except Exception as _e:  # noqa: BLE001
        print('файл соседа НЕ скачался: %s' % str(_e)[:60])
if os.path.exists(_ih):
    _sh = None
    for _s in io.open(_ih, encoding='utf-8-sig'):
        _p = _s.rstrip('\n').split(';')
        if _sh is None:
            _sh = _p
            continue
        ...
        if _i.isdigit() and _i not in vidno:
            vidno.add(_i)
            if _o.get('nazvanie'):
                imena[_i] = _o['nazvanie'][:180]
            if _i not in est_kontakt:
                celi.append(_i)
for f in []:
