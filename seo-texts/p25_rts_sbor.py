...
# СПИСОК СЛОВ ЗАДАЁТСЯ СНАРУЖИ, иначе «перепроверить площадку» означает «спросить у неё ровно
# то же самое ещё раз». Зашитый список — это не проверка канала, а его повторение: он вернёт
# те же 15 строк и создаст впечатление, что площадка исчерпана. По умолчанию остаётся прежний
# список, чтобы прежние прогоны воспроизводились.
SLOVA = [w for w in os.environ.get(
    'P25_SLOVA',
    'компрессор|винтовой компрессор|поршневой компрессор|генератор азота|'
    'генератор кислорода|азотная станция|кислородная станция|'
    'компрессорная станция|воздуходувка|осушитель сжатого воздуха').split('|') if w.strip()]
