...
VHODY = ['PARK-PLOSHCHADKI-DLYA-PARKA-3S.jsonl',   # ТЭК-Торг, Росэлторг, РТС
         ...
         'PARK-EIS-TIK5-PODTV-3S.jsonl',
         'PARK-EIS-TIK6-PODTV-3S.jsonl',
         'PARK-EIS-TIK7-PODTV-3S.jsonl',
         'PARK-EIS-TIK8-PODTV-3S.jsonl',
         'PARK-EIS-TIK9-PODTV-3S.jsonl',
         'PARK-EIS-TIK10-PODTV-3S.jsonl',
         'PARK-EIS-TIK11-PODTV-3S.jsonl',
         'PARK-EIS-TIK12-PODTV-3S.jsonl',
         'PARK-EIS-TIK13-PODTV-3S.jsonl',
         'PARK-EIS-TIK14-PODTV-3S.jsonl',
         'PARK-EIS-TIK15-PODTV-3S.jsonl',
         'PARK-EIS-TIK16-PODTV-3S.jsonl',
         'PARK-EIS-TIK17-PODTV-3S.jsonl',
...
for f in VHODY:
    ...
    for s in syr.splitlines():
        ...
        z = potok.get(k)
        if not z:
            # ИНН, СШИТЫЙ ПО НАЗВАНИЮ, ПОМЕЧАЕТСЯ. Класс назвала 1-я сессия, я померила у
            # себя: из 40 проверенных строк со сшитым ИНН у ДВУХ на карточке стоит чужой ИНН
            # (7 % среди тех, где ИНН вообще напечатан). Однофамильных юрлиц много, и сшивка
            # по имени их сливает. Не выбрасываю — помечаю, чтобы вес факта был виден.
            z = potok[k] = {'inn': inn, 'vid': vid, 'predpriyatie': zak[:200],
                            'inn_sshit_po_nazvaniyu':
                                'сшит по названию' in str(o.get('inn_otkuda') or ''),
