...
ПРАВКА ПРОМПТА, СДЕЛАННАЯ ПО ЗАМЕРУ (16.09.2026). На первом прогоне сводка «рассматривает
возможность строительства (детали не раскрыты)» получила «не определить», хотя это буквально
намерение - самая дорогая для нас стадия. В описание стадии 1 добавлены слова-признаки.
Перепроверено на тех же 11 неопределённых строках плюс приманки: стадию получила ОДНА строка
(та самая), остальные десять остались «не знаю», приманки по-прежнему ноль. То есть правка
добавила точность, а не размыла гейт - иначе это была бы подгонка под желаемый ответ.
"""
import json
import os
import re
...
1 намерение - решение или замысел объявлены, работ нет: соглашение о намерениях, меморандум,
  инвестсовет одобрил, компания заявила о планах, РАССМАТРИВАЕТ ВОЗМОЖНОСТЬ, изучает вариант,
  ведёт переговоры о проекте, «планируется построить», статус резидента ОЭЗ/ТОР/АЗРФ, СЗПК, СПИК.
  Слова «рассматривает возможность строительства» - это намерение, а не отсутствие стадии.
...
def zvat(prompt, model='claude-fable-5', attempts=3):
    ...
    # ПУТЬ ДОБАВЛЯЕМ ДО ПРОБЫ ИМПОРТА. Первый прогон на сервере ушёл в песочный путь и упал
    # с ModuleNotFoundError('gen_provider'): проба `import verify_company` шла ДО того, как
    # C:\sender\server попадал в sys.path (его добавлял сам _zvat_server, то есть слишком
    # поздно). Сбой при этом отработал правильно - статус sboy_provaydera, стадии нет.
