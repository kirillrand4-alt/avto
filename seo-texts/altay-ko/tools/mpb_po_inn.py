...
# СРЕДА. Правило владельца: нужна ВОЗДУШНАЯ, в крайнем случае неопределённая. Газовые машины
# не выбрасываем — владелец продаёт и их, — но в один список с воздушными они идти не должны.
# Замер на 5 238 центробежных из реестра: газ 1 668, воздух 617, «и газ, и воздух» 327, среда
# не названа 2 626. То есть без этой колонки почти треть списка была бы про синтез-газ,
# конвертируемый газ и ГПА — то, за чем владелец сейчас не идёт.
...
def sreda(ob):
    """Воздух, газ, оба или не названа. «Не названа» — не то же, что «воздух»."""
    g, v = bool(GAZ.search(ob)), bool(VOZDUH.search(ob))
    if g and v:
        return 'и газ, и воздух'
    if g:
        return 'газ'
    if v:
        return 'воздух'
