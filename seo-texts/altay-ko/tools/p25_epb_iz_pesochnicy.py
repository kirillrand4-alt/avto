...
def citata(t, obrazec, dlina=420):
    # ЦИТАТА БЫЛА КОРОТКА ДЛЯ РАЗБОРА МАРКИ. В заключениях ЭПБ настоящая марка и позиция
    # стоят в одной фразе: «Центробежный компрессор КВД тип 3М7 (…установки К-901),
    # технологический индекс К-901 КВД, заводской № 3-7-3228». При обрезке в 220 знаков
    # пара «тип X + индекс Y» доезжает редко: из 656 строк слова «технологический индекс»
    # или «поз.» видны у 284, а полная пара — только у 3. Удлиняю окно, чтобы марку можно
    # было отличить от позиции по САМОМУ ДОКУМЕНТУ, а не по форме кода.
