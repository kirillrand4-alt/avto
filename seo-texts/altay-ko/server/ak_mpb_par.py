...
def _reestr_zhiv():
    ...
    _k = [r[0] for r in _c.execute("select inn from fakty where vid_fakta like 'ЭПБ%' limit 3")]
    _c.close()
    for _inn in _k:
        try:
            _r = M.po_inn(_inn) if 'M' == 'O' else M.po_inn(_inn, max_stranic=2, tolko_tu=True)
