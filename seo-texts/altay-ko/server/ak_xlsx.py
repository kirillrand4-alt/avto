# -*- coding: utf-8 -*-
"""Промежуточная выгрузка в Excel: три листа (Предприятия, Карточки доказательств, Финансы) + лист Сводка.
Кладёт AK-ITOG.xlsx на дроп. argv: [FULL] - включить кандидатов и неоценённых."""
import os, sys, re, json, sqlite3, collections, time, urllib.request
sys.stdout.reconfigure(encoding='utf-8', errors='replace')
from openpyxl import Workbook
from openpyxl.styles import Font, Alignment, PatternFill
from openpyxl.utils import get_column_letter
AK = r'C:\sender\_ops\ak'; DB = os.path.join(AK, 'AK-BAZA.sqlite'); OUT = os.path.join(AK, 'AK-ITOG.xlsx')
...
stroki += [[], ['Карточек доказательств всего', len(d2)], ['Предприятий с карточкой', len({x[0] for x in d2})], []]
...
req = urllib.request.Request(os.environ['DROP_URL'].rstrip('/') + '/AK-ITOG.xlsx', data=data, method='PUT', headers={'X-Drop-Token': os.environ['DROP_TOKEN'], 'Content-Type': 'application/octet-stream'})
