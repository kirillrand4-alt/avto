...
def hard_filters(d):
    ...
    # ОТМЕНЕНО владельцем 18.08: два hard-фильтра сняты, метрики остаются в скоринге.
    #   * индексация <70% - «мы по другому будем смотреть»: колонка «% индекс Я»
    #     гримируется через robots.txt и без «Стр. в индексе» ничего не значит,
    #     а решение по индексации принимается отдельным контуром;
    #   * >700 размещений - отсечка выбрасывала домен целиком, хотя пять доноров
    #     из десятки владельца отсеивались только ею.
    # Обе метрики продолжают работать баллами: индексация - блок D, число размещений -
    # блок E (>700 даёт -10). То есть такие доноры теперь не исчезают из выборки,
    # а падают в ранжире и остаются видимыми для ручного решения.
    ...
    # ОТМЕНЕНО владельцем 18.08 вместе с индексацией и «линкопомойкой»: отсечки
    # «PBN» (DR>=45 при трафике <300) и «дроп» (ИКС>=500 при трафике <500) сняты.
    # Обе судили не сам сайт, а РАССОГЛАСОВАНИЕ метрик, то есть выбрасывали домен по
    # косвенной улике. Штрафы за то же рассогласование остаются в блоке C: DR
    # обнуляется, ИКС срезается до 3 - домен виден в ранжире с низким баллом.
