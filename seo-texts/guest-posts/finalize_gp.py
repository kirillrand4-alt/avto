...
# ── Линза, добавленная 20.08 по итогам сплошной ручной вычитки ────────────────
# Наблюдение: пять статей волны прошли все 16 линз, а при чтении целиком в каждой
# нашлось сквозное числовое противоречие. Причина структурная - линзы смотрят
# абзац и его окрестность, а расхождение живёт МЕЖДУ разделами: вступление против
# расчёта, формула против рекомендации, пример против опорной константы.
# Пойманное руками:
#   * вступление «суммарный расход 180-220 л/мин» против списка инструмента на 4150;
#   * сценарий «компрессор 0,8 м³/мин, ресивер 500-1000 л» против собственной формулы
#     статьи, дающей для этого дефицита 4300 л;
#   * «22 кВт даёт 3,3-3,6 м³/мин» против удельной мощности 6,5-7,5 кВт/(м³/мин)
#     двумя абзацами выше;
#   * тариф на электричество в ДОЛЛАРАХ в статье для российского цеха.
CHAIN_LENS = {
'numbers_chain': """Ты инженер-нормоконтролёр. Ты НЕ проверяешь отдельные утверждения -
для этого есть другие проверки. Твоя работа одна: прочитать статью ЦЕЛИКОМ и найти
места, где числа СПОРЯТ ДРУГ С ДРУГОМ ЧЕРЕЗ РАЗДЕЛЫ.

Как это выглядит на практике (все случаи настоящие, из этой кампании):
* вступление называет суммарный расход 180-220 л/мин, а список оборудования в расчётной
  части даёт в сумме 4150 л/мин - расхождение на порядок, но разделы стоят далеко;
* статья вводит формулу буферного объёма, а через два экрана рекомендует ресивер,
  который по этой же формуле в четыре раза мал;
* статья задаёт удельную мощность 6,5-7,5 кВт на м³/мин, а ниже пишет «машина 22 кВт
  даёт 3,3-3,6 м³/мин» (это 6,1-6,7 - мимо собственной константы);
* кратность экономии в лиде «в 5-7 раз», в теле «в 5-6», в резюме снова другая;
* пример считает потери на продувку осушителя умножением на 1,2, хотя продувка забирает
  долю потока и требует деления на 0,8.

Порядок работы:
1. Выпиши про себя все опорные величины, которые статья ЗАДАЁТ (удельные мощности,
   коэффициенты, формулы, тарифы, нормы).
2. Найди все места, где статья ими ПОЛЬЗУЕТСЯ.
3. Пересчитай каждое такое место сам и сравни.
Считай честно, в столбик. Расхождение в пределах округления - НЕ ошибка.

Отдельно проверь бытовую достоверность для российского читателя: тариф на
электричество в рублях, а не в долларах; ГОСТ/ISO названы существующие; единицы
те, которыми пользуются в отрасли.

Правку давай точечную - меняй ТО ЧИСЛО, которое неверно, а не то, которое неудобно.
Если противоречат два числа, скажи в обосновании, почему верно именно оставленное.
FAIL - только при реальном расхождении, которое заметит читающий инженер.""",
}
LENSES.update(CHAIN_LENS)
...
def finalize(fname, only=None, from_ready=False):
    ...
    # Дозапуск (--only --from-ready) меняет вердикт: принятая статья может уйти
    # в NEEDS-REVIEW и наоборот. Файл-двойник с прежним вердиктом надо убрать,
    # иначе в ready/ лежат две версии одной статьи и непонятно, какая свежая.
    stale = f'{base}.NEEDS-REVIEW.html' if ok else f'{base}.final.html'
    if os.path.exists(os.path.join(READY, stale)):
        os.remove(os.path.join(READY, stale))
    ...
    # Дозапуск отдельных линз ДОПИСЫВАЕТ лог, а не затирает: в прежнем логе лежат
    # разборы конфликтов, сделанные руками, и история прошлых кругов. Потерять их
    # нельзя - именно по ним видно, кто и почему правил каждое место.
    log_path = os.path.join(READY, f'{base}.finalize-log.md')
    mode = 'a' if (only and os.path.exists(log_path)) else 'w'
    with open(log_path, mode, encoding='utf-8') as fh:
        if mode == 'a':
            fh.write(f'\n\n# Дозапуск линз: {", ".join(only)}\n\n')
        fh.write('\n'.join(log))
