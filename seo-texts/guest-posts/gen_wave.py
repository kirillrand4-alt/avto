...
=== АВТОРИТЕТНЫЕ ИСТОЧНИКИ (по желанию, 0-2 ссылки) ===
...
редакционным: обычный автор цитирует источники. Правила: не больше двух; только когда
ссылка реально подтверждает сказанное (номер ГОСТа, физический принцип, статистика);
НЕ в тот же абзац, где стоит наша ссылка; не на конкурентов и не на магазины. Если
уверенности в точном адресе страницы нет - не ставь ссылку вовсе, неработающий адрес
хуже её отсутствия.
...
def qa_multi(html, links):
    ...
    if len(auth) > 2:
        issues.append(f'авторитетных ссылок {len(auth)} (максимум 2 - больше выглядит рефератом)')
    for h in our:
