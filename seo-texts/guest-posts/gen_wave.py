...
# Первым - opus-4-8 (решение владельца 19.08). Раньше первым стоял gemini-3.6-flash:
# он отдаёт статью за ~16с против ~2 минут, и это имело смысл, пока черновик был
# заготовкой. Но если черновик проходит мех-QA с первого раза, доводки не будет и в
# публикацию уйдёт текст flash-модели целиком - так уже случилось с тремя статьями
# волны 1. У нас статьи с расчётами давлений, точек росы и коэффициентов
# одновременности; flash слабее держит фактическую точность и обобщает - ровно там,
# где потом ловят инженерные линзы. Разница в цене на всю волну ~4 $ при стоимости
# размещения в тысячи рублей, скорость при фоновом прогоне значения не имеет.
DRAFT_MODELS = ['claude-opus-4-8', 'openai:gemini-3.1-pro-preview', 'claude-haiku-4-5']
# В доводках flash тоже понижен: правка по мех-QA может задеть расчётную часть.
FIX_MODELS = ['claude-opus-4-8', 'openai:gemini-3.1-pro-preview', 'claude-haiku-4-5']
...
_JOBS_WAVE1 = [
 dict(slug='dizel-na-strojke', donor='kineshemec.ru',   # №7 + №13, пара производитель+каталог
  ...
  skeleton='проблемная постановка (типовая ошибка: станция впритык по расходу) -> расчёт от инструмента -> '
 ...
 dict(slug='azot-zernohranilishche', donor='kz24.news',   # №5 + №9 (enger: категория + подкатегория)
  ...
  skeleton='постановка задачи (потери при хранении и их причины) -> что кислород '
 ...
 dict(slug='kislorod-rybovodstvo', donor='gazetapetrovka.ru',   # №2 + №14
  ...
  skeleton='постановка задачи (замор как главный операционный риск) -> почему кислород лимитирует -> '
           'три источника и их экономика -> подбор от биомассы -> резервирование -> практическое резюме',
 ...
 dict(slug='kompressor-mehdvor', donor='gazetapervomaisk.ru',   # №1, 1 ссылка
  ...
  skeleton='постановка задачи (типовые потребители воздуха в мехдворе) -> задачи мехдвора -> '
           'поршневой против винтового честно -> подбор от инструмента -> отступление про паспорта '
           'старой техники -> практическое резюме с критериями',
 ...
 dict(slug='vozduh-po-schetchiku', donor='operativa.ru',   # №8, 1 ссылка
  ...
  skeleton='постановка задачи (воздух - единственная неучтённая утилита завода) -> воздух как четвёртая '
...
ВАЖНО про эти величины. Цифры бери отсюда, а ФОРМУЛИРОВКИ - нет. Справочник читают
все статьи волны, и если каждая перескажет его теми же словами, получится серия
одинаковых текстов: проверка по готовым статьям уже поймала 210 общих фраз, из них
самые частые - дословные обороты этого блока («рефрижераторный осушитель обеспечивает
точку росы», «кВт на каждый м³/мин»). Излагай величину так, как она нужна ЭТОЙ статье
и её читателю: где-то это таблица, где-то фраза в расчёте, где-то вообще упоминание
вскользь. И не тащи в текст все разделы справочника - только то, что нужно теме.

Структуру тоже не бери шаблоном. «Проблема -> решение -> параметры -> критерии подбора»
и финальный раздел «Критерии подбора» уже стоят в большинстве статей волны; если твоя
тема допускает другой ход - выбирай другой.
=== SEO (обязательно) ===
{seo}
...
ВАЖНО про сами запросы: список выше - это ПОИСКОВЫЕ ЗАПРОСЫ, а не готовые фразы для
текста. Люди пишут в поиск телеграфом («атлас копко хас 90 характеристики», «компрессор
винтовой воздушный цена»), и вставленный дословно такой запрос мгновенно выдаёт
SEO-текст. Каждый запрос сперва преврати в нормальную русскую фразу с согласованием
и правильным порядком слов; транслит брендов («хас», «берг») в тексте недопустим -
только латиницей, как пишет производитель. Если запрос не превращается в естественную
фразу - используй его тему, а не формулировку.
...
текста-якоря ссылки, если якорь брендовый).
ТОН (канон владельца 04.08): спокойный инженерно-деловой разбор с цифровой
конкретикой (диапазоны, классы, потери, коэффициенты - только проверенные). БЕЗ
сцен-диалогов, вымышленных звонков, «ударных» однострочных абзацев и разговорных
словечек. Образная фраза - максимум одна на статью, работающая на предмет.
Списки <ul>/<ol> и <h3> разрешены (2-3 списка на статью), но не три параллельные
нумерации подряд. Финал - практическое резюме с конкретикой, без моральной
...
def build_prompt(job):
    ...
    if allow:
        auth_note = f"""=== АВТОРИТЕТНЫЕ ИСТОЧНИКИ (по желанию, максимум {allow}) ===
Там, где утверждение опирается на норму или общеизвестный факт, можно поставить ссылку
на источник: ГОСТ/СНиП (docs.cntd.ru), Википедию (ru.wikipedia.org - только на статьи,
которые точно существуют, по точному названию), Росстат. Это делает материал
редакционным: обычный автор цитирует источники. ЖЁСТКИЕ рамки: не больше {allow} -
лимит площадки на ссылки в статье общий, и НАШИ ссылки в нём приоритетны, их число
менять нельзя; только когда ссылка реально подтверждает сказанное; НЕ в тот же абзац,
где стоит наша ссылка; не на конкурентов и не на магазины. Нет уверенности в точном
адресе страницы - не ставь вовсе, битая ссылка хуже её отсутствия.
"""
    else:
...
# Куда можно ссылаться как на источник. Домены, которые инженер и так бы процитировал.
# Конкретные страницы генератор не выдумывает - только домены из этого списка, и
# правило промпта требует ссылаться на существующие разделы (Википедия - на статью
# по точному названию, ГОСТ - на карточку документа по его номеру).
AUTHORITY_RX = re.compile(
    r'https://(ru\.wikipedia\.org|docs\.cntd\.ru|www\.consultant\.ru|'
    r'protect\.gost\.ru|www\.iso\.org|rosstat\.gov\.ru|www\.gost\.ru)/')


def qa_multi(html, links, auth_allow=2):
    ...
    issues += [i for i in qa_text.naturalness(html, text, low) if 'CV' not in i]
    hrefs = re.findall(r'href="([^"]+)"', html)
    allowed = [u.rstrip('/') for u, _ in links]
    # Авторитетные источники разрешены владельцем 19.08 («а если остальные добавить
    # на какие-нибудь убедительные авторитеты типа Википедии?»). Смысл: статья с
    # единственной - и той коммерческой - ссылкой выглядит ссылочной; ГОСТ или вики
    # рядом делают её редакционной. Волной 1 Gemini сам тянулся к docs.cntd.ru,
    # и гейт его резал - теперь это штатный приём, а не нарушение.
    our = [h for h in hrefs if not AUTHORITY_RX.match(h)]
    auth = [h for h in hrefs if AUTHORITY_RX.match(h)]
    if not (1 <= len(our) <= len(links)):
        issues.append(f'наших ссылок {len(our)} (нужно {len(links)})')
    if len(auth) > auth_allow:
        issues.append(f'авторитетных ссылок {len(auth)} (квота площадки {auth_allow}: '
                      f'лимит карточки общий, наши ссылки в нём первые)')
    for h in our:
    ...
    if len(links) > 1 and len(our) < len(links):
        issues.append(f'наших ссылок {len(our)}, а задано {len(links)} - добавь недостающую в другой раздел')
    ...
    if re.search(r'<(?!/?(p|h2|h3|a|strong|ul|ol|li)\b)[a-z]', html):
        issues.append('посторонние HTML-теги (разрешены p, h2, h3, a, strong, ul, ol, li)')
...
def gen_job(job, model_override=None, tag=None):
    ...
    for rnd in range(MAX_ROUNDS):
        ...
        try:
            ...
            continue
        issues = qa_multi(html, job['links'], job.get('auth_allow', 0))
        if not issues:
            break
