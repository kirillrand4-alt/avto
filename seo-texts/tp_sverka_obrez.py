...
def main():
    ...
    # Сверка идёт ПО ЦИФРАМ, а не поиском телефонной регуляркой в тексте, и это не мелочь.
    # Первая версия этой сверки искала номер в тексте регуляркой и напечатала «было 2406, стало
    # 946» — то есть будто в перевыкаченном файле 946 номеров всё ещё вне текста. Проверка по
    # цифрам дала ноль: регулярка сверки просто не узнаёт `+7 (3842) 78-01-82 *261` с
    # добавочным и пятизначные коды, которые сборщик берёт другим уровнем. Мерился один объект,
    # а вывод писался про другой — ровно класс «запрет шире, чем доказан».
    def ne_v_tekste(r):
        cd = D.sub('', r.get('comment') or '')
        return {c for c in nomera_polya(r) if c not in cd}

    nevidno_st = [t for t in obshch if ne_v_tekste(st[t])]
    nevidno_nv = [t for t in obshch if ne_v_tekste(nv[t])]
    print(f'было — карточек, где добытый номер отсутствует в сохранённом тексте: {len(nevidno_st)}')
    print(f'стало — таких же карточек:                                          {len(nevidno_nv)}')
    # Провайдеру уходили первые 1 500 знаков поля. Считаем, у скольких карточек номер стоит
    # дальше этого рубежа: это и есть карточки, о номере которых модель не могла знать.
    za1500 = sum(1 for t in obshch
                 if nomera_polya(nv[t]) - {c for c in nomera_polya(nv[t])
                                           if c in D.sub('', (nv[t].get('comment') or '')[:1500])})
    print(f'карточек, где номер стоит за 1500-м знаком полного текста: {za1500}')
    print('  (при старой отправке модель их не видела; теперь уходит полный текст)')
