# -*- coding: utf-8 -*-
"""ТЭК-ТОРГ: сбор закупок наших машин. НОВЫЙ канал, открытый по сигналу соседа.

ПОЧЕМУ ИМЕННО ЭТА ПЛОЩАДКА. 1-я сессия написала: из двух последних моих потоков ей пришло
ВСЕГО ПЯТЬ новых предприятий — значит ЕИС по словам выбрана, и надо менять канал, а не
обороты. Померила источники своего парка по доменам:

    zakupki.gov.ru   3 020 предприятий      tender.pro        82
    etpgpb.ru          355                  zakupki.mos.ru    43
    monitor-pb.ru      219                  rts-tender.ru     20
    roseltorg.ru        14                  ТЭК-ТОРГ          13
    Сбербанк-АСТ         0  (не собиралась ни разу)

ТЭК-Торг — площадка «Роснефти» и «Интер РАО», то есть ровно те предприятия, у которых стоят
ЦЕНТРОБЕЖНЫЕ машины, а собрано с неё 13 предприятий из 3 559. Канал не выбран.

ФОРМУ СПРОСИЛА У САМОЙ ПЛОЩАДКИ, а не подобрала: поле поиска называется `name`
(«Введите ключевое слово или…»), и проба с контролем это подтвердила:

    ?name=компрессор     21 карточка, текст 12 029 знаков
    ?name=щварцкопфер     0 карточек, текст  4 875 знаков   <-- контроль ЧИСТ
...
VYHOD = os.path.join(SCRATCH, 'PARK-TEKTORG-ZAKUPKI-3S.jsonl')
POISK = 'https://www.tektorg.ru/procedures?name=%s'
...
SPISOK = '''
var a = [].slice.call(document.querySelectorAll('a[href*="/procedures/"]'));
var vid = [];
...
window.__v = JSON.stringify({kartochek: vid.length, spisok: vid.slice(0, 60),
                             dlina: (document.body.innerText||'').length});
'''
...
for slovo in SLOVA:
    ...
    for x in celi:
        h = x['h']
        adres = h if h.startswith('http') else 'https://www.tektorg.ru' + h
        if adres in uzhe:
        ...
        novye.append({
            ...
            'inn_otkuda': 'стоит после слова ИНН на карточке ТЭК-Торга' if inn else '',
            ...
            'istochniki': adres + ' | ' + u,
            'istochnikov': 2,
            'kto': '3-я сессия, ТЭК-Торг по форме площадки'})
        sch['взято'] += 1
        time.sleep(0.4)
