...
def _data_nablyudeniya(tekst, poz):
    """(год, чем подтверждён) для контакта на позиции `poz`. Пусто честнее выдуманного."""
    okno = tekst[max(0, poz - 600):poz + 600]
    m = GOD.search(okno)
    if m:
        return {'god_iz_teksta': m.group(1),
                'chem_data': 'год в тексте рядом с контактом'}
    gody = set(GOD.findall(tekst or ''))
    if len(gody) == 1:
        return {'god_iz_teksta': gody.pop(), 'chem_data': 'единственный год на странице'}
    if gody:
        # Несколько разных годов и ни одного рядом: выбрать наугад значит выдумать свежесть.
        return {'god_iz_teksta': '',
                'chem_data': 'на странице годы %s, рядом с контактом ни одного'
                             % ','.join(sorted(gody))}
    return {'god_iz_teksta': '', 'chem_data': 'даты в тексте страницы нет'}
    ...
    for rx, tip in ((TEL, 'телефон'), (POCHTA, 'почта')):
        for m in rx.finditer(tekst):
            ...
            out.append({'znachenie': m.group(0).strip(), 'tip': tip,
                        ...
                        # ДАТА СЧИТАЕТСЯ ЗДЕСЬ, ПОКА ПОЛНЫЙ ТЕКСТ СТРАНИЦЫ ПОД РУКОЙ. Урок
                        # уже был усвоен на именном канале и НЕ перенесён сюда — замечание
                        # 1-й сессии по моим выкладкам: дата есть у 8 % строк обратного хода
                        # против 94 % у соседей, а без даты строка принимается, но В МЕРУ
                        # УСПЕХА НЕ ИДЁТ. Причина ровно та же, что тогда: в поток кладётся
                        # цитата в 800 знаков, а год стоит вне её — в шапке страницы, в имени
                        # файла, в подписи протокола. Считать надо в канале, а не в выкладке.
                        # Порядок от надёжного к слабому и ПОДПИСЫВАЕТСЯ, а не теряется.
                        **_data_nablyudeniya(tekst, m.start())})
    return out
