...
# ТРЕТИЙ ЗАХОД, и снова по образцу глазами. Второй дал у 188 карточек слово «Заблокирована»
# — это бейдж состояния, который стоит в том же заголовке, что и название. Позиция в
# разметке подводит меня четвёртый раз подряд, поэтому якорь беру СМЫСЛОВОЙ: в плоском
# тексте карточки название стоит РОВНО ПЕРЕД подписью «Местонахождение».
#
#     «… 44-ФЗ 223-ФЗ АКЦИОНЕРНОЕ ОБЩЕСТВО "САЛАВАТНЕФТЕМАШ" Местонахождение 453256, …»
#
# Такой якорь не зависит ни от порядка полей, ни от бейджей: подпись «Местонахождение»
# печатает сам ЕИС, и она названа словом, а не классом.
# ЧЕТВЁРТЫЙ ЗАХОД. Третий взял панель сортировки («Сортировать по: NAME Релевантности…»):
# поиск слева направо начинает совпадение за 200 знаков до подписи, и если имя короткое,
# в захват попадает всё, что было раньше. Режу иначе: беру кусок ПЕРЕД подписью и оставляю
# то, что идёт ПОСЛЕ последнего служебного маркера страницы.
#
# И главное — ставлю проверку САМОГО РЕЗУЛЬТАТА, которой не было все три раза: название
# организации обязано содержать правовую форму. Она поймала бы и адреса, и «Заблокирована»,
# и панель сортировки, не дожидаясь моего взгляда.
PERED_ADRESOM = re.compile(r'(.{6,300})Местонахождение', re.S)
MARKERY = re.compile(r".*(?:'>|Выгрузить результаты поиска|Подписаться на RSS-ленту|"
                     r"Релевантности|Наименованию|записей|Заблокирована|Иное)", re.S)
PRAVOVAYA_FORMA = re.compile(r'\b(ООО|ОАО|ЗАО|ПАО|НАО|АО|ФГУП|ГУП|МУП|ФГБУ|ФКУ|ГБУЗ|МБУ|'
                             r'АК|КОМБИНАТ|ЗАВОД|ОБЩЕСТВО|УЧРЕЖДЕНИЕ|ПРЕДПРИЯТИЕ|КОМПАНИЯ)\b',
                             re.I)
...
for inn in bez:
    ...
    if inn not in t:
        ...
        continue
    m = PERED_ADRESOM.search(t)
    nm = re.sub(r'\s+', ' ', m.group(1)).strip() if m else ''
    nm = MARKERY.sub('', nm).strip(' ,-')
    nm = re.sub(r'^(?:44-ФЗ|223-ФЗ|\s|,)+', '', nm).strip()
    if nm and not PRAVOVAYA_FORMA.search(nm):
        ishody['добытое не похоже на название: правовой формы нет'] += 1
        continue
