...
`claude/seo-texts-enrichment-prompt-449lyw` (HEAD `b2f9d5c`, 2026-07-27).
...
| Выгрузка Битрикса (CSV, 4.2 ГБ) | потоковое чтение csv | `bitrix-power.json`, `diz-brands-aggregates*.json`, `props-fill-table.csv` |
...
python3 crawl_pages.py            # всё, что помечено NO_TEXT / DUP*
...
даёт индекс `-1` и молча пропускается (`extract_segments.py:14`, `extract_priority.py:13`).
...
Ровно пять. Все требуют `PROVIDER_API_KEY` и модуль `anthropic` (см. §6.2):
...
Прогон 2026-07-27 в песочнице (на симлинках, репозиторий не тронут):

* `scan_ranges.py` → 248 несовпадений (в закоммиченном `range-mismatch.md` записано 246 —
  тексты с тех пор правились);
* `scan_ranges2.py` → «проверено 116 брендовых страниц, расхождения на 12 страницах» —
  совпадает с закоммиченным `range-mismatch-final.md` и с 12 строками `range-fix-queue.txt`;
...
| `brand_facets` | ссылки вида `<текущий URL>/<slug>/` из сайдбара (`:88`) |
| `page_brand` | хвост URL, если он не входит в список 26 «категорийных» баз `cat_bases` (`:92-100`) |
| `category` | функция `categorize` (`:18-44`) — 11 типов |
...
**Эта функция продублирована** в `build_payloads.py:56` и `build_compressor_payloads.py`
(там же комментарий про этот баг) — при правке надо менять во всех местах.
...
характеристик (`:17-19`). Даёт 532 непустых значения из 1017.
...
**Скрипта, который сливает эти источники в поле `city`, в репозитории нет** (grep по
всему репо: `city-prop.json` нигде не читается, поле `'city'` нигде не присваивается).
Судя по `wf-args.json` (26 путей на `city-batches/batch-NN.json` + массив `verify`),
финальные города доставали LLM-прогоном по батчам, запущенным ad-hoc через воркфлоу.
Каталог `city-batches/` — это его вход, 26 файлов по 40 кейсов (26 × 40 ≈ 1040 ≥ 1018).
...
Флаг ставится при потолке выше +30 %, ниже −30 % или поле ниже −30 % (`:76-79`).
...
из 645 страниц с `enger_category_url` ссылка уже стоит на 487, и скрипт при запуске
изменил бы ещё **89**. То есть последний массовый прогон текстов через него не проходил.
...
### 4.2. `city-batches/` — 26 файлов по 40 кейсов
...
| Файл | Что |
|---|---|
| `urls-indexable-all.txt` (14 007 URL) | выгрузка обхода (Screaming Frog) по всем сайтам группы |
| `acceptor_value.py` → `acceptor-value.json`, `acceptor-candidates.json` | коммерческое ранжирование страниц-акцепторов; B2B CTR-кривая `:22-29`, восстановление показов из кликов |
...
Три из четырёх скриптов читают Битрикс/прайс **по абсолютным путям в чужой скретчпад**
(`bitrix_prices.py:8`, `gap_models.py:28`) или в удалённый каталог uploads
(`acceptor_value.py:16`) — в текущей среде не запускаются без правки путей.

### 4.5. Чего в репозитории нет, но оно есть на дропе

Каталоги `inventory/` и `bitrix/` отсутствуют и на диске, и во всех ветках git
(проверено `git ls-tree -r` по 7 веткам, включая `origin/*`; `seo-texts/bitrix/`
к тому же в `.gitignore:3`). Оба лежат на файловом обменнике — проверено листингом
содержимого архивов прямо из потока:
...
`export_file_2ov3pw9ju90trsle.csv`, 4 214 568 961 байт (4.2 ГБ).
...
| `projects-index.json` | 2.4 МБ, 1018 записей | `crawl_projects.py:65`, перезаписывает `fix_cities.py:58` | `match_clients.py:29`, `region_aggregate.py:65`, `segment_master.py:8`, `build_compressor_payloads.py` |
| `projects-photos.json` | 55 записей | `crawl_project_photos.py:27` | `build_compressor_payloads.py:167` |
| `city-prop.json` | 532 непустых из 1017 | `fetch_city_prop.py:32` | **никто** |
...
11. **Колонки CSV ищутся по русскому имени.** Переименование колонки в базе обзвона
    или в выгрузке Битрикса не даст ошибки — даст пустую статистику
    (`extract_segments.py:14`, `extract_priority.py:13`).
...
13. **`crawl_pages.py:18-44` дублирует `categorize`** из `build_payloads.py:56`.
    Расхождение этих двух копий = расхождение категории в слепке и в payload'е.
...
`kb/manuf-raw/`. Каталог `kb/manuf-raw/` существует и содержит 20 файлов — значит,
результат когда-то был получен (возможно, рабочей версией скрипта или через
`check_domains.sh`, который наполняет тот же каталог через curl).

### 6.2. В этой среде не установлены `anthropic`, `httpx`, `openpyxl`, `playwright`

Проверено импортом. Следствия:

* все 5 API-скриптов (§2.5) падают ещё на `from gen_provider import …`, потому что
  `gen_provider.py:6-7` импортирует `anthropic` и `httpx`;
* `clean_keywords.py:6` и все скрипты `frog/`, читающие xlsx, падают на `openpyxl`;
* `fetch_playwright.py` — на `playwright` (если починить отступы).
...
ни `/root/.claude/uploads/` в этой среде не существуют (проверено `ls`).
На эти пути завязаны: `analyze_export.py:7`, `clean_keywords.py:10`,
`frog/acceptor_value.py:16`, `frog/bitrix_prices.py:8`, `frog/gap_models.py:28`,
`wf-args.json`, и первой строкой — `fetch_brand_pages.sh`, `fetch_categories.sh`,
`check_domains.sh`, `chain_tech.sh`, `bitrix_noncomp.sh`.
...
(проверил полный листинг 824 файлов по подстрокам `keywords`, `all_sites`, `props_export`, `xml`).
...
* `fetch_city_prop.py` → `city-prop.json` — не читается ничем (сравнение показало:
  753 значения поля `city` в `projects-index.json` против 532 в `city-prop.json`,
  797 совпадений из 1018, т.е. поле `city` собрано не из этого файла);
...
* `data/okved_directions.json` и `data/monotowns_1398r.json` — вызывающих нет.
...
в репозитории нет** (grep по всему репо даёт только чтение). Их, судя по структуре,
собирали одноразовым кодом поверх той же выгрузки Битрикса. Если выгрузка обновится,
пересобрать эталон будет нечем без переписывания.
...
Явно перечисляю всё, в чём не уверен. Пункты со звёздочкой — те, где скептику стоит
...
1. **\* «Скрипта, собирающего поле `city` в `projects-index.json`, нет».** Я проверил
   ...
   `['city'] =` не встречается) и по 7 веткам git — но искал по подстрокам. Код мог
...
5. **\* Инвентарь дропа.** Утверждения «файла `keywords_all_*.xlsx` на дропе нет»
   ...
   (на дропе 824 файла, я просматривал только совпадения по подстрокам).
...
7. Я **не запускал** ни один скрипт с провайдерским API. Про их работоспособность
   сужу только по коду и по наличию выходных файлов.
...
13. Каталог `frog/` описан обзорно. Логику `acceptor_value.py` (750+ строк) я
    прочитал только в первых 40 строках и по докстрингу — детали CTR-кривой и
    сегментации могут отличаться от моего пересказа.
