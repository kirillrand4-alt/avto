...
В репозитории нет ни главной ветки `main`/`master`, ни тегов, ни pull-request'ов.
Есть шесть веток вида `claude/<имя>`, каждая — след отдельной длинной сессии.
Работа НЕ сливалась через merge: ветки жили параллельно, а код доезжал до
результата двумя другими путями:

1. **Через боевой сервер.** Сессия готовила патч, клала его копией в
...
2. **Копированием файлов.** Часть содержимого просто скопирована из ветки в ветку.

Поэтому обычные вопросы «что не влито» здесь решаются не `git log`, а сравнением
СОДЕРЖИМОГО. Этот раздел даёт готовые ответы и команды для перепроверки.
...
| `seo-texts/sender-patches/` | 92 файла: слепки промежуточных патчей 24–26.07, которые ехали на сервер архивом | почти весь — архив; ОДИН подкаталог используется скриптом сборки (см. ниже) |
| `seo-texts/sender-divergent/` | 4 файла: версии из репозитория, которые РАЗОШЛИСЬ с боем и были бы затёрты синхронизацией; сохранены намеренно | архив, но внутри есть потерянная функциональность |
...
git checkout origin/claude/hopeful-galileo-n8gg7o -- \
  seo-texts/paket-poisk-v3 seo-texts/build_stats_problems.py \
...
**Почти всё это УЖЕ В HEAD** — приехало не мержем, а синхронизацией с боевого
сервера (`48ef189`). Проверено по датам: для КАЖДОГО из 56 файлов, которые
различаются, последний коммит в HEAD новее последнего коммита в ветке
(HEAD — 26–27.07, ветка — 20–24.07). Ни одного файла, где ветка новее, нет.
...
### 2. `seo-texts/sender-patches/` — 92 файла, 17 подкаталогов
...
|---|---|---|---|
| (корень) `cli.py`, `dtos.py`, `orchestrator.py`, `store.py`, `test_*.py` | очередь подтверждений: исходящие кампаний → `confirm.submit` | 6 | **влито** |
...
| `panel-redesign/` | дизайн-система панели (задача 47) | 17 | **влито в бой, но исходники CSS есть только здесь** ↓ |
...
| `obzvon-pagination/` | пагинация панели обзвона (ДРУГОЕ приложение) | 4 | **живой вход скрипта сборки**, но устарел ↓ |

Доказательства «влито» на примерах (можно перепроверить построчно):

* `store.mark_pending_review` из корневого патча — есть в бою:
  `seo-texts/sender/store.py:1160`.
...
ветке origin, и во всей истории git (`git log --all -- .../test_ai_letter.py`
пуст). При этом `sender/tests/test_ai_letter_meyer.py:4` пишет:
...
в `sender/tests/` нет. Базовое покрытие КЦ-генерации в боевом сьюте отсутствует.

#### `obzvon-pagination/` — единственный ЖИВОЙ вход, и он устарел
...
Ещё один потребитель — копипастом:
`seo-texts/server/enrich_panel/panel_core.py:210` (`parse_money` скопирован
оттуда как «единый канон разбора денег в проекте»).

**Грабля:** сегодня в 15:52 коммитом `7e2b968` в репозиторий положили СВЕЖИЕ
...
Четыре файла пересекаются, и **боевые везде больше патча**:
...
В боевом `callbase.py` есть, а в патче нет: `_backfill`, `_backfill_region`,
`_needs_derived`, `_backfill_derived` и шесть `CREATE INDEX IF NOT EXISTS`
(`ix_cc_queue`, `ix_cc_queue_active`, `ix_cc_base_region`, `ix_cc_base_okved`,
`ix_cc_base_equipment`, `ix_cc_needs_backfill`).

**Следствие: `bash build_panel_update.sh obzvon` сейчас соберёт пакет из
устаревших файлов и откатит боевую панель обзвона.** Перед сборкой обзвона
нужно либо перенести правки из `obzvon-src/` в `sender-patches/obzvon-pagination/`,
либо переучить `build_obzvon()` на `obzvon-src/` (там другая схема имён:
`obz__X` вместо `api__X`, разворот `${base//__//}` даст `obz/routes_obzvon.py`
вместо `api/routes_obzvon.py` — то есть просто подменить каталог НЕЛЬЗЯ).
...
**Потеряна функциональность «добавить ящик из панели».** Это не только роут:
в `sender-divergent/config.py.old-2026-07-23` есть методы `add_mailbox()` и
`load_mailbox_overrides()` (подхватывают ящики, добавленные из панели, из таблицы
`mailbox_overrides`), а обработчик — `sender-divergent/api__app.py.old-2026-07-23:679`.
Во всём боевом `seo-texts/sender/` строк `add_mailbox`, `load_mailbox_overrides`,
`mailbox_overrides`, `list_mailbox_overrides` **нет ни одной**
(grep по `*.py`, `*.ts`, `*.tsx`). Ящики теперь берутся только из yaml-конфига —
что согласуется с тем, что `mailboxes.kc.yaml` стал единственным источником
раскладки. Осознанное это решение или потеря — по коду не определить.
...
Всего отслеживаемых `.md` — **266**. Даты ниже — дата последнего коммита,
...
#### `seo-texts/docs/` (10 отслеживаемых + 3 в работе) — новая документация
...
#### `seo-texts/sender/` (27 файлов) — документация рассыльщика
...
68 файлов `brand-*.md` (по бренду на файл: страна, серии, готовые факты,
раздел «НЕ утверждать»), 7 `category-*.md`, сводки `BRENDY-SVODKA.md` (68 КБ),
...
* `seo-texts/paket-*` (5 каталогов) — разовые пакеты сдачи. **[ОТЧЁТ]**.
...
* **Слепки патчей** — `/home/user/avto/seo-texts/sender-patches/` (92 файла, ~2 МБ).
...
5. **`build_panel_update.sh obzvon` сейчас откатит панель обзвона.**
   Он пакует из `sender-patches/obzvon-pagination/` (25.07), а живое
   ушло вперёд (см. таблицу размеров). Схема имён в `obzvon-src/` другая,
   подменить каталог одной строкой нельзя.
...
* Всё в `seo-texts/sender-patches/`, **кроме** `obzvon-pagination/`, — архив.
  Ни один скрипт эти файлы не читает: `grep -rn "sender-patches"` по репозиторию
  даёт только `build_panel_update.sh:18,84,96` (обзвон),
  `enrich_panel/panel_core.py:210` и `enrich_panel/README.md:43`
  (копипаст `parse_money` из obzvon-части) и `sender-data/README.md:8`
  (историческая ссылка). Ценность архива — не код, а `.md`-объяснения и три
  файла без копий (`ai_gen_quota.py`, два `test_ai_letter.py`).
* Всё в `seo-texts/sender-divergent/` — архив по определению.
  Единственная ссылка: `server/PANEL-DEPLOY.md:92`.
...
**Утраченная функциональность (подтверждено грепом по всему `sender/`):**

* Добавление ящика из панели: `POST /mailboxes` + `Config.add_mailbox` +
  `Config.load_mailbox_overrides` + таблица `mailbox_overrides`.
  Живёт только в `sender-divergent/`.
* Ручная отправка одного письма через экран Compose (`POST /send/manual`).
  Заменена очередью подтверждений — но это МОЯ реконструкция по коду,
  подтверждения владельца нет.
* Тесты КЦ-генерации `tests/test_ai_letter.py` (14–16 тестов).
  Нет ни в дереве, ни в одной ветке, ни в истории git.
...
* `sender/web/dist/` содержит только `index.html` (433 б) — бандл в
  `sender/web/.gitignore` (`dist/`). Собранного фронта в репозитории нет и
  быть не должно.
* `sender/tests/` содержит 69 тестовых файлов, но `test_ai_letter.py` среди них
  нет (см. выше) — это не забытая чистка, файла не было никогда.

---

## Что не проверено

1. **Я не запускал ни одного теста.** Утверждения о покрытии сделаны по именам
   функций и грепу, а не по прогону pytest. Возможно, часть проверок из
   потерянного `test_ai_letter.py` дублируется под другими именами в
   `test_gate_fixes.py` или `test_ai_letter_meyer.py` — я сверял только точные
   имена функций.
...
11. **Причины, по которым ветки не мержились, мне неизвестны.** Возможно,
    это осознанная политика (каждая сессия — своя ветка, слияние через сервер),
    возможно — накопившийся долг. Решение о слиянии принимает владелец.
