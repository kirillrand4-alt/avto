...
**Ревизия-скептик 2026-07-27 (вторая сессия, HEAD `7e0d60f`).** Документ
перепроверен по коду построчно; исправленные утверждения помечены
**[исправлено]**, доведённые до фактической проверки — **[проверено]**.
Ключевые константы раннера (`ALLOW`, `PULL_ALLOW`, `JOB_TIMEOUT`, `WORKERS`,
`RUNNER_HEAVY`, `_SEM_HEAVY`) дополнительно сверены по ВСЕМ веткам
`origin` (`git for-each-ref` + `git show <ветка>:seo-texts/server/job_runner.py`):
файл существует только в двух ветках (`…449lyw`, `…youthful-sagan-ny4fm6`)
и в них идентичен. Про боевую копию на `C:\sender\server` это по-прежнему
ничего не говорит — см. раздел 7.
...
Лаунчеры — тонкие драйверы над `run_on_server.submit()`. **[исправлено]** Они
двух разных типов, в первой редакции это было слито в один: `launch_*.py` режут
СВОЙ список компаний на порции и шлют компании в args
(`launch_sales_enrich.py:114-121` и аналоги), а `mass_enrich_loop.py/.sh` списка
не шлют вовсе — они гонят задания `{'mass_base': True, 'cap': N}`
(`mass_enrich_loop.py:30-33`), а кого именно обрабатывать, решает сам
`enrich_contacts` на сервере (берёт необработанные из базы).
...
**[исправлено]** CLI требует НЕ МЕНЕЕ двух аргументов: `<task> <args-json>`
(проверка — `len(sys.argv) < 3`, `run_on_server.py:87-89`); лишние
позиционные аргументы просто игнорируются, а не вызывают ошибку.
...
R._load_secret_from_drop()          # НЕОБЯЗАТЕЛЬНО: submit() зовёт его сам
res = R.submit('enrich_contacts', {...args...}, wait=True, poll=15, timeout=1800)
```

**[исправлено]** Ручной вызов `_load_secret_from_drop()` не нужен: `submit()`
делает его первой же строкой (`run_on_server.py:52`), а сама функция выходит
сразу, если `JOB_SECRET` уже есть (`:28-29`). В лаунчерах он встречается
(`mass_enrich_loop.py:26`) как страховка, не как требование.
...
**[исправлено] В ТЕКУЩЕЙ песочнице эта команда не сработает.** Проверено
2026-07-27: `fastapi 0.140.2` и `openpyxl 3.1.5` установлены, но НЕТ
`python-multipart` (без него `create_app()` падает на `@app.post('/upload')` с
`RuntimeError: Form data requires "python-multipart"`, `enrich_panel.py:142`) и
НЕТ `uvicorn` (`enrich_panel.py:314`). Для локального запуска сначала
`pip install python-multipart uvicorn`. На сервере обе зависимости ставятся
по `enrich_panel/README.md:126`.
...
`job_runner.sig_ok` покрыта тестом — **[исправлено]**, ссылка теперь на сам тест,
а не на README: `enrich_panel/tests/test_topup.py:40-59`
(`test_signature_accepted_by_real_job_runner` импортирует настоящий
`job_runner`, подменяет `JOB_SECRET`, проверяет `sig_ok(job)`, ломает args и
проверяет, что подпись перестала сходиться, плюс `job['task'] in job_runner.ALLOW`).
Этот тест в песочнице проходит.
...
захватывается (единственное вхождение имени — строка 234). Его
роль забрал отдельный heavy-пул после инцидента 2026-07-24 (комментарий `:253-255`).
**[проверено]** Ревизия повторила `grep` по всему рабочему дереву И по всем
веткам `origin` (`git grep _SEM_HEAVY <ветка>`): вхождение везде одно, строка
объявления. Про боевую копию файла на сервере это не говорит ничего.
...
недоделанные джобы висели бы вечно (инцидент 2026-07-22, комментарий `:313-315`
— **[исправлено]**, в первой редакции стояло `:314-316`).
...
**[исправлено]** Наружу (443/`/drop`) он выставлен через обратный прокси —
это **Caddy**, и конфиг лежит НЕ в репозитории, а на сервере: `C:\Caddyfile`.
Прямые указания в коде: `_ops_caddy_reload.py:7-8` (`CADDY = 'C:\caddy.exe'`,
`CFG = 'C:\Caddyfile'`, перезагрузка `caddy reload --config … --adapter caddyfile`)
и `_ops_caddy_track.py:20-21` (`CADDY = 'C:\\Caddyfile'`, якорь вставки —
`ANCHOR = b'reverse_proxy 127.0.0.1:8013'`). Из якоря следует ещё один факт:
в боевом Caddyfile УЖЕ есть блок `reverse_proxy` на порт 8013, то есть маршрут
панели обогащения `/enrich/*` в прокси заведён. Самого текста Caddyfile я не
читал — только пути и якорь (в первой редакции это было записано как
«косвенные признаки», что занижало доказательность).
...
**Конфигурация (env)** — `panel_core.py:38-73`, `enrich_panel.py:70,74,315`
(**[исправлено]**: порт читается на строке 315, не 314):
...
health `http://127.0.0.1:8012/obzvon/kc` (`:22`; маршруты смонтированы под
`/obzvon` даже локально — комментарий `:12-13`, **[исправлено]** было `:11-13`),
архив меньше 500 байт → отмена (`:44`). Бэкап
...
| `seo-texts/server/*.md` | **[исправлено]** их шесть: `RUNNER-SETUP.md`, `PANEL-DEPLOY.md`, `ENRICH-SALES-BASE-PROMPT.md`, `NIGHT-RUN-STATUS.md`, `ENRICH-ROADMAP.md`, `NEWS-LEADS-PIPELINE.md` (последний в первой редакции был потерян; проверено `git ls-files seo-texts/server/`) |

**[проверено]** Про `_ops_*.py` — механизм подтверждён кодом-вызывающим, это уже
не предположение: `preflight_panel.py:56-61` кладёт скрипт через
`panel_file_put` в `C:\sender\_ops\_preflight.py` и запускает его через
`panel_py`; тот же канон описан в `ENRICH-SALES-BASE-PROMPT.md:33-35`. Иначе и
нельзя: `panel_file_put` пишет только под `C:\sender` (`enrich_contacts.py:5895`),
`panel_py` запускает только оттуда же (`:6235`). Сами `_ops_*.py` при этом
оперируют путями за пределами `C:\sender` (например `_ops_pull_obzvon.py:7-8` —
`C:\seostat\app` → `C:\seostat\drop\drop-storage`), что и подтверждает: они
исполняются НА СЕРВЕРЕ, а не в песочнице.
...
| `C:\sender\enrich.db` | **[исправлено]** SQLite обогащения. По схеме `enrich_db.py:23-45` таблиц ШЕСТЬ: `companies`, `emails`, `signals`, `donors`, `seen_news`, `stage_log` (+ индексы) — в первой редакции были перечислены только три, которые читает панель. Сверх того `ENRICH-SALES-BASE-PROMPT.md:47-49` упоминает созданную ранее `phone_contacts`, которой в `_SCHEMA` нет. Живую БД я не видел: там может быть ещё больше таблиц/колонок |
...
| `C:\seostat\drop\drop-storage\` | хранилище дропа и тяжёлые исходники. **[исправлено]** ссылка была на `ENRICH-SALES-BASE-PROMPT.md:57-58`, фактически это строка **52** (строка 58 — про конкретный файл `centrifugal-core-inns.txt` внутри). Подтверждение в коде: `_ops_pull_obzvon.py:7-8` (`DST = r'C:\seostat\drop\drop-storage'`) |
| `<DIR>/<script>.out` | лог detached-процесса из `spawn_campaign` (`job_runner.py:182`) |

### Живое состояние дропа на момент сверки (read-only `list`)

Снимок №1, 2026-07-27 ~15:20 UTC: всего 826 файлов, `job-*.json` — 55 (из них
29 панельных), `result-*.json` — 56.

**[проверено]** Снимок №2 ревизии, 2026-07-27 17:06 UTC:

- всего файлов: **840**;
- `job-*.json` (невыполненные задания): **53**, из них **29 панельных**
  (`job-1785080855-r1-base-b*.json`, все со временем 2026-07-26 15:47 UTC) и 24
  лаунчерных/сессионных;
- `result-*.json` (незабранные результаты): **58**;
- `runner-secrets.env` на дропе присутствует (GET по нему отвечает 200) —
  отсюда `run_on_server` подтягивает `JOB_SECRET`.

Важное наблюдение между снимками: общий поток идёт (файлов стало больше, часть
джобов исполнилась), но **те же 29 панельных заданий не сдвинулись за ~25 часов**.
Это усиливает гипотезу о зависании из раздела 7, но диагнозом по-прежнему не
является. Любой следующий читатель увидит уже другие числа — команда:
`bash seo-texts/server/drop_client.sh list`.
...
задания прогона №1. Это не мёртвый код. **[дополнено]** Косвенное подтверждение
с другой стороны: в боевом `C:\Caddyfile` есть блок `reverse_proxy 127.0.0.1:8013`
(он служит якорем вставки в `_ops_caddy_track.py:21`), то есть внешний маршрут
`/enrich/*` заведён. Что служба `EnrichPanel` поднята ПРЯМО СЕЙЧАС, по-прежнему
не доказано.
...
9. **`/list` дропа не фильтрует имена по `SAFE_NAME`, а `GET`/`PUT`/`DELETE` —
   ...
   через маршрут нельзя (400). **[проверено]** В первой редакции 400 был
   предсказанием по коду; ревизия проверила экспериментально (read-only GET):
   `GET /New%20Text%20Document.txt` → **400**, `GET /runner-secrets.env` → **200**.
...
12. **Батч должен помещаться в 1800 с.** `launch_core_enrich.py` шлёт все 396
    компаний одним заданием и ждёт 3000 с (`launch_core_enrich.py:30-41`).
    Об этом прямо написано в `launch_core_chunked.py:5-14` как о причине его
    существования. **[исправлено]** формулировка «задание ГАРАНТИРОВАННО умрёт»
    была сильнее фактов. Точнее так: у задания стоит `resume: True` поверх общего
    `stream_file` `enrich_core2.jsonl` (`:37-38`), поэтому реально обрабатываются
    только НЕсделанные ИНН, и объём зависит от состояния стрима. Расчёт при
    полном прогоне: замеренная скорость 2.5-2.67 компании/мин
    (`launch_sales_enrich.py:62-63`) → 396 компаний ≈ 150 минут ≫ 30 минут
    JOB_TIMEOUT. То есть таймаут почти неизбежен, но это РАСЧЁТ, а не
    наблюдённый прогон.
...
- **`launch_core_enrich.py` — фактически устаревший лаунчер.** Его заменил
  ...
  **[исправлено]** «заведомо получить таймаут» → по расчёту таймаут почти
  неизбежен (см. грабли, п.12), но фактического прогона ни первая редакция, ни
  ревизия не делали; `resume` может сократить объём работы до влезающего в
  30 минут. Второй дефект подтверждён точно: `extract_model` в args не
  передаётся (`launch_core_enrich.py:30-39`), значит модель = `claude-fable-5`
  (`enrich_contacts.py:6649-6650`).
...
- **Все `launch_*.py`, `mass_enrich_loop.*`, `preflight_panel.py`,
  ...
  их не импортирует (проверено `grep` по всему репо; ревизия повторила поиск —
  все упоминания в других файлах это строки «Использование:» в докстрингах,
  `PANEL-DEPLOY.md:69,83` и `obzvon-centro/README.md:155,164`). Единственное
  исключение — **[исправлено]** направление импорта в первой редакции было
  указано наоборот: это `launch_core_chunked.py:29` импортирует `build_args`
  ИЗ `launch_sales_enrich`, а не наоборот.
  Это НЕ мёртвый код: их запускает человек/сессия командой. Но и автоматики,
  которая бы их дёргала, в репозитории нет.

- **`run_on_server.py` используют, помимо лаунчеров, ещё два модуля вне этой
  области** (**[исправлено]**, было «три» — третий импортирует не тот модуль):
  `seo-texts/night_orchestrator.py:52` и `seo-texts/heartbeat.py:15`.
  Отдельно `job_runner` импортируют `seo-texts/server/news_schedule/news_cron.py:46`
  (в `try/except`, ТОЛЬКО ради побочного эффекта — загрузки `runner-secrets.env`,
  комментарий `:41-44`) и тест `enrich_panel/tests/test_topup.py:45`.
...
- **Тесты панели в песочнице проходят не полностью — из-за отсутствия
  зависимостей, а не из-за кода.** **[исправлено]** цифры и причина в первой
  редакции неверны. Фактический прогон ревизии (2026-07-27 17:0x,
  `python3 -m pytest tests/ -q` в `seo-texts/server/enrich_panel`):
  **44 passed, 1 failed, 20 errors** (итого 65 — README:192 заявляет 65, сходится).
  Причина ВСЕХ 21 непрошедших — одна и та же: `RuntimeError: Form data requires
  "python-multipart" to be installed` (падает импорт `enrich_panel` на
  `@app.post('/upload')`). `fastapi` (0.140.2) и `openpyxl` (3.1.5) в песочнице
  УСТАНОВЛЕНЫ, вопреки первой редакции; нет `python-multipart` и `uvicorn`.
  Чистая логика (`panel_core`) проходит целиком, включая тест совместимости
  подписи с настоящим `job_runner`. Единственный `failed` —
  `tests/test_auth.py::test_no_users_configured_503`, падает по той же причине
  (фикстура `client` не может импортировать модуль).
  Лечится `pip install python-multipart uvicorn`.
...
- **Скрипта, собирающего `enrich-panel.zip`, в репозитории нет.**
  ...
  Установка панели обогащения описана в `enrich_panel/README.md:109-140` как
  ручная процедура, причём README:112-113 прямо говорит «сессия собирает
  `enrich-panel.zip`» — то есть архив предполагается собирать руками каждый раз.
  **[проверено]** Ревизия повторила поиск не только по рабочему дереву, но и по
  ВСЕМ веткам `origin` (`git grep -l -i 'EnrichPanel\|enrich-panel.zip' <ветка>`):
  совпадения только в `enrich_panel/README.md` и в файлах `seo-texts/docs/`.
  Скрипта нигде нет. **Уточнение:** сама выкатка zip-ом технически возможна и
  без нового скрипта — `panel_zip_deploy` принимает имя архива и имя службы
  (`enrich_contacts.py:6161-6162`), но распаковывает всегда в `C:\sender`
  (`:6212`) и HTTP-пробу делает по 8091 (`:6220`), то есть для `EnrichPanel`
  результат пробы будет чужим.
...
- **НЕ ПРОВЕРЕНО: почему 29 панельных заданий висят с 26.07 15:47.**
  ...
  содержимым `.runner-seen.json` на сервере. **[дополнено]** Ревизия сняла
  второй листинг дропа спустя ~1,8 часа (и ~25 часов после постановки джобов):
  общее число файлов выросло, часть других заданий исполнилась, а эти 29 —
  на месте. То есть очередь в целом движется, стоят именно они.
...
- **ЧАСТИЧНО ПРОВЕРЕНО [исправлено]: обратный прокси.** `drop_server.py` слушает
  `127.0.0.1:8787`, наружу его выставляет **Caddy** — это уже не «косвенный
  признак»: `_ops_caddy_reload.py:7-8` и `_ops_caddy_track.py:20-21` жёстко
  задают `C:\caddy.exe` и конфиг `C:\Caddyfile`, а якорь вставки
  `reverse_proxy 127.0.0.1:8013` показывает, что маршрут панели обогащения в
  этом конфиге уже есть. НЕ ПРОВЕРЕНО: сам текст `C:\Caddyfile` (в репозитории
  его нет и быть не должно) и то, каким именно блоком проксируется `/drop`.
- **НЕ ПРОВЕРЕНО: содержимое `enrich.db`, `obzvon-index.db`, `stage_log`.**
  Живых БД я не видел. Все названия таблиц и колонок взяты из кода
  (`enrich_db.py:23-45` — канонная схема, шесть таблиц) и фикстур тестов
  (`enrich_panel/tests/conftest.py:21-31` — намеренно УРЕЗАННАЯ копия трёх
  таблиц, которые читает панель). Реальная схема на сервере может содержать
  больше таблиц и колонок — например `phone_contacts`
  (`ENRICH-SALES-BASE-PROMPT.md:47-49`), которой в `_SCHEMA` нет. Никаких
  выводов вида «такой таблицы/колонки не существует» из этого документа делать
  нельзя.
...
- **`_ops_*.py` — механизм ПОДТВЕРЖДЁН [исправлено], содержимое нет.** Способ
  доставки и запуска больше не предположение: `preflight_panel.py:56-61` —
  живой пример «`panel_file_put` в `C:\sender\_ops\…` → `panel_py`», тот же
  канон в `ENRICH-SALES-BASE-PROMPT.md:33-35`. Ни один `_ops_*.py` не
  импортирует `run_on_server` (`grep`), то есть они исполняются на сервере, а не
  в песочнице. НЕ ПРОВЕРЕНО: что делает каждый из 20 файлов — читались только
  шапки `_ops_pixel_e2e.py`, `_ops_caddy_track.py`, `_ops_caddy_reload.py`,
  `_ops_find_caddy.py`, `_ops_pull_obzvon.py`.
- **НЕ ПРОВЕРЕНО: работают ли `.ps1` на сервере сейчас.** PowerShell я не
  запускал. **[проверено]** BOM — проверен побайтово: оба файла в git начинаются
  с `EF BB BF` (UTF-8 BOM), требование `update-panel.ps1:15-16` /
  `update-obzvon.ps1:14` соблюдено.
