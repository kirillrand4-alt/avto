...
for o in vybor:
    ...
    if nashli_vid and nashli_inn:
        ...
        # ДВА РАЗНЫХ ИСХОДА, которые прежде складывались в один. «Предприятие не названо»
        # может значить, что на странице его правда нет, — а может, что имени НЕТ У МЕНЯ и
        # сравнивать было не с чем. Второе — дефект моих данных, а не страницы, и лечится
        # справочником, а не пересбором ссылок. Тот же класс ошибки уже стоил мне 34
        # «частично» из 48 в замере по доменам.
        v = ('машина есть, а ИМЕНИ ПРЕДПРИЯТИЯ У МЕНЯ НЕТ — сравнивать не с чем'
             if not imena.get(o.get('inn'))
             else 'машина есть, предприятие на странице не названо ни ИНН, ни именем')
    elif otkrylos:
