...
def main():
    ...
    for line in [
        ...
        ("", False),
        ("Как читать метрики пересечения:", True),
        ("Коэффициент перекрытия = общих запросов / размер МЕНЬШЕГО набора. "
         "Показывает, насколько маленький сайт поглощён большим. 100% — вся "
         "семантика меньшего есть у большего.", False),
        ("Жаккар = общих запросов / объединение обоих наборов. Показывает "
         "похожесть сайтов как равных; сильно штрафует разницу в размерах.", False),
        ("Пример: prokompressor.ru (64 532 запроса) и vladivostok.prokompressor.ru "
         "(262), общих 228. Перекрытие 228/262 = 87% — поддомен почти целиком "
         "внутри головного. Жаккар 228/64 566 = 0,4% — на его фоне поддомен мал. "
         "Обе цифры верны, вопросы разные.", False),
        ("Высокое перекрытие при низком Жаккаре — спутник живёт внутри семантики "
         "старшего (норма). Оба высокие — сайты делают одно и то же: berg-пара "
         "(36,8 / 16,2), meyer-corp и usort (57,6 / 15,6), usort и "
         "vsefotoseparatory (54,0 / 15,4).", False),
        ("Обе метрики считают штуки запросов, а не трафик: хвост опечаток весит "
         "столько же, сколько ВЧ-запрос. Колонки «показов A/B на общих» — "
         "то же пересечение, но взвешенное по показам.", False),
        ("", False),
