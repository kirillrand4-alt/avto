...
# ЧЕТВЁРТАЯ ЦЕЛЬ, и она нашлась потому, что прогон честно упёрся в ноль. Заход по сериям
# напечатал «серий с принципом „не установлен“: 388, не разобрано прежде: 0» — цель
# исчерпана. Но в шапке этого же скрипта стояло допущение: «каталожные модели владельца
# (2 920) не трогаю: у них принцип уже известен из его же каталога». Живой файл допущение
# опровергает:
#
#     строк словаря                                    3906
#     принцип «не установлен» ВСЕГО                    1689
#        из них серия из документов                     213
#        из них серия из предмета закупки               175
#        из них МОДЕЛЬ КАТАЛОГА                        1301   <- допущение было неверным
#
# Из этих 1 301: 758 вообще не наши машины (фильтры, картриджи, ресиверы, сепараторы —
# у них принципа сжатия и не бывает), а 543 наши, и среди них 184 названы КОМПРЕССОРОМ.
# Компрессор с неназванным принципом — прямая цель владельца: если хоть часть окажется
# центробежной, это приоритетная машина, лежавшая неразобранной.
NASHI_MASHINY = re.compile(r'компрессор|воздуходув|нагнетател|турбо|генератор\s+азот|'
                           r'генератор\s+кислор|\bВРУ\b|\bГПА\b', re.I)
TOLKO_KATALOG = os.environ.get('P25_TOLKO_KATALOG') == '1'

celi = []
with io.open(SLOVAR, encoding='utf-8-sig') as f:
    for r in csv.DictReader(f, delimiter=';'):
        if TOLKO_KATALOG:
            if (r.get('vid_zapisi') or '').strip() != 'модель каталога':
                continue
            if (r.get('princip') or '').strip() != 'не установлен':
                continue
            if not NASHI_MASHINY.search((r.get('vid_mashiny') or '')):
                continue
            if (r.get('princip_provajder') or '').strip():
                continue
        elif TOLKO_VID:
...
CHTO_MERIM = ('моделей КАТАЛОГА: наша машина, а принцип не назван' if TOLKO_KATALOG else
              'строк, где не установлен ВИД машины' if TOLKO_VID else
