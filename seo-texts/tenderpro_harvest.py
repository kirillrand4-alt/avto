...
# Замер по корпусу 9 021 карточки, 30.07.2026: из четырнадцати слов этого рубежа **двенадцать
# не отсеяли ни одного номера** — оба уровня шаблона требуют код страны, а `id1190815` его не
# имеет, поэтому реквизитные строки до рубежа просто не доходят. Работали только два слова, и
# оба во вред: «тендер» отсеял 4 ЖИВЫХ номера («Специалист по тендерам +7 (3842) 78-01-82»),
# «№» — один живой мобильный («моб.№ 89085719649»), для которого пришлось делать отмену.
# «тендер» убрано (замер третьей сессии, подтверждён у нас независимо). Остальные слова
# оставлены как дешёвая страховка на случай другого корпуса, но иллюзий про них быть не должно:
# по этим данным они не работают.
...
def s_otkryvayushchey(txt, m):
    """Вернуть найденный номер, добрав открывающую скобку, если она осталась слева.

    Нашла третья сессия, дефект в моём модуле. Шаблон начинает совпадение с `\b8`, и на записи
    «(8216) 76-20-60» этой восьмёркой оказывается ПЕРВАЯ ЦИФРА КОДА ГОРОДА внутри скобок —
    открывающая остаётся снаружи, закрывающая уезжает в результат: «8216) 76-20-60».
    В карточках таких 551.

    Цифры при этом верные (код плюс номер — те же десять знаков), поэтому распознавание
    мобильного не ломается и звонить по такому номеру можно. Дефект косметический, но продавец
    видит рваную строку и не понимает, доверять ей или нет. Правим отображение, а не разбор:
    трогать шаблон опаснее, он пережил замер на 9 021 карточке.
    """
    tekst = m.group(0).strip()
    if ')' in tekst and '(' not in tekst and m.start() > 0 and txt[m.start() - 1] == '(':
        return '(' + tekst
    return tekst
...
def telefony_iz_teksta(txt):
    ...
    for m in TEL_YASNO.finditer(txt):
        ...
        if TEL_PERED.search(pered) and not TEL_PERED_OTMENA.search(pered):
            continue
        vzyato.add(c)
        out.append(s_otkryvayushchey(txt, m))
...
def spisok_kompanii(cid, slova, predel=400, pauza=1.5):
    """Закупки одной компании. `slova` — список слов-фильтров; **пустая строка в списке означает
    БЕЗ фильтра, то есть всё, что компания вообще закупала.**

    Почему это важнее, чем кажется. Приём «страница компании» мы взяли, а словарь внутри него
    оставили — и он остался тем же узким местом, за которое мы ругали словарный обход. Замер
    03.08 по уже обойдённым компаниям:
        РУСАЛ Менеджмент  по словам 1 833  →  без слова около 173 775
        ООО «ИСО»         по словам   930  →  без слова около  64 375
        Черкизово         по словам   654  →  без слова около  73 325
        Химпром           по словам   647  →  без слова около  16 525
    Та же пропорция примерно 1/50, которую мы сами и назвали, но не устранили. Закупка
    называется «ремонт ТВ-80», «запчасти к К-500», «поставка агрегата 4МСГП» — десять слов их
    не догоняют, а страница компании отдаёт всё.

    Мусор отсеивает `tip_mashiny.py` — это его работа и он на ней проверен. Словарь на входе
    отсекает мусор вместе с целью, и цену этого мы уже измерили.

    Чужие строки считаются отдельно: если сужение потечёт, это должно быть видно числом, а не
    обнаружиться потом в панели.
    """
    ...
    for slovo in slova:
        ...
        for p in range(stranic):
            ...
            for b in BLOK_KOMP.findall(h or ''):
                ...
                r['klyuch'] = slovo or 'без слова, вся страница компании'
