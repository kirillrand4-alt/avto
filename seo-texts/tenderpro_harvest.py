...
# Слово «тендер» из этого списка убрано 30.07.2026 по замеру на 9 021 перевыкаченной карточке.
# Оно отсеивало ровно пять номеров по всему корпусу, и все пять живые:
#   «ЛУКЬЯНОВ Владимир Андреевич Специалист по тендерам +7 (3842) 78-01-82» — три карточки ОКС;
#   «По вопросам участия в тендере- 8-3843-99-37-33 доб. 134,133» — ТопПром;
#   «участия в тендере 8-35161-94869» — на спорном уровне.
# Защищать ему при этом нечего: остальные реквизитные слова (id, ИНН, ОГРН, №, лот, заказ) не
# отсеяли по корпусу НИ ОДНОГО номера, потому что оба уровня требуют код страны, а `id1190815`
# его не имеет. На этой площадке «тендер» — обычное слово почти в каждой карточке, а не
# реквизитный префикс, приклеенный к цифрам.
TEL_PERED = re.compile(r'(?:id|ИНН|ОГРН|КПП|р/с|к/с|БИК|цена|сумма|№|лот|заказ|инв)'
                       r'\D{0,4}$', re.I)
...
def bez_tegov(s):
    """Разметка долой, сущности раскодировать, пробелы свести.

    Раскодировка добавлена 30.07.2026 и она не косметическая: `&nbsp;` внутри номера ломает
    ...
    Замер по 9 021 перевыкаченной карточке: сущности встречаются в 3 014 карточках (17 128 раз
    один `&nbsp;`), и раскодировка достаёт **101 номер на 101 карточке**, которых не было вовсе.
    Первый же из них — целевой: «Инженер-энергетик Волков Максим Валерьевич … Тел.
    +7 924 228 04 44» (ТехноНИКОЛЬ Дальний Восток).

    Порядок важен: сначала снять разметку, потом раскодировать. Наоборот `&lt;div&gt;` превратится
    в `<div>` и будет снято как тег вместе с содержимым — а это 340 случаев экранированного
    текста, который надо сохранить.
    """
...
def spisok(threads, max_stranic):
    ...
    def odna(z):
        ...
        #
        # ЗАСЛОН БЫЛ МЁРТВЫМ, И ЭТО ХУЖЕ, ЧЕМ ЕГО ОТСУТСТВИЕ. Проверялось `h is None`, а
        # `vzyat` при провале возвращает СТРОКУ «__ОШИБКА__ ...», а не None, — значит
        # ветка не срабатывала ни разу, `upalo` всегда печатался нулём, и упавшие
        # страницы по-прежнему исчезали, но теперь под отчёт «УПАЛО 0». Сборщик карточек
        # ниже (строка с `h.startswith('__ОШИБКА__')`) делал это правильно всё время —
        # то есть верный образец лежал в том же файле в двадцати строках ниже.
        # Мало написать заслон: надо проверить, что его кто-то спрашивает.
...
def kartochki(threads, out=None, zanovo=False):
    ...
    # Заслон против сдвига колонок при дописывании. Соседняя сессия 30.07.2026 испортила
    # 3 597 строк ровно так: в заголовке файла оказалось 12 колонок (`tp_dogon.py` дописал
    # `telefony_tekst_staryy`), а дописывающий писатель работал по жёсткому списку из 11 — и
    # значения уехали на одну позицию, текст комментария попал в колонку «почты», а `comment`
    # стал строкой 'None'. Файл при этом читался без ошибок, строк было ровно столько же,
    # заголовок на месте: поломка выглядела как нормальные данные. Поэтому — не «подстроиться»,
    # а остановиться с объяснением: молчаливое приспособление и есть механизм порчи.
    if not novyy:
        with open(kart, encoding='utf-8-sig', newline='') as pr:
            bylo = next(csv.reader(pr, delimiter=';'), [])
        if bylo != cols:
            sys.exit(f'заголовок {kart} не совпадает с набором колонок сборщика — дописывать '
                     f'нельзя, значения сдвинутся.\n  в файле:   {bylo}\n  у сборщика: {cols}\n'
                     f'  либо --zanovo (перевыкачать в новый файл), либо привести файл к '
                     f'набору сборщика.')
